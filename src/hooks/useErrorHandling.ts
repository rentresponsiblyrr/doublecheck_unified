...
export function useErrorHandling(
  ...
  const setError = useCallback(
    (error: Error | ApiError | string | null) => {
      ...
      const errorMessage =
        "userMessage" in errorObj && errorObj.userMessage
  ...
  const handleError = useCallback(
    ...
    [config, setError, retry],
  ); // Remove errorState.retryCount dependency

  /**
   * Handle API errors specifically
   */
  const handleApiError = useCallback(
    async (error: Error | ApiError | string, apiContext: ApiErrorContext) => {
      try {
    ...
    [config, handleError, setError, retry],
  ); // Remove errorState.retryCount dependency
  ...
  const withErrorHandling = useCallback(
    async <T>(
      ...
      try {
        ...
        if (
          options?.context &&
          "url" in options.context &&
          "method" in options.context
        ) {
