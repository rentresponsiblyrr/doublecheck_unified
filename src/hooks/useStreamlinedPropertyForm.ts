...
export const useStreamlinedPropertyForm = (): UseStreamlinedPropertyFormReturn => {
  ...
  const validateForm = (): boolean => {
    ...
    if (hasVrbo) {
      ...
      const isValidVRBO = vrboPatterns.some(pattern => pattern.test(formData.vrbo_url));
      
      if (!isValidVRBO) {
        errors.vrbo_url = 'Please enter a valid VRBO URL (e.g., https://www.vrbo.com/123456)';
      }
    }
    ...
    if (hasAirbnb) {
      ...
      const isValidAirbnb = airbnbPatterns.some(pattern => pattern.test(formData.airbnb_url));
      
      if (!isValidAirbnb) {
        errors.airbnb_url = 'Please enter a valid Airbnb URL (e.g., https://www.airbnb.com/rooms/123456)';
      }
    }
