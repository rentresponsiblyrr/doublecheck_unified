...
export const useEnhancedAI = (): UseEnhancedAIReturn => {
  ...
  const [photoAnalysisError, setPhotoAnalysisError] = useState<Error | null>(
    null,
  );
  ...
  const [knowledgeSearchError, setKnowledgeSearchError] =
  ...
  const { deduplicatePhotoAnalysis, cancelRequestsMatching } =
  ...
  const analyzePhoto = useCallback(
    async (
      ...
      try {
        logger.info(
          "Starting enhanced photo analysis",
          {
        ...
        logger.info(
          "Enhanced photo analysis completed",
          {
  ...
  const generateChecklist = useCallback(
    async (propertyData: PropertyData, context: InspectionContext) => {
      setIsGeneratingChecklist(true);
      setChecklistError(null);

      try {
        logger.info(
          "Starting contextual checklist generation",
          {
        ...
        logger.info(
          "Contextual checklist generated",
          {
        ...
        logger.error(
          "Contextual checklist generation failed",
          error,
          "USE_ENHANCED_AI",
        );
        throw checklistErr;
  ...
  const validateInspection = useCallback(
    async (
      ...
      try {
        logger.info(
          "Starting contextual inspection validation",
          {
        ...
        const result =
          await enhancedAIService.validateInspectionCompletenessWithContext(
            checklistItems,
            photos,
            context,
          );
        ...
        const mockResults: KnowledgeSearchResult[] = [
          {
            ...
            content:
              "Bathrooms must meet specific accessibility requirements including grab bar placement, door width, and clear floor space...",
            category: "ada_compliance",
            source: "ADA Standards for Accessible Design",
            similarity: 0.89,
            metadata: {
              section: "606",
              applies_to: ["bathrooms", "accessibility"],
            },
            query_count: 45,
          ...
          },
          {
            ...
            content:
              "Smoke detectors must be installed in all sleeping areas and hallways. Carbon monoxide detectors required near fuel-burning appliances...",
            category: "fire_safety",
            source: "NFPA 101 Life Safety Code",
            similarity: 0.76,
            metadata: {
              section: "29",
              applies_to: ["smoke_detectors", "fire_safety"],
            },
            query_count: 67,
  ...
  const refreshInsights = useCallback(async () => {
    try {
      const insights =
        await aiLearningService.generateLearningInsights("weekly");
      setLearningInsights(insights);

      logger.info(
        "Learning insights refreshed",
        {
      ...
      logger.error(
        "Failed to refresh learning insights",
        error,
        "USE_ENHANCED_AI",
      );
      ...
      setLearningInsights([
        {
          ...
          description:
            "AI model showing 12% improvement in bathroom safety compliance detection over the past week",
          affected_categories: ["safety_compliance"],
          suggested_actions: [
            "Continue current training approach",
            "Expand bathroom safety dataset",
          ],
        ...
        },
        {
          ...
          description:
            "Inconsistent photo quality affecting model confidence in kitchen assessments",
          affected_categories: ["photo_quality"],
          suggested_actions: [
            "Review photo capture guidelines",
  ...
  const refreshPerformance = useCallback(
    async (modelVersion = "v1.1.0-cag") => {
      try {
        const performance =
          await aiLearningService.getModelPerformance(modelVersion);
        setModelPerformance(performance);

        logger.info(
          "Model performance refreshed",
          {
            modelVersion,
        ...
        logger.error(
          "Failed to refresh model performance",
          error,
          "USE_ENHANCED_AI",
        );
  ...
  const submitFeedback = useCallback(
    async (
      ...
      try {
        ...
        logger.info(
          "Auditor feedback submitted successfully",
          {
            inspectionId,
            category,
          },
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        logger.error(
          "Failed to load initial AI data after retries",
          error,
          "USE_ENHANCED_AI",
        );
