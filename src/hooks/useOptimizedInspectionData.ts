...
export const useOptimizedInspectionData = (inspectionId: string) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!inspectionId) {
        log.error(
          "No inspectionId provided to query",
          undefined,
          {
      ...
      // Check cache first
      const cachedData = cache.get<ChecklistItemType[]>(
        CACHE_KEYS.CHECKLIST_ITEMS(inspectionId),
      );
      if (cachedData) {
        // REMOVED: Console logging to prevent infinite loops
        return cachedData;
      ...
      try {
        ...
        const checklistResult = await supabase
          ...
          .select(
            "id, inspection_id, label, category, evidence_type, status, created_at",
          )
        ...
        if (error) {
          ...
          if (error && error.message?.includes("column")) {
            log.info(
              "Trying checklist_items with alternative field names",
              {
                component: "useOptimizedInspectionData",
                action: "queryFn",
                inspectionId,
        ...
        if (error) {
          ...
          if (
            ...
            log.warn(
              "Checklist table access failed, checking if inspection exists",
              {
                component: "useOptimizedInspectionData",
                action: "queryFn",
                inspectionId,
            ...
            if (!inspectionError && inspection) {
              log.info(
                "Inspection exists but no checklist items found - returning empty array",
                {
                  component: "useOptimizedInspectionData",
                  action: "queryFn",
                  inspectionId,
        ...
        if (!data || data.length === 0) {
          log.warn(
            "No checklist items found, checking if inspection exists",
            {
              component: "useOptimizedInspectionData",
              action: "queryFn",
              inspectionId,
        ...
        log.error(
          "Error in checklist items query",
          fetchError,
          {
            component: "useOptimizedInspectionData",
            action: "queryFn",
