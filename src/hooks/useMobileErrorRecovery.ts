...
export const useMobileErrorRecovery = (
  ...
  const getRecoveryStats = useCallback(
    () => ({
      ...
      recoveryRate:
        recoveryState.errorHistory.length > 0
          ? recoveryState.errorHistory.filter((r) => r.recovered).length /
            recoveryState.errorHistory.length
          : 0,
