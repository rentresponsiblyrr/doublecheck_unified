...
export const useSystemHealth = (
  ...
  const performHealthCheck = useCallback(
    async (attempt: number = 0): Promise<HealthCheckResult> => {
      try {
        ...
        if (enableDebugLogging) {
          logger.debug(
            "Health check completed successfully (mock)",
            {
        ...
        const error =
          err instanceof Error ? err : new Error("Unknown health check error");

        if (attempt < maxRetries) {
          const delay = Math.pow(backoffMultiplier, attempt) * 1000;
          logger.warn(
            `Health check failed, retrying in ${delay}ms`,
            {
              attempt: attempt + 1,
              maxRetries,
              error: error.message,
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      logger.info(
        "System health updated successfully",
        {
          status: result.status,
      ...
      const error =
        err instanceof Error ? err : new Error("Failed to fetch system health");
      setError(error);
