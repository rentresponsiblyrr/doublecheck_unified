...
interface EliteInspectionHookReturn {
  // 🎯 Core Operations
  joinInspection: (options?: InspectionJoinOptions) => Promise<InspectionJoinResult | null>;
...
export const useMobileInspectionOptimizer = (): EliteInspectionHookReturn => {
  ...
  const [operationState, setOperationState] = useState<InspectionOperationState>({
    isLoading: false,
    error: null,
    retryCount: 0,
    lastAttempt: null,
  ...
  const performanceMetrics = useRef({
    operations: [] as { duration: number; success: boolean; timestamp: number }[],
    totalOperations: 0,
    ...
    try {
      ...
      if (!user?.id) {
        ...
        toast({
          ...
          description: "User information unavailable. Please refresh and try again.",
          variant: "destructive"
        });
  ...
  const joinInspection = useCallback(async (options: InspectionJoinOptions = {}): Promise<InspectionJoinResult | null> => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error?.userMessage || 'Inspection join failed');
      }
      ...
      if (!inspectionData.inspectionId || 
          !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(inspectionData.inspectionId)) {
        const uuidError = new Error('Invalid inspection ID received from server');
      ...
      if (options.showSuccessToast !== false) {
        const statusText = inspectionData.status === 'created' ? 'Created' : 'Joined';
        const actionText = inspectionData.status === 'created' ? 'created' : 'joined';
      ...
      if (options.autoNavigate !== false) {
        ...
        if (!inspectionData.startUrl || !inspectionData.startUrl.startsWith('/inspection/')) {
      ...
      analytics.track('inspection_join_hook_success', {
        ...
        performanceGrade: processingTime < 500 ? 'excellent' : processingTime < 1000 ? 'good' : 'average'
      });
      ...
      toast({
        ...
        variant: "destructive",
        action: options.retryOnFailure !== false && operationState.retryCount < 3 ? {
  ...
  const retryLastOperation = useCallback(async (): Promise<InspectionJoinResult | null> => {
  ...
  const clearError = useCallback(() => {
    ...
    log.info('Error state cleared', { component: 'useMobileInspectionOptimizer' });
  }, []);
  ...
  const resetOperationState = useCallback(() => {
    ...
    log.info('Operation state reset', { component: 'useMobileInspectionOptimizer' });
  }, []);

  const getPerformanceMetrics = useCallback(() => {
    const { operations, totalOperations, successfulOperations } = performanceMetrics.current;
  ...
  const canJoinInspection = isAuthenticated && !!user?.id && !operationState.isLoading;
  const hasRecentError = !!(operationState.error && operationState.lastAttempt && 
                           (Date.now() - operationState.lastAttempt) < 300000); // 5 minutes
