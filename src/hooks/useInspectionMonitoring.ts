...
export interface MonitoringHookResult {
  ...
  trackError: (error: Error | InspectionErrorEvent, context?: Record<string, unknown>) => void;
  
  /** Track a successful inspection */
  trackSuccess: (data: { processingTime: number; context?: Record<string, unknown> }) => void;
...
export const useInspectionMonitoring = (config: InspectionMonitoringConfig = {}): MonitoringHookResult => {
  const {
  ...
  const trackError = useCallback((
    ...
    try {
      ...
      setLastError(trackingError instanceof Error ? trackingError : new Error('Tracking failed'));
    }
  }, [componentName, userContext]);

  const trackSuccess = useCallback((data: { 
    ...
    try {
      ...
      if (data.processingTime > performanceThreshold) {
        trackError(new Error(`Slow performance detected: ${data.processingTime}ms`), {
  ...
  const refreshMetrics = useCallback(() => {
    try {
      ...
      trackError(error instanceof Error ? error : new Error('Metrics refresh failed'));
    } finally {
      setIsLoading(false);
    }
  }, [trackError]);
  ...
  const getComponentMetrics = useCallback((): ErrorMetrics | null => {
    ...
    const errorsByCode = recentComponentErrors.reduce((acc, error) => {
    ...
    return {
      ...
      criticalErrors: recentComponentErrors.filter(e => e.businessImpact?.severity === 'critical').length,
...
export const useErrorTracking = (componentName: string) => {
  const { trackError, trackSuccess, lastError, clearError } = useInspectionMonitoring({
    componentName,
    autoTrack: false,
