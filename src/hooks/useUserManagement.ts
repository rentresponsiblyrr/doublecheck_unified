...
export const useUserManagement = () => {
  ...
  const runDiagnostic = useCallback(async (): Promise<DiagnosticResult> => {
    logger.info('🔍 Starting user management diagnostic...', {}, 'USER_MANAGEMENT');
    ...
    try {
      ...
      if (!usersError) {
        ...
        logger.info('✅ Users table exists and is accessible', {}, 'USER_MANAGEMENT');
      } else {
        result.errorDetails = usersError.message;
        logger.warn('⚠️ Users table not accessible:', usersError, 'USER_MANAGEMENT');
      }
      ...
      if (result.usersTableExists) {
        ...
        if (!currentUserError && currentUserData) {
          ...
          result.currentUserHasAccess = ['admin', 'super_admin'].includes(currentUserData.role);
          logger.info('✅ Current user found in users table', { 
            role: currentUserData.role,
            hasAccess: result.currentUserHasAccess 
          }, 'USER_MANAGEMENT');
        } else {
          logger.warn('⚠️ Current user not found in users table', currentUserError, 'USER_MANAGEMENT');
        }
      }

      // Step 4: Fallback to profiles table if needed
  ...
  const loadUsers = useCallback(async (): Promise<User[]> => {
    ...
    try {
      ...
      if (!diagResult.usersTableExists) {
        throw new Error('Users table does not exist. Please contact your administrator to run the database migration.');
      }

      if (!diagResult.currentUserHasAccess) {
        throw new Error(`Access denied. Current user role (${diagResult.currentUserRole || 'unknown'}) does not have permission to manage users.`);
      }
      ...
      const enhancedUsers = await Promise.all(
        usersData.map(async (user) => {
          ...
          try {
            ...
            logger.warn('⚠️ Failed to load activity data for user:', user.id, 'USER_MANAGEMENT');
          }

          // Determine if user is active (signed in within last 30 days)
          const isActive = user.last_login_at ? 
            (new Date(user.last_login_at).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000) : 
            false;
      ...
      logger.info('✅ Users loaded successfully', { count: enhancedUsers.length }, 'USER_MANAGEMENT');
  ...
  const createUser = useCallback(async (userData: Omit<User, 'id' | 'created_at' | 'is_active' | 'inspection_count' | 'audit_count'>): Promise<User> => {
    logger.info('➕ Creating new user...', { email: userData.email, role: userData.role }, 'USER_MANAGEMENT');
    ...
    try {
      ...
      logger.info('✅ User created successfully', { userId: newUser.id }, 'USER_MANAGEMENT');
      return {
        ...newUser,
        is_active: false,
        inspection_count: 0,
  ...
  const updateUser = useCallback(async (userId: string, updates: Partial<User>): Promise<User> => {
    logger.info('✏️ Updating user...', { userId, updates }, 'USER_MANAGEMENT');
    ...
    try {
      ...
      logger.info('✅ User updated successfully', { userId }, 'USER_MANAGEMENT');
  ...
  const deleteUser = useCallback(async (userId: string): Promise<void> => {
    ...
    try {
      ...
      logger.info('✅ User deleted successfully', { userId }, 'USER_MANAGEMENT');
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to initialize user management';
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: errorMessage,
  ...
  const filterUsers = useCallback((searchQuery: string, roleFilter: string, statusFilter: string) => {
