...
interface PropertyData extends PropertyWithInspections {
  ...
  _statusResult?: ReturnType<
    typeof propertyStatusService.calculatePropertyStatus
  >;
...
export const useMobileDataManager = (userId?: string) => {
   ...
   */
  const enhancePropertiesWithStatus = useCallback(
    (properties: PropertyData[]): PropertyData[] => {
      return properties.map((property) => {
        try {
          const statusResult =
            propertyStatusService.calculatePropertyStatus(property);
          return {
            ...property,
          ...
          logger.warn(
            "Failed to calculate property status",
            error,
            "MOBILE_DATA_MANAGER",
            {
   ...
   */
  const fetchPropertiesFromDatabase = useCallback(
    async (userId?: string) => {
      try {
        ...
        const properties =
          allProperties?.filter((property) => {
            ...
            const hasCompleted = inspections.some(
              (i) => i.status === "completed",
            );
            const hasRejected = inspections.some(
              (i) => i.status === "rejected",
            );
        ...
        const inspectionsByProperty = (inspections || []).reduce(
          (acc, inspection) => {
        ...
        const enrichedData = (properties || []).map((property) => {
          ...
          const inspectionCounts =
            calculateInspectionCounts(propertyInspections);
   ...
   */
  const calculateInspectionCounts = useCallback(
  ...
  const {
    ...
    queryFn: async (): Promise<PropertyData[]> => {
      ...
      try {
        logger.info(
          "Starting mobile properties fetch",
          { userId },
          "MOBILE_DATA_MANAGER",
        );
        ...
        if (cached && isCacheValid(cached)) {
          ...
          logger.debug(
            "Cache hit for mobile properties",
            {
              userId,
          ...
          setState((prev) => ({
            ...prev,
            cacheHitRate:
              (prev.cacheHitRate * (prev.totalRequests - 1) + 1) /
              prev.totalRequests,
            isCacheLoading: false,
          }));
        ...
        if (fetchResult.error) {
          throw new Error(
            `Database fetch failed: ${fetchResult.error.message}`,
          );
        }

        const enhancedProperties = enhancePropertiesWithStatus(
          fetchResult.data || [],
        );
        ...
        logger.info(
          "Mobile properties fetch completed",
          {
            userId,
        ...
        setState((prev) => ({
          ...
          isCacheLoading: false,
          cacheHitRate:
            (prev.cacheHitRate * (prev.totalRequests - 1)) / prev.totalRequests,
        }));
        ...
        logger.error(
          "Mobile properties fetch failed",
          error,
          "MOBILE_DATA_MANAGER",
          {
            userId,
        ...
        const fallbackCache = mobileCache.get<PropertyData[]>(
          MOBILE_CACHE_KEYS.PROPERTIES(userId),
        );
  ...
  const getPropertyStatus = useCallback(
    (
      ...
      try {
        ...
        return propertyStatusService.calculatePropertyStatus(
          propertyForCalculation,
        );
      } catch (error) {
        logger.error(
  ...
  const fetchChecklistItems = useCallback(
    async (inspectionId: string): Promise<ChecklistItemType[]> => {
      if (!inspectionId) {
        logger.warn(
          "fetchChecklistItems called without inspectionId",
          {},
          "MOBILE_DATA_MANAGER",
        );
        return [];
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(
            "id, inspection_id, title, category, evidence_type, status, created_at",
          )
        ...
        const transformedData: ChecklistItemType[] = (data || []).map(
          (item) => ({
            id: item.id,
        ...
        logger.debug(
          "Checklist items fetched and cached",
          {
            inspectionId,
