...
export const useFunctionalChecklistManagement = () => {
  ...
  const loadSafetyItems = async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to load safety items";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        ...
        const updatedItem = await productionDb.updateSafetyItem(
          editingItem.id,
          formData,
        );
        setSafetyItems(
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to save safety item";
      setError(errorMessage);
  ...
  const handleDeleteItem = async (item: ProductionSafetyItem) => {
    if (
      !confirm(
        `Are you sure you want to delete the safety item "${item.label}"? This action cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to delete safety item";
      setError(errorMessage);
