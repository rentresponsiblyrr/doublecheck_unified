...
export const useComponentHealth = () => {
  ...
  const checkComponentHealth = useCallback(
    async (componentName: string, componentPath: string) => {
      ...
      try {
        ...
        return {
          ...
          lastChecked: new Date(),
          error:
            error instanceof Error ? error.message : "Component check failed",
  ...
  const runHealthCheck = useCallback(async () => {
    ...
    const criticalUnhealthy = healthChecks.filter(
      (check) =>
        !check.isHealthy &&
        ["User Management", "Audit Center", "Checklist Management"].includes(
          check.name,
        ),
    ).length;
  ...
  const reportComponentError = useCallback(
    (componentName: string, error: Error) => {
      setHealthData((prev) => ({
        ...
        overallHealth: prev.components.some(
          (c) =>
            [
              "User Management",
              "Audit Center",
              "Checklist Management",
            ].includes(c.name) && !c.isHealthy,
        )
          ? "critical"
          : "degraded",
        lastUpdated: new Date(),
