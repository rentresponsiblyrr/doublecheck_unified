...
export const useChannelManager = () => {
  ...
  const subscribeChannel = useCallback(
    async (channelName: string, onStatusChange?: (status: string) => void) => {
      if (!isMountedRef.current) {
        debugLogger.warn(
          "ChannelManager",
          "Attempted to subscribe to unmounted component",
          { channelName },
        );
        return;
      }

      debugLogger.debug("ChannelManager", "Subscribing to channel", {
        channelName,
      });
      ...
      if (currentStatus === "subscribed") {
        debugLogger.info("ChannelManager", "Channel already subscribed", {
          channelName,
        });
      ...
      if (currentStatus === "pending") {
        debugLogger.info("ChannelManager", "Subscription already pending", {
          channelName,
        });
      ...
      const channel =
        ...
        channelRegistry.get(channelName);
      if (!channel) {
        debugLogger.error(
          "ChannelManager",
      ...
      const subscriptionPromise = new Promise<void>((resolve, reject) => {
        try {
          debugLogger.info("ChannelManager", "Starting subscription", {
            channelName,
          });
          subscriptionRegistry.set(channelName, "pending");

          channel.subscribe((status: string) => {
            debugLogger.info("ChannelManager", "Channel subscription status", {
              channelName,
              status,
            });
          ...
          debugLogger.error("ChannelManager", "Error subscribing to channel", {
            channelName,
            error,
          });
          subscriptionRegistry.set(channelName, "error");
      ...
      try {
        ...
        debugLogger.info("ChannelManager", "Channel subscription successful", {
          channelName,
        });
      } catch (error) {
        debugLogger.error("ChannelManager", "Subscription failed", {
          channelName,
          error,
        });
      } finally {
  ...
  const cleanupChannel = useCallback((channelName: string) => {
    ...
    if (channel) {
      try {
        ...
        debugLogger.debug("ChannelManager", "Channel removed from Supabase", {
          channelName,
        });
      } catch (error) {
        debugLogger.warn("ChannelManager", "Error removing channel", {
          channelName,
          error,
        });
      }
    ...
    debugLogger.debug("ChannelManager", "Channel cleanup complete", {
      channelName,
    });
  }, []);
  ...
  const cleanupAllChannels = useCallback(() => {
    ...
    debugLogger.info("ChannelManager", "All channels cleaned up", {
