...
export const useVirtualizedPropertyList = (
  ...
  const searchStats = useMemo(
    () => ({
      ...
      searchEfficiency:
        filteredProperties.length / Math.max(properties.length, 1),
   ...
   */
  const scrollToProperty = useCallback(
    (propertyId: string) => {
  ...
  const getProperty = useCallback(
    (index: number): PropertyData | null => {
    ...
    () => ({
      ...
      memoryUsage: {
        ...
        renderEfficiency:
          ...
          Math.max(virtualMetrics.itemCount, 1),
      },
    }),
  ...
  const createItemRenderer = useCallback(
    (
      renderItem: (property: PropertyData, index: number) => React.ReactNode,
    ) => {
      return ({
        index,
        style,
