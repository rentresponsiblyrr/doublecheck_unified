...
export const usePropertyScraper = (
  ...
  const [propertyData, setPropertyData] = useState<VRBOPropertyData | null>(
    null,
  );
  const [photos, setPhotos] = useState<PhotoData[]>([]);
  const [deduplicatedPhotos, setDeduplicatedPhotos] =
    useState<PhotoDeduplicationResult | null>(null);
        ...
        if (photoResult.success && photoResult.data) {
          ...
          if (enablePhotoDeduplication) {
            ...
            const deduplicator = createPhotoDeduplicator(
              photoSimilarityThreshold,
            );
            const dedupResult = await deduplicator.deduplicatePhotos(
              photoResult.data,
            );
  ...
  // Computed values
  const isLoading =
    propertyScrapingMutation.isPending || photoScrapingMutation.isPending;
  const isError =
    propertyScrapingMutation.isError || photoScrapingMutation.isError;
  const error = (propertyScrapingMutation.error ||
    photoScrapingMutation.error) as ScrapingError | null;
