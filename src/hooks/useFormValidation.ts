...
export const useFormValidation = (options: UseFormValidationOptions): FormValidationHookResult => {
  const {
    formId,
    rules,
    initialData = {},
  ...
  useEffect(() => {
    if (isInitialized.current && config.validateOnMount && Object.keys(formData).length > 0) {
      validateForm();
    }
  ...
  const setValue = useCallback((field: string, value: any) => {
    setFormData(prev => {
      ...
      if (config.validateOnChange !== false) {
        setTimeout(() => {
          formValidationService.validateField(formId, field, value, newData, { trigger: 'change' })
            .then(fieldState => {
  ...
  }, [formId, config.validateOnChange, onFieldChange, onValidationChange, reportFormError]);
  ...
  const validateField = useCallback(async (field: string): Promise<FieldValidationState> => {
    try {
  ...
  const getFieldState = useCallback((field: string): FieldValidationState | null => {
  ...
  const getFieldProps = useCallback((field: string) => ({
    ...
    'aria-describedby': hasFieldError(field) ? `${formId}-${field}-error` : undefined,
    'aria-required': rules.some(rule => rule.field === field && !rule.schema.isOptional()),
    id: `${formId}-${field}`,
  }), [formId, formData, setValue, validateField, hasFieldError, rules, config.validateOnBlur]);
