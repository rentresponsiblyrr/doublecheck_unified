...
export function useEnterprisePerformanceMonitoring() {
  ...
  const trackFileUpload = useCallback((
    ...
    performanceMonitor.trackFileUpload(fileName, fileSize, uploadTime, success);
  }, []);
  ...
  const trackAIRequest = useCallback((
    ...
    performanceMonitor.trackAIRequest(operation, duration, success, tokens, cost);
  }, []);
  ...
  const trackDatabaseOperation = useCallback((
    ...
    performanceMonitor.trackDatabaseOperation(operation, table, duration, success, rowsAffected);
  }, []);
