...
import { ArrowRight, PlayCircle, Wifi, WifiOff } from "lucide-react";
 ...
 */
export const PropertyInspectionActions: React.FC<
  PropertyInspectionActionsProps
> = ({
  propertyId,
  activeInspection,
  isOnline,
  ...
  const handleInspectionAction = useCallback(
    async (isResume: boolean, e: React.MouseEvent) => {
      e.stopPropagation();
      ...
      try {
        ...
        if (isResume && activeInspection) {
          logger.info(
            "Resuming inspection via PropertyInspectionActions",
            {
          ...
          logger.info(
            "Starting new inspection via PropertyInspectionActions",
            {
              propertyId,
            },
            "PROPERTY_ACTIONS",
  ...
  return (
    <div
      ...
      id={`inspection-actions-${propertyId}`}
    >
      {activeInspection ? (
        <>
          ...
          <div id={`continue-description-${propertyId}`} className="sr-only">
            Resume inspection with {activeInspection.completed_items} of{" "}
            {activeInspection.total_items} items completed
          </div>
          ...
          <div
            id={`new-inspection-description-${propertyId}`}
            className="sr-only"
          >
