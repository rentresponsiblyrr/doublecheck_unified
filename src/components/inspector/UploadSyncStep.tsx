...
export const UploadSyncStep: React.FC<UploadSyncStepProps> = ({
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      for (const checklistItem of checklistItems) {
        for (let i = 0; i < checklistItem.photos.length; i++) {
          ...
          try {
            ...
            setCurrentStep(
              `Uploading photos... (${photoIndex}/${getTotalPhotoCount()})`,
            );
          } catch (error) {
            updateItemProgress(
              itemId,
      ...
      logger.logInfo("Inspection upload completed", {
        inspectionId,
        propertyId: property.id,
      });
      onComplete(inspectionId);
    } catch (error) {
      logger.logError("Upload process failed", {
        propertyId: property.id,
        error,
      });
      toast({
    ...
    <Card id="upload-sync-step" className={className}>
      ...
      <CardContent className="space-y-6">
        ...
        {!isOnline && (
          <Alert>
            <AlertDescription>
              You're currently offline. Data will be queued for upload when
        ...
        <ScrollArea className="h-[300px]">
          <div className="grid gap-2">
            {uploadItems.map((item) => (
              <UploadItemCard key={item.id} item={item} onRetry={retryItem} />
            ))}
          </div>
        </ScrollArea>
