...
const getStatusIcon = (status: string) => {
  switch (status) {
    ...
    case 'active':
      return <ArrowRight className="w-4 h-4 text-blue-500" aria-label="Active" />;
    case 'skipped':
      return <AlertTriangle className="w-4 h-4 text-yellow-500" aria-label="Skipped" />;
    default:
...
const getStepNumber = (index: number, status: string) => {
  if (status === 'completed') {
    return <CheckCircle className="w-5 h-5 text-green-500" aria-label={`Step ${index + 1} completed`} />;
  }
...
export const InspectionStepsList: React.FC<InspectionStepsListProps> = ({
   ...
   */
  const handleStepClick = useCallback((step: InspectionStep) => {
   ...
   */
  const handleKeyDown = useCallback((event: React.KeyboardEvent, step: InspectionStep) => {
