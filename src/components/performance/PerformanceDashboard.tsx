...
import { useCoreWebVitalsMonitoring, useCoreWebVitals, usePerformanceBudgets } from '@/hooks/useCoreWebVitalsMonitoring';
import { Activity, AlertTriangle, CheckCircle, Clock, Download, Gauge, Monitor, Smartphone, TrendingUp, TrendingDown, Wifi } from 'lucide-react';
...
export const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'metrics' | 'budgets' | 'trends' | 'suggestions'>('metrics');

  // Get performance rating color
  const getRatingColor = (rating: 'good' | 'needs-improvement' | 'poor' | null): string => {
    switch (rating) {
  ...
  if (variant === 'compact') {
    return (
      <div id={id} className={`bg-white border border-gray-200 rounded-lg p-3 shadow-sm ${className}`}>
        <div id="compact-performance-header" className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Gauge className="h-4 w-4 text-blue-600" />
            <span className="text-sm font-semibold text-gray-900">Performance</span>
          </div>
          <div className={`px-2 py-1 rounded-full text-xs font-medium ${getScoreColor(state.metrics.performanceScore)}`}>
        ...
        <div id="compact-core-metrics" className="grid grid-cols-3 gap-2">
          <div className="text-center">
            ...
            <div className={`text-sm font-medium ${state.metrics.lcp ? getRatingColor(state.metrics.lcp.rating) : 'text-gray-400'}`}>
              {state.metrics.lcp ? formatMetricValue(state.metrics.lcp.value, 'ms') : 'N/A'}
            </div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-500">FID</div>
            <div className={`text-sm font-medium ${state.metrics.fid ? getRatingColor(state.metrics.fid.rating) : 'text-gray-400'}`}>
              {state.metrics.fid ? formatMetricValue(state.metrics.fid.value, 'ms') : 'N/A'}
            </div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-500">CLS</div>
            <div className={`text-sm font-medium ${state.metrics.cls ? getRatingColor(state.metrics.cls.rating) : 'text-gray-400'}`}>
              {state.metrics.cls ? formatMetricValue(state.metrics.cls.value, '') : 'N/A'}
            </div>
          </div>
        </div>

        {state.alerts.length > 0 && (
          <div id="compact-alerts" className="mt-3 pt-3 border-t border-gray-200">
  ...
  if (variant === 'detailed') {
    return (
      <div id={id} className={`bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
        <div id="detailed-performance-header" className="flex items-center justify-between p-4 border-b border-gray-200">
          <div className="flex items-center gap-2">
            <Gauge className="h-5 w-5 text-blue-600" />
            <h3 className="text-lg font-semibold text-gray-900">Core Web Vitals</h3>
          </div>
          <div className="flex items-center gap-3">
            <div className={`px-3 py-1 rounded-full text-sm font-medium ${getScoreColor(state.metrics.performanceScore)}`}>
        ...
        <div id="detailed-performance-content" className="p-4">
          ...
          <div id="core-web-vitals-grid" className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {/* LCP */}
            <div className={`border rounded-lg p-4 ${state.metrics.lcp ? getRatingColor(state.metrics.lcp.rating) : 'border-gray-200'}`}>
            ...
            {/* FID */}
            <div className={`border rounded-lg p-4 ${state.metrics.fid ? getRatingColor(state.metrics.fid.rating) : 'border-gray-200'}`}>
              <div className="flex items-center justify-between mb-2">
            ...
            {/* CLS */}
            <div className={`border rounded-lg p-4 ${state.metrics.cls ? getRatingColor(state.metrics.cls.rating) : 'border-gray-200'}`}>
              <div className="flex items-center justify-between mb-2">
          ...
          {/* Supporting Metrics */}
          <div id="supporting-metrics" className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
              <div className="text-sm text-gray-600 mb-1">FCP</div>
              <div className="text-lg font-semibold">
                {state.metrics.fcp ? formatMetricValue(state.metrics.fcp.value, 'ms') : 'N/A'}
              </div>
            </div>
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
              <div className="text-sm text-gray-600 mb-1">TTFB</div>
              <div className="text-lg font-semibold">
                {state.metrics.ttfb ? formatMetricValue(state.metrics.ttfb.value, 'ms') : 'N/A'}
              </div>
            </div>
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
          ...
          {state.alerts.length > 0 && (
            <div id="performance-alerts" className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-gray-900">Performance Alerts</h4>
                <button
          ...
          {/* Actions */}
          <div id="performance-actions" className="flex items-center justify-between pt-4 border-t border-gray-200">
            <div className="flex items-center gap-2">
              <button
  ...
  return (
    <div id={id} className={`bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
      <div id="full-performance-header" className="flex items-center justify-between p-6 border-b border-gray-200">
        <div className="flex items-center gap-3">
          <Gauge className="h-6 w-6 text-blue-600" />
          <div>
            <h2 className="text-xl font-bold text-gray-900">Performance Dashboard</h2>
            <p className="text-sm text-gray-600">Core Web Vitals monitoring and optimization</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className={`px-4 py-2 rounded-full text-lg font-bold ${getScoreColor(state.metrics.performanceScore)}`}>
            {state.metrics.performanceScore}
          </div>
          <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${state.isMonitoring ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
            {state.isMonitoring ? <CheckCircle className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
      ...
      <div id="performance-tabs" className="border-b border-gray-200">
        <nav className="flex space-x-8 px-6" aria-label="Tabs">
          {[
            ...
            { id: 'budgets', label: 'Performance Budgets', icon: AlertTriangle },
      ...
      <div id="performance-tab-content" className="p-6">
        {activeTab === 'metrics' && (
          <div id="metrics-tab" className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* LCP Card */}
              <div className={`border-2 rounded-xl p-6 ${state.metrics.lcp ? getRatingColor(state.metrics.lcp.rating) : 'border-gray-200'}`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">Largest Contentful Paint</h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${state.metrics.lcp ? getRatingColor(state.metrics.lcp.rating) : ''}`}>
                ...
                {state.metrics.lcp?.trend && (
                  <div className="flex items-center gap-1 mt-2 text-sm">
                    ...
                    <span className="capitalize">{state.metrics.lcp.trend}</span>
                  </div>
                )}
              </div>

              {/* FID Card */}
              <div className={`border-2 rounded-xl p-6 ${state.metrics.fid ? getRatingColor(state.metrics.fid.rating) : 'border-gray-200'}`}>
                <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${state.metrics.fid ? getRatingColor(state.metrics.fid.rating) : ''}`}>
                ...
                {state.metrics.fid?.trend && (
                  <div className="flex items-center gap-1 mt-2 text-sm">
                    ...
                    <span className="capitalize">{state.metrics.fid.trend}</span>
                  </div>
                )}
              </div>

              {/* CLS Card */}
              <div className={`border-2 rounded-xl p-6 ${state.metrics.cls ? getRatingColor(state.metrics.cls.rating) : 'border-gray-200'}`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">Cumulative Layout Shift</h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${state.metrics.cls ? getRatingColor(state.metrics.cls.rating) : ''}`}>
                ...
                {state.metrics.cls?.trend && (
                  <div className="flex items-center gap-1 mt-2 text-sm">
                    ...
                    <span className="capitalize">{state.metrics.cls.trend}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold flex items-center gap-2">
                  ...
                  <span className="capitalize">{state.deviceMetrics.deviceType}</span>
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {state.deviceMetrics.isLowEndDevice ? 'Low-end device' : 'Standard device'}
                </div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold flex items-center gap-2">
                  ...
                  <span className="uppercase">{state.deviceMetrics.effectiveConnectionType}</span>
                </div>
                <div className="text-xs text-gray-500 mt-1">
        ...
        {activeTab === 'budgets' && (
          <div id="budgets-tab" className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">Performance Budgets</h3>
              <span className="text-sm text-gray-500">{state.budgets.length} budgets configured</span>
            </div>
                ...
                <div key={index} className={`border rounded-lg p-4 ${
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      budget.status === 'exceeded' ? 'bg-red-200 text-red-800' :
                      budget.status === 'warning' ? 'bg-yellow-200 text-yellow-800' :
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <span>Current: <strong>{Math.round(budget.current)}ms</strong></span>
                    <span>Target: <strong>{Math.round(budget.target)}ms</strong></span>
                    <span>Warning: <strong>{Math.round(budget.warning)}ms</strong></span>
                    <span>Critical: <strong>{Math.round(budget.critical)}ms</strong></span>
                  </div>
                  <div className="mt-2">
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Average:</span>
                      <span className="font-medium">{Math.round(trend.average)}ms</span>
                    </div>
                    <div className="flex justify-between">
                      <span>P75:</span>
                      <span className="font-medium">{Math.round(trend.p75)}ms</span>
                    </div>
                    <div className="flex justify-between">
                      <span>P95:</span>
                      <span className="font-medium">{Math.round(trend.p95)}ms</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Samples:</span>
            ...
            <div className="space-y-4">
              {actions.getOptimizationSuggestions().map((suggestion, index) => (
                <div key={index} className={`border rounded-lg p-4 ${
                  ...
                  suggestion.type === 'important' ? 'border-yellow-200 bg-yellow-50' :
                  suggestion.type === 'moderate' ? 'border-blue-200 bg-blue-50' :
                  'border-gray-200 bg-gray-50'
                }`}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h4 className="font-medium">{suggestion.suggestion}</h4>
                      <p className="text-sm text-gray-600 mt-1">{suggestion.issue}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        suggestion.type === 'critical' ? 'bg-red-200 text-red-800' :
                        suggestion.type === 'important' ? 'bg-yellow-200 text-yellow-800' :
                        suggestion.type === 'moderate' ? 'bg-blue-200 text-blue-800' :
      ...
      {/* Footer Actions */}
      <div id="performance-footer" className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50">
        <div className="flex items-center gap-4">
          <button
            onClick={actions.refreshMetrics}
