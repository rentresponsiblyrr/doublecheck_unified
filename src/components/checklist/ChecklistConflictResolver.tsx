...
export const ChecklistConflictResolver: React.FC<
  ChecklistConflictResolverProps
> = ({ conflictData, onResolve, isResolving }) => {
  if (!conflictData) {
    return null;
  }
  ...
  return (
    <Card
      id="checklist-conflict-resolver"
      ...
      <CardContent className="space-y-4">
        <Alert className="border-orange-200">
          ...
          <AlertDescription className="text-orange-800">
            Another inspector has modified this item while you were working on
            it. Please choose how to resolve this conflict:
          </AlertDescription>
        </Alert>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="p-3 bg-white border border-orange-200 rounded text-sm">
              <div>
                ...
                {conflictData.local?.status || "No changes"}
              </div>
              <div>
                <strong>Notes:</strong>{" "}
                {conflictData.local?.notes || "No notes"}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <h4 className="font-semibold text-sm text-orange-800">
              Other Inspector's Version
            </h4>
            <div className="p-3 bg-white border border-orange-200 rounded text-sm">
              <div>
                <strong>Status:</strong>{" "}
                {conflictData.remote?.status || "No changes"}
              </div>
              <div>
                <strong>Notes:</strong>{" "}
                {conflictData.remote?.notes || "No notes"}
              </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            >
              ...
              <span className="text-xs text-gray-600 ml-2">
                (Overwrite other inspector's changes)
              </span>
            </Button>
            ...
            >
              ...
              Merge Both Versions
              <span className="text-xs text-gray-600 ml-2">
                (Combine notes and use latest status)
              </span>
            </Button>
          </div>
        </div>
