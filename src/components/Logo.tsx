...
export const Logo = ({
  ...
  const getLogoUrl = () => {
    ...
    const fallbackLogo =
      "/lovable-uploads/ea9dd662-995b-4cd0-95d4-9f31b2aa8d3b.png";
  ...
  const isHorizontal =
    variant === "horizontal" || variant === "horizontal-strapline";
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {showText && (
        <div
          className={`flex ${isHorizontal ? "flex-row items-center gap-2" : "flex-col"}`}
        >
          <div
            className={`font-bold ${isDark ? "text-white" : "text-blue-600"} ${textSizeClasses[size]}`}
          >
            STR Certified
          </div>
          {hasStrapline && (
            <div
              className={`text-xs ${isDark ? "text-gray-300" : "text-gray-500"} leading-none`}
            >
