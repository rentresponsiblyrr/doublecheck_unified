...
export const PhotoWorkflowManager: React.FC<PhotoWorkflowManagerProps> = ({
  ...
  const handlePhotoCapture = useCallback(
    async (photoBlob: Blob) => {
      ...
      try {
        ...
        const file = new File(
          [photoBlob],
          `inspection_${checklistItem.id}_${Date.now()}.jpg`,
          {
            type: "image/jpeg",
        ...
        announceToScreenReader(
          "Photo captured successfully. Processing image...",
        );
      } catch (error) {
  ...
  const handleFileProcessed = useCallback(
    async (processedFile: File) => {
      ...
      try {
        ...
        announceToScreenReader(
          "Image processed successfully. Running quality analysis...",
        );
      } catch (error) {
        ...
        onError(
          error instanceof Error
            ? error
            : new Error("Failed to handle processed file"),
        );
      } finally {
        if (mountedRef.current) {
          setIsLoading(false);
  ...
  const handleAnalysisComplete = useCallback(
    (analysisResult: PhotoAnalysis) => {
      ...
      try {
        ...
        announceToScreenReader(
          `Analysis complete. Quality score: ${analysisResult.qualityScore}%. Starting upload...`,
        );
      } catch (error) {
        ...
        onError(
          error instanceof Error
            ? error
            : new Error("Failed to handle analysis results"),
        );
      }
    },
    [announceToScreenReader, onError],
  ...
  const handleUploadComplete = useCallback(
    (itemId: string, uploadResult: any) => {
      ...
      try {
        ...
        announceToScreenReader(
          "Photo upload complete. Inspection item updated.",
        );
        ...
        onError(
          error instanceof Error
            ? error
            : new Error("Failed to handle upload completion"),
        );
      }
    },
    [
  ...
  const getStepStatus = useCallback(
    (step: WorkflowState["currentStep"]) => {
      const currentStepIndex = [
        ...
        "upload",
        "complete",
      ].indexOf(workflowState.currentStep);
      const stepIndex = [
        "capture",
        "processing",
        "analysis",
  ...
  const getStepBadgeVariant = useCallback(
    (status: string): "default" | "secondary" | "outline" => {
      switch (status) {
        case "complete":
          return "default";
        case "active":
