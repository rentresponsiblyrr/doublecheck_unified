...
export const UploadManager: React.FC<UploadManagerProps> = ({
  ...
  const [uploadStatus, setUploadStatus] = useState<'preparing' | 'uploading' | 'processing' | 'complete'>('preparing');
  ...
  return (
    <div id="upload-manager" className="space-y-4">
      <Alert className={uploadStatus === 'complete' ? 'border-green-200 bg-green-50' : ''}>
        {getStatusIcon()}
        <AlertDescription>
