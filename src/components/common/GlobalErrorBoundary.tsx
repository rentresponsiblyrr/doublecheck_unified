...
import { AlertTriangle, RefreshCw, Home, FileText, ExternalLink, MessageCircle } from 'lucide-react';
...
> {
  ...
  private calculateSeverity(error: Error, errorInfo: ErrorInfo): ErrorInfo['severity'] {
    // Critical errors that completely break the app
    if (this.props.criticalComponent || 
  ...
  private reportError(error: ErrorInfo): void {
    ...
    const now = Date.now();
    if (now - this.state.lastErrorTime < this.ERROR_THROTTLE_MS && 
        ...
        <div className="max-w-2xl w-full space-y-6">
          ...
          <Card id="error-status-card" className="border-red-200">
            <CardHeader id="error-header" className="text-center">
              <div id="error-icon-container" className="flex justify-center mb-4">
                <div id="error-icon-background" className="bg-red-100 p-3 rounded-full">
              ...
              <CardTitle id="error-title" className="text-2xl text-gray-900">
                {error?.severity === 'critical' ? 'Critical System Error' :
                 error?.severity === 'high' ? 'Application Error' :
                 'Something went wrong'}
              </CardTitle>
              
              <div id="error-badges-container" className="flex justify-center gap-2 mt-2">
                <Badge 
            ...
            <CardContent id="error-content" className="text-center space-y-4">
              ...
              {process.env.NODE_ENV === 'development' && error && (
                <details id="error-details" className="text-left bg-gray-100 p-4 rounded">
          ...
          <Card id="recovery-actions-card">
            ...
            <CardContent id="recovery-content">
              <div id="recovery-buttons-container" className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {/* Retry Button */}
                {canRetry && (
                  <Button
          ...
          {(isRecovering || retryCount > 0) && (
            <Card id="recovery-status-card" className="border-blue-200 bg-blue-50">
              <CardContent className="pt-4">
                <div className="flex items-center">
                  <RefreshCw className={`w-5 h-5 mr-3 text-blue-600 ${isRecovering ? 'animate-spin' : ''}`} />
                  <div>
                    <div className="font-medium text-blue-800">
                      {isRecovering ? 'Attempting to recover...' : `Recovery attempted ${retryCount} time${retryCount > 1 ? 's' : ''}`}
                    </div>
                    <div className="text-sm text-blue-600">
                      {isRecovering ? 'Please wait while we try to resolve the issue.' : 
                       canRetry ? 'You can try again or use one of the options above.' :
...
export function withErrorBoundary<P extends object>(
  ...
  return function WrappedComponent(props: P) {
    return (
      <GlobalErrorBoundary {...errorBoundaryProps} context={Component.displayName || Component.name}>
        <Component {...props} />
      </GlobalErrorBoundary>
    );
  };
