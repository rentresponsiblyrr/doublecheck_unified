...
import { cn, RESPONSIVE_LAYOUTS, RESPONSIVE_SPACING, RESPONSIVE_A11Y } from '@/lib/utils/responsive';
...
export const ResponsiveContainer: React.FC<ResponsiveContainerProps> = ({
  ...
  const getLayoutClasses = () => {
    ...
    return cn(
      ...
      // Tablet override
      isTablet && `md:${layouts[tabletLayout].split(' ').map(c => c.startsWith('flex') || c.startsWith('grid') ? c : '').filter(Boolean).join(' ')}`,
      // Desktop override  
      isDesktop && `lg:${layouts[desktopLayout].split(' ').map(c => c.startsWith('flex') || c.startsWith('grid') ? c : '').filter(Boolean).join(' ')}`
    );
  };
  ...
  const getSpacingClasses = () => {
    const paddingMap = {
      none: '',
      sm: RESPONSIVE_SPACING.mobile.sm + ' ' + RESPONSIVE_SPACING.tablet.sm + ' ' + RESPONSIVE_SPACING.desktop.sm,
      md: RESPONSIVE_SPACING.mobile.md + ' ' + RESPONSIVE_SPACING.tablet.md + ' ' + RESPONSIVE_SPACING.desktop.md,
      lg: RESPONSIVE_SPACING.mobile.lg + ' ' + RESPONSIVE_SPACING.tablet.lg + ' ' + RESPONSIVE_SPACING.desktop.lg,
      xl: RESPONSIVE_SPACING.mobile.xl + ' ' + RESPONSIVE_SPACING.tablet.xl + ' ' + RESPONSIVE_SPACING.desktop.xl,
    };
 ...
 */
export interface ResponsiveGridProps extends Omit<ResponsiveContainerProps, 'mobileLayout' | 'tabletLayout' | 'desktopLayout'> {
  columns?: {
    mobile?: number;
    tablet?: number; 
    desktop?: number;
 ...
 */
export interface ResponsiveFlexProps extends Omit<ResponsiveContainerProps, 'mobileLayout' | 'tabletLayout' | 'desktopLayout'> {
  direction?: {
    mobile?: 'row' | 'col';
    tablet?: 'row' | 'col';
    desktop?: 'row' | 'col';
