    ...
    try {
      ...
      const pwaScore = Math.min(
        ...
        Math.max(
          0,
          100 -
            (metrics.coreWebVitals.lcp - 2500) / 100 -
            ...
            <div className="flex items-center justify-between">
              ...
              {showPerformanceStatus && pwaStatus && (
                <div id="mobile-pwa-status" className="flex items-center gap-2">
                  <Badge
                    className={`text-xs ${getPerformanceColor(pwaStatus.score)}`}
                  >
                  ...
                  <div className="flex items-center gap-1">
                    <Wifi
                      className={`h-4 w-4 ${getNetworkColor(pwaStatus.networkMode)}`}
                    />
                    <Battery
                      className={`h-4 w-4 ${getBatteryColor(pwaStatus.batteryMode)}`}
                    />
                    {!pwaStatus.isOnline && (
                      <div
