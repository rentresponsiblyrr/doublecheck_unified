...
import { Activity, AlertTriangle, TrendingUp, TrendingDown, Gauge, Globe, Smartphone } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {regressionAlerts.length > 0 && (
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>{regressionAlerts.length} performance regression(s)</strong> detected.
            {regressionAlerts.filter(a => a.severity === 'emergency').length > 0 && 
              ' Emergency-level issues require immediate attention.'}
          </AlertDescription>
        </Alert>
      )}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        {alert.degradation.toFixed(1)}% degradation • {alert.confidence.toFixed(0)}% confidence
                      </div>
                    </div>
                  </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold">{deviceMetrics.mobile.toFixed(1)}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Desktop</span>
                <span className="text-sm font-bold">{deviceMetrics.desktop.toFixed(1)}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Tablet</span>
                <span className="text-sm font-bold">{deviceMetrics.tablet.toFixed(1)}%</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {Object.entries(deviceMetrics.networkTypes).map(([type, percentage]) => (
