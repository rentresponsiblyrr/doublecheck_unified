...
export const PhotoQualityAssistant: React.FC<PhotoQualityAssistantProps> = ({
  ...
  const { guidance, isAnalyzing, analyzeCurrentFrame, clearGuidance } =
    usePhotoGuidance(checklistItem);

  const { comparison, compareWithReference, isComparing } =
    usePhotoComparison();
  ...
  const getQualityScore = () => {
    ...
    const maxScore = 100;
    const issueCount = guidance.messages.filter(
      (m) => m.type === "error",
    ).length;
