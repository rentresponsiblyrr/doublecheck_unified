...
interface WorkflowGestureControllerProps {
  ...
  onGesture?: (
    gesture: GestureType,
    data?: { direction?: string; velocity?: number; distance?: number },
  ) => void;
...
export const WorkflowGestureController: React.FC<
  WorkflowGestureControllerProps
> = ({ children, onGesture, enableGestures = true }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const touchDataRef = useRef<TouchData | null>(null);
  const longPressTimerRef = useRef<NodeJS.Timeout | null>(null);
  const { state, transitionTo, canTransitionTo } =
    useInspectorWorkflowStateMachine();
  ...
  const handleTouchEnd = useCallback(
    (e: TouchEvent) => {
      ...
      if (
        !touchDataRef.current.isLongPress &&
