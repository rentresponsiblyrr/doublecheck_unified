...
export const PWAStatusIndicator: React.FC<PWAStatusIndicatorProps> = ({
  ...
  const getNetworkIcon = () => {
    ...
    switch (status.networkQuality) {
      ...
      case "fair":
        return <Wifi className="h-4 w-4 opacity-75" />;
      case "poor":
  ...
  const handleInstallClick = async () => {
    try {
      ...
      if (!result.success) {
        ...
        alert(
          'To install this app, use your browser\'s "Add to Home Screen" option in the menu.',
        );
      }
    } catch (error) {
      ...
      >
        ...
        {status.retryQueueSize > 0 && (
          <div
            ...
            className="flex items-center gap-2 mb-2 text-orange-600"
          >
