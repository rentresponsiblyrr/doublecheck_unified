...
export const OfflineDetector: React.FC<OfflineDetectorProps> = ({
  ...
  const getNetworkMessage = () => {
    if (!status.isOnline) {
      return {
        ...
        message:
          "No internet connection. Your work is being saved locally and will sync when connection returns.",
        color: "orange",
      ...
      switch (status.networkQuality) {
        ...
        case "fair":
          return {
            ...
            message:
              "Connection quality is fair. Some features may load slowly.",
            color: "yellow",
      ...
      <div id="offline-detector-container" className={className}>
        ...
        >
          <div id="offline-banner-content" className="flex items-start gap-3">
            <div className={`mt-0.5 ${colors.text}`}>{networkInfo.icon}</div>
            ...
            <div id="offline-banner-text" className="flex-1 min-w-0">
              ...
              {status.syncQueueSize > 0 && (
                <div
                  id="sync-queue-info"
                  className={`flex items-center gap-2 mt-2 text-xs ${colors.text} opacity-75`}
                >
  ...
  if (variant === "toast") {
    return (
      <>
        ...
        >
          <div
            id="offline-toast-container"
            className={`border border-gray-200 rounded-lg shadow-lg p-4 ${colors.bg}`}
          >
            <div id="offline-toast-content" className="flex items-start gap-3">
              <div className={colors.text}>{networkInfo.icon}</div>
  ...
  if (variant === "inline") {
    return (
      <div id="offline-detector-container" className={className}>
        <div id={id} className={`rounded-md border p-3 ${colors.bg}`}>
