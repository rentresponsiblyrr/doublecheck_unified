...
import { ChecklistItem, ChecklistFilters, SystemHealth, ChecklistStats } from './types';
...
export const ChecklistDataManager: React.FC<ChecklistDataManagerProps> = ({ children }) => {
  ...
  const calculateStats = useCallback((checklistItems: ChecklistItem[]): ChecklistStats => {
    return checklistItems.reduce((acc, item) => {
      ...
      acc.byEvidenceType[item.evidence_type] = (acc.byEvidenceType[item.evidence_type] || 0) + 1;
  ...
  const applyFilters = useCallback((checklistItems: ChecklistItem[], filterOptions: ChecklistFilters) => {
    return checklistItems.filter(item => {
      if (filterOptions.search && !item.label.toLowerCase().includes(filterOptions.search.toLowerCase())) {
        return false;
      }
      if (filterOptions.category && item.category !== filterOptions.category) {
        return false;
      }
      if (filterOptions.evidenceType && item.evidence_type !== filterOptions.evidenceType) {
        return false;
      }
  ...
  const handleItemCreate = useCallback(async (formData: ChecklistFormData) => {
    try {
  ...
  const handleItemUpdate = useCallback(async (id: string, formData: ChecklistFormData) => {
    try {
