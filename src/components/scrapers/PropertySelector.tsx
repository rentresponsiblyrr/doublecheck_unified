...
export function PropertySelector({ 
   ...
   */
  const handlePropertySelect = useCallback((propertyData: PropertyData) => {
    try {
   ...
   */
  const handleAddProperty = useCallback(async (url: string) => {
    if (!user?.id) {
      throw new Error('User not authenticated');
    }

    // Security validation
    SecurityEvents.logUserAction(user.id, 'property_addition_attempted', { url });
    ...
    try {
      ...
      if (error) {
        log.error('Property addition failed', { error, url, userId: user.id });
        SecurityEvents.logSecurityEvent('property_addition_failed', user.id, { url, error });
        throw new Error('Failed to add property. Please check the URL and try again.');
      }

      log.info('Property added successfully', { propertyId: data.id, userId: user.id });
      SecurityEvents.logUserAction(user.id, 'property_added', { propertyId: data.id, url });
      ...
      log.error('Property addition exception', { error, url, userId: user.id });
      throw error;
    }
  ...
  return (
    <div className="space-y-6" role="main" aria-label="Property Selection">
      ...
      <PropertySearchFilter
        ...
        error={propertiesError instanceof Error ? propertiesError.message : null}
      />
