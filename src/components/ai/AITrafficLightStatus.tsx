...
export const AITrafficLightStatus: React.FC<AITrafficLightStatusProps> = ({
    ...
    switch (status) {
      ...
      case 'yellow':
        return {
          ...
          description: 'AI analysis shows moderate confidence - human review suggested',
      ...
      case 'red':
        return {
          ...
          description: 'AI analysis shows low confidence - human review mandatory',
  ...
  return (
    <div className={`p-4 rounded-lg border ${config.bgColor} ${config.borderColor} ${className}`} id="ai-traffic-light-status">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1">
            ...
            <Badge variant={config.badgeVariant} className="text-xs">
              {reliabilityResult ? `${Math.round(reliabilityResult.overallScore * 100)}%` : 'N/A'}
            </Badge>
          </div>
          ...
          {reliabilityResult && (
            <div className="mt-2 text-xs text-gray-600">
              Risk Level: {reliabilityResult.failureRiskLevel} • 
              Mitigations: {reliabilityResult.appliedMitigations.length}
            </div>
          )}
        </div>
      </div>
