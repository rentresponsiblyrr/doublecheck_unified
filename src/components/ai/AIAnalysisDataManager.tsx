...
import { aiReliabilityOrchestrator, ReliabilityAnalysis } from '@/services/AIReliabilityOrchestrator';
import { aiConfidenceValidator, ConfidenceValidationResult } from '@/services/AIConfidenceValidator';
import { aiExplainabilityEngine, ExplainabilityResult } from '@/services/AIExplainabilityEngine';
...
export const AIAnalysisDataManager: React.FC<AIAnalysisDataManagerProps> = ({
  ...
  const [reliabilityResult, setReliabilityResult] = useState<ReliabilityAnalysis | null>(null);
  const [confidenceResult, setConfidenceResult] = useState<ConfidenceValidationResult | null>(null);
  const [explainabilityResult, setExplainabilityResult] = useState<ExplainabilityResult | null>(null);
  const [analysisProgress, setAnalysisProgress] = useState(0);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const { toast } = useToast();

  const determineTrafficLightStatus = useCallback((reliability: ReliabilityAnalysis | null): 'green' | 'yellow' | 'red' => {
    if (!reliability) return 'red';
    
    if (reliability.overallScore >= 0.8 && reliability.failureRiskLevel === 'low') {
      return 'green';
    } else if (reliability.overallScore >= 0.6 && reliability.failureRiskLevel !== 'high') {
      return 'yellow';
    } else {
      return 'red';
    }
  ...
  const runAIAnalysis = useCallback(async () => {
    try {
      ...
      logger.info('Starting AI analysis', { checklistItem: checklistItem?.id }, 'AI_ANALYSIS_DATA_MANAGER');

      // Step 1: Reliability Analysis (40% progress)
      setAnalysisProgress(10);
      const reliabilityAnalysis = await aiReliabilityOrchestrator.analyzePhotoReliability(
        photo,
        checklistItem,
        analysisContext
      );
  ...
  const handleInitiateAppeal = useCallback(() => {
    logger.info('Initiating appeal process', { checklistItem: checklistItem?.id }, 'AI_ANALYSIS_DATA_MANAGER');
    toast({
  ...
  const canProceed = analysisState === 'complete' && trafficLightStatus !== 'red';
