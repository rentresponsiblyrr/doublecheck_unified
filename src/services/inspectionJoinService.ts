...
class InspectionJoinService extends StandardService {
   ...
   */
  async joinInspection(
    request: InspectionJoinRequest,
  ): Promise<ServiceResponse<InspectionJoinResult>> {
    ...
    try {
      ...
      const serviceError = this.createServiceError(
        ...
        "Inspection join workflow failed",
        {
          details: error,
          userMessage:
            "Unable to join inspection. Please try again or contact support.",
          category: "system",
          severity: "high",
          retryable: true,
        },
  ...
  private async validateInputs(
    ...
    const requiredValidation = ValidationUtils.validateRequired(request, [
      "userId",
    ]);
    ...
    const userIdValidation = ValidationUtils.validateFormat(
      request.userId,
      "uuid",
      "userId",
    );
    ...
    if (request.propertyId) {
      const propertyIdValidation = ValidationUtils.validateFormat(
        request.propertyId,
        "uuid",
        "propertyId",
      );
    ...
    try {
      ...
      if (error || !user) {
        ...
        return {
          success: false,
          error: userError,
        } as ServiceResponse<UserDetails>;
      }
   ...
   */
  private async resolvePropertyForInspection(
    request: InspectionJoinRequest,
  ): Promise<ServiceResponse<PropertySummary>> {
    try {
      ...
      if (!propertyId) {
        ...
        return {
          success: false,
          error: propertyError,
        } as ServiceResponse<PropertySummary>;
      }
      ...
      if (error || !property) {
        const propError = this.createServiceError(
          ...
          "Property not found",
          {
            details: error,
            userMessage:
              "Selected property not found. Please try another property.",
            category: "business",
            severity: "medium",
            retryable: false,
          },
    ...
    try {
      ...
      if (searchError) {
        ...
        return {
          success: false,
          error: dbError,
        } as ServiceResponse<InspectionDetails>;
      }
      ...
      if (existingInspections && existingInspections.length > 0) {
        ...
        const checklistResult = await this.populateInspectionChecklist(
          inspection.id,
        );
  ...
  private async buildInspectionJoinResult(
    ...
    const estimatedDuration = this.calculateEstimatedDuration(
      inspection.checklistItemsCount,
    );
   ...
   */
  private async populateInspectionChecklist(
    inspectionId: string,
