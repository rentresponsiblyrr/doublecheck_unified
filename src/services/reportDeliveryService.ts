...
© 2024 STR Certified. All rights reserved.
  ...
  async sendReportByEmail(
    ...
    try {
      logger.info(
        ...
        { inspectionId, recipientEmail, templateId },
        "REPORT_DELIVERY",
      );

      // Generate the report first
      const reportResult =
        await reportService.generateInspectionReport(inspectionId);
      ...
      const emailVariables = {
        ...
        passedItems: inspection.checklist_items.filter(
        ...
        urgentIssues: inspection.checklist_items
          .filter(
            (item) => item.ai_status === "fail" && item.category === "safety",
          )
          .map((item) => item.title)
          .join(", "),
      };

      const emailSubject = this.processTemplate(
        template.subject,
        emailVariables,
      );
      const emailHtml = this.processTemplate(
        template.htmlContent,
        emailVariables,
      );
      const emailText = this.processTemplate(
      ...
      const emailSent = await this.sendEmailViaProvider(
        recipientEmail,
        emailSubject,
        emailHtml,
        emailText,
  ...
  async getPropertyManagers(
    propertyId: string,
  ): Promise<PropertyManagerContact[]> {
    try {
