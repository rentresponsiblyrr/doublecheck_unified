...
class SchemaValidationService {
  ...
  async validateInspectionData(data: {
    ...
    try {
      ...
      if (!constraints.validStatuses.includes(data.status)) {
        ...
        result.errors.push(
          `Invalid status '${data.status}'. Valid statuses: ${constraints.validStatuses.join(", ")}`,
        );
      ...
      if (!data.property_id || data.property_id.length === 0) {
        ...
        !this.isValidUUID(data.property_id) &&
        !this.isValidInteger(data.property_id)
      ) {
  ...
  async getInspectionConstraints(): Promise<InspectionConstraints> {
    ...
    try {
      ...
      log.warn(
        "Could not fetch database constraints, using defaults",
        {
  ...
  async validateTableAccess(): Promise<SchemaValidationResult> {
      ...
      try {
        ...
        if (error) {
          if (error.code === "PGRST116" || error.message?.includes("404")) {
            result.errors.push(
              `Table '${table}' is not accessible (404 error)`,
            );
            result.isValid = false;
          } else if (error.code === "42501") {
            result.warnings.push(
              `Insufficient permissions for table '${table}'`,
            );
          } else {
            result.warnings.push(
              `Error accessing table '${table}': ${error.message}`,
            );
          }
        }
      } catch (error) {
        result.errors.push(
          `Failed to test table '${table}': ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        result.isValid = false;
      }
    }
    ...
    if (result.errors.length > 0) {
      result.suggestions.push(
        "Check database RLS policies and user permissions",
      );
  ...
  private async verifyPropertyExists(propertyId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        log.warn(
          "Could not verify property via RPC function",
          {
            component: "SchemaValidationService",
  ...
  async validateChecklistItemData(data: {
    ...
    if (!validStatuses.includes(data.status)) {
      result.isValid = false;
      result.errors.push(
        `Invalid checklist item status. Valid: ${validStatuses.join(", ")}`,
      );
    }
