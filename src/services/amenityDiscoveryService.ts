...
export class AmenityDiscoveryService {
  ...
  async compareInspectionToListing(
    ...
    try {
      logger.info(
        "Starting amenity comparison analysis",
        {
      ...
      for (const rule of this.AMENITY_MATCHING_RULES) {
        const discovery = await this.analyzeAmenityRule(
          rule,
          inspection,
          scrapedData,
        );
      ...
      scrapedData.amenities.forEach((amenity) => {
        const hasEvidence = this.hasInspectionEvidence(
          amenity.name,
          inspection,
        );
        if (hasEvidence) {
      ...
      logger.error(
        "Failed to compare inspection to listing",
        error,
        "AMENITY_DISCOVERY",
      );
      throw error;
  ...
  private async analyzeAmenityRule(
    ...
    const relevantChecklistItems = inspection.checklist_items.filter((item) => {
      ...
      return rule.checklistItemKeywords.some((keyword) =>
    ...
    const hasEvidence =
      evidence.checklistItems.length > 0 || evidence.photos.length > 0;
    ...
    discoveredAmenities.forEach((discovery) => {
      if (discovery.currentStatus === "not_listed") {
        amenityCheckboxes.push(
          `Check "${discovery.name}" in your listing amenities`,
        );
        descriptionAdditions.push(
          `Add "${discovery.name}" to your property description`,
        );
      } else if (discovery.currentStatus === "listed_but_not_highlighted") {
        descriptionAdditions.push(
          `Highlight "${discovery.name}" more prominently in your description`,
        );
      }

      if (
        discovery.evidence.photos.length > 0 &&
        discovery.priority === "high"
      ) {
        photoRecommendations.push(
          `Consider featuring "${discovery.name}" photos more prominently in your listing`,
        );
      }
    });
  ...
  private generateReasoning(
    ...
    if (evidence.checklistItems.length > 0) {
      reasons.push(
        `Found ${evidence.checklistItems.length} inspection item(s) related to ${rule.amenityName}`,
      );
    }

    if (evidence.photos.length > 0) {
      reasons.push(
        `Found ${evidence.photos.length} photo(s) showing ${rule.amenityName}`,
      );
    }
  ...
  async analyzeRoomAmenities(
    ...
    { roomType: string; discoveredAmenities: string[]; suggestions: string[] }[]
  > {
    const roomAnalysis: {
      roomType: string;
      discoveredAmenities: string[];
      suggestions: string[];
    }[] = [];
    ...
    for (const roomType of roomTypes) {
      const roomItems = inspection.checklist_items.filter(
        (item) =>
          ...
          (item as { room_type?: string }).room_type?.toLowerCase() ===
            roomType,
      );
      ...
      roomItems.forEach((item) => {
        ...
        const features = this.extractRoomFeatures(
          item.title,
          item.ai_reasoning,
          roomType,
        );
      ...
      if (discoveredAmenities.length > 0) {
        suggestions.push(
          `Consider highlighting ${roomType} features: ${discoveredAmenities.join(", ")}`,
        );
      }
