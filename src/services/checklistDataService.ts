...
export class ChecklistDataService {
  async fetchStaticSafetyItems(): Promise<StaticSafetyItem[]> {
    log.info(
      "Fetching static safety items from database",
      {
    ...
    if (fetchError) {
      log.error(
        "Error fetching static safety items",
        fetchError,
        {
          component: "ChecklistDataService",
          action: "fetchStaticSafetyItems",
  ...
  async insertChecklistItems(checklistItems: ChecklistItem[]): Promise<void> {
    ...
    try {
      ...
      log.warn(
        "Checklist data service needs redesign - current table is for templates only",
        {
          component: "ChecklistDataService",
          action: "insertChecklistItems",
      ...
      log.info(
        "Successfully inserted checklist items",
        {
          component: "ChecklistDataService",
          action: "insertChecklistItems",
      ...
      log.error(
        "Checklist items insertion error",
        error as Error,
        {
          component: "ChecklistDataService",
          action: "insertChecklistItems",
