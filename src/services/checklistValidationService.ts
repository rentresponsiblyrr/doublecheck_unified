...
export class ChecklistValidationService {
  ...
  validateStaticItems(staticItems: StaticSafetyItem[]): void {
    if (!staticItems || staticItems.length === 0) {
      throw new Error(
        "No valid static safety items found for checklist population",
      );
    }

    // Log category distribution
    const categoryStats = staticItems.reduce(
      (acc, item) => {
  ...
  async validateChecklistCreation(
    ...
    const finalCategoryStats = checklistItems.reduce(
      (acc, item) => {
