...
import {
  ...
  type InspectionCreationRequest,
...
export class MobileInspectionOrchestrator {
  ...
  static async getOrCreateInspectionOptimized(
    ...
    try {
      log.info(
        "Mobile inspection workflow initiated",
        {
      ...
      if (inspectionResult.inspection) {
        ...
        const checklistResult = await this.executeWithRetry(
          () =>
            InspectionCreationService.populateChecklist(
              creationResult.inspectionId,
            ),
        ...
        if (!checklistResult.success) {
          ...
          if (creationResult.rollback) {
            try {
              ...
              log.warn(
                "Inspection rolled back due to checklist population failure",
                {
                  component: "MobileInspectionOrchestrator",
                  action: "getOrCreateInspectionOptimized",
              ...
              log.error(
                "Failed to rollback inspection after checklist failure",
        ...
        const newInspectionResult =
          await InspectionQueryService.getInspectionById(
            creationResult.inspectionId,
          );
      ...
      log.info(
        "Mobile inspection workflow completed successfully",
        {
          component: "MobileInspectionOrchestrator",
          action: "getOrCreateInspectionOptimized",
          propertyId,
      ...
      log.error(
        "Mobile inspection workflow failed with exception",
        error as Error,
        {
          component: "MobileInspectionOrchestrator",
          action: "getOrCreateInspectionOptimized",
  ...
  static async getInspectionDashboard(inspectorId: string): Promise<{
    ...
    try {
      log.debug(
        "Loading inspection dashboard data",
        {
          component: "MobileInspectionOrchestrator",
      ...
      const propertyIds = [
        ...new Set(activeInspections.map((i) => i.property_id)),
      ];
      const recentProperties =
        await PropertyLookupService.getMultipleProperties(propertyIds);
  ...
  private static async executeWithRetry<T>(
    ...
    throw (
      lastError ||
      new Error(
        `Operation ${operationName} failed after ${this.MAX_RETRIES} attempts`,
      )
    );
  }
   ...
   */
  private static async getChecklistItemCount(
    inspectionId: string,
  ): Promise<number> {
    try {
      ...
      if (inspectionError) {
        log.warn(
          "Failed to get inspection for checklist count",
          {
            component: "MobileInspectionOrchestrator",
      ...
      if (!inspectionData) {
        log.warn(
          "Inspection not found for checklist count",
          {
            component: "MobileInspectionOrchestrator",
            action: "getChecklistItemCount",
            inspectionId,
      ...
      if (error) {
        log.warn(
          "Failed to get checklist item count",
          {
            component: "MobileInspectionOrchestrator",
            action: "getChecklistItemCount",
            inspectionId,
      ...
      log.error(
        "Exception getting checklist item count",
        error as Error,
        {
          component: "MobileInspectionOrchestrator",
          action: "getChecklistItemCount",
