...
export class InspectionCreationService {
   ...
   */
  static async createInspection(request: InspectionCreationRequest): Promise<InspectionCreationResult> {
    ...
    try {
      ...
      log.info('Inspection creation initiated', {
      ...
      const existingCheck = await this.checkForExistingActiveInspection(property_id);
      ...
      log.error('Inspection creation failed with exception', error as Error, {
        component: 'InspectionCreationService',
        action: 'createInspection',
        request,
        createdInspectionId,
      ...
      if (createdInspectionId) {
        try {
          ...
          log.error('Failed to rollback partially created inspection', rollbackError as Error, {
            component: 'InspectionCreationService',
            action: 'createInspection',
            inspectionId: createdInspectionId
   ...
   */
  static async populateChecklist(inspectionId: string): Promise<ChecklistPopulationResult> {
    try {
      ...
      log.info('Checklist population initiated', {
        component: 'InspectionCreationService',
      ...
      const { data: populationResult, error: populationError } = await Promise.race([
        supabase.rpc('populate_inspection_checklist_safe', {
          inspection_id: cleanInspectionId
        }),
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Checklist population timeout')), this.TIMEOUT_MS)
        )
      ]);

      if (populationError) {
        log.error('Checklist population failed', populationError, {
          component: 'InspectionCreationService',
          action: 'populateChecklist',
          inspectionId: cleanInspectionId,
      ...
      log.info('Checklist population completed', {
        component: 'InspectionCreationService',
        action: 'populateChecklist',
        inspectionId: cleanInspectionId,
        itemsCreated
      ...
      log.error('Checklist population failed with exception', error as Error, {
        component: 'InspectionCreationService',
        action: 'populateChecklist',
        inspectionId,
        errorType: error?.constructor?.name || 'UnknownError'
   ...
   */
  private static validateCreationRequest(request: InspectionCreationRequest): { valid: boolean; error?: string } {
    if (!request) {
    ...
    if (request.start_time) {
      ...
      if (isNaN(parsedDate.getTime())) {
        return { valid: false, error: 'Start time must be a valid ISO date string' };
      }
    }
   ...
   * Check for existing active inspection
   */
  private static async checkForExistingActiveInspection(property_id: string): Promise<{ canCreate: boolean; reason?: string }> {
    try {
      ...
      if (error) {
        log.warn('Could not check for existing inspections', {
          component: 'InspectionCreationService',
      ...
      if (existingInspections && existingInspections.length > 0) {
        ...
        log.warn('Active inspection already exists', {
          component: 'InspectionCreationService',
          action: 'checkForExistingActiveInspection',
          property_id,
      ...
      log.error('Exception while checking for existing inspections', error as Error, {
        component: 'InspectionCreationService',
        action: 'checkForExistingActiveInspection',
        property_id
  ...
  private static async rollbackInspection(inspectionId: string): Promise<void> {
    try {
      log.warn('Rolling back inspection creation', {
        component: 'InspectionCreationService',
      ...
      if (inspectionError) {
        log.error('Failed to delete inspection during rollback', inspectionError, {
          component: 'InspectionCreationService',
          action: 'rollbackInspection',
          inspectionId
      ...
      log.info('Inspection rollback completed', {
        component: 'InspectionCreationService',
        action: 'rollbackInspection',
        inspectionId
      ...
      log.error('Inspection rollback failed', error as Error, {
        component: 'InspectionCreationService',
        action: 'rollbackInspection',
        inspectionId
