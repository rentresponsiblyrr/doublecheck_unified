...
export class InspectionQueryService {
   ...
   */
  static async findActiveInspection(propertyId: string): Promise<InspectionQueryResult> {
    try {
      ...
      log.debug('Active inspection search initiated', {
      ...
      const { data, error } = await Promise.race([
        supabase
          .from('inspections')
          .select('id, property_id, inspector_id, status, completed, start_time, end_time')
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Active inspection query timeout')), this.TIMEOUT_MS)
        )
      ]);

      if (error) {
        log.error('Active inspection query failed', error, {
          component: 'InspectionQueryService',
          action: 'findActiveInspection',
          propertyId: cleanPropertyId,
      ...
      if (!data) {
        log.debug('No active inspection found', {
          component: 'InspectionQueryService',
          action: 'findActiveInspection',
          propertyId: cleanPropertyId
      ...
      log.info('Active inspection found', {
        component: 'InspectionQueryService',
        action: 'findActiveInspection',
        propertyId: cleanPropertyId,
      ...
      log.error('Active inspection search failed with exception', error as Error, {
        component: 'InspectionQueryService',
        action: 'findActiveInspection',
   ...
   */
  static async getInspectionById(inspectionId: string): Promise<InspectionQueryResult> {
    try {
      ...
      log.debug('Inspection lookup by ID initiated', {
        component: 'InspectionQueryService',
      ...
      const { data, error } = await Promise.race([
        supabase
          ...
          .single(),
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Inspection by ID query timeout')), this.TIMEOUT_MS)
        )
      ]);
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          // No rows returned
          log.warn('Inspection not found by ID', {
            component: 'InspectionQueryService',
            action: 'getInspectionById',
            inspectionId: cleanInspectionId
        ...
        log.error('Inspection by ID query failed', error, {
          component: 'InspectionQueryService',
          action: 'getInspectionById',
          inspectionId: cleanInspectionId,
          errorCode: error.code
      ...
      log.info('Inspection found by ID', {
        component: 'InspectionQueryService',
        action: 'getInspectionById',
        inspectionId: cleanInspectionId,
      ...
      log.error('Inspection by ID lookup failed with exception', error as Error, {
        component: 'InspectionQueryService',
        action: 'getInspectionById',
   ...
   */
  static async queryInspections(filters: InspectionStatusFilter = {}): Promise<InspectionSummary[]> {
    try {
      const {
        completed,
        status,
      ...
      log.debug('Inspection query with filters initiated', {
        component: 'InspectionQueryService',
      ...
      const { data, error } = await Promise.race([
        query,
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Inspection query timeout')), this.TIMEOUT_MS)
        )
      ]);

      if (error) {
        log.error('Inspection query with filters failed', error, {
          component: 'InspectionQueryService',
          action: 'queryInspections',
          filters,
          errorCode: error.code
      ...
      log.info('Inspection query completed', {
        component: 'InspectionQueryService',
        action: 'queryInspections',
      ...
      log.error('Inspection query failed with exception', error as Error, {
        component: 'InspectionQueryService',
        action: 'queryInspections',
        filters,
        errorType: error?.constructor?.name || 'UnknownError'
   ...
   */
  static async getInspectionCounts(inspector_id?: string): Promise<Record<string, number>> {
    try {
      log.debug('Inspection count query initiated', {
        component: 'InspectionQueryService',
      ...
      const { data, error } = await Promise.race([
        query,
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Inspection count query timeout')), this.TIMEOUT_MS)
        )
      ]);

      if (error) {
        log.error('Inspection count query failed', error, {
          component: 'InspectionQueryService',
          action: 'getInspectionCounts',
          inspector_id,
          errorCode: error.code
      ...
      log.info('Inspection count query completed', {
        component: 'InspectionQueryService',
        action: 'getInspectionCounts',
        inspector_id,
        statusCounts: counts,
      ...
      log.error('Inspection count query failed with exception', error as Error, {
        component: 'InspectionQueryService',
        action: 'getInspectionCounts',
        inspector_id,
        errorType: error?.constructor?.name || 'UnknownError'
