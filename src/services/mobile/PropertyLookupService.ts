...
export class PropertyLookupService {
  ...
  static async getPropertyInfo(
    ...
    try {
      ...
      if (!propertiesData || propertiesData.length === 0) {
        log.warn(
          "No properties available in database",
          {
            component: "PropertyLookupService",
            action: "getPropertyInfo",
            propertyId: cleanPropertyId,
      ...
      const property = propertiesData.find(
        (p) => p.id?.toString() === cleanPropertyId.toString(),
      );

      if (!property) {
        log.warn(
          "Property not found in available properties",
          {
            component: "PropertyLookupService",
            action: "getPropertyInfo",
            propertyId: cleanPropertyId,
      ...
      log.error(
        "Property lookup failed with exception",
        error as Error,
        {
          component: "PropertyLookupService",
          action: "getPropertyInfo",
   ...
   */
  static validatePropertyId(propertyId: string): {
    valid: boolean;
    error?: string;
  } {
    ...
    if (!isUUID && !isInteger) {
      return {
        valid: false,
        error: "Property ID must be either a UUID or integer",
      };
    }
   ...
   */
  static async getMultipleProperties(
    propertyIds: string[],
    ...
    try {
      ...
      const validIds = propertyIds.filter(
        (id) => this.validatePropertyId(id).valid,
      );

      if (validIds.length === 0) {
        log.warn(
          "No valid property IDs provided for batch lookup",
          {
            component: "PropertyLookupService",
      ...
      log.info(
        "Batch property lookup completed",
        {
          component: "PropertyLookupService",
          action: "getMultipleProperties",
          requestedCount: propertyIds.length,
      ...
      log.error(
        "Batch property lookup exception",
        error as Error,
        {
          component: "PropertyLookupService",
          action: "getMultipleProperties",
