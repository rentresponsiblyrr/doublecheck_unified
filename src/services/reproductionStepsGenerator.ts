...
interface UserStep {
  timestamp: string;
  action: 'page_view' | 'click' | 'form_submit' | 'scroll' | 'type' | 'wait' | 'error';
  target?: string;
  value?: string;
  duration: number;
  url: string;
...
export class ReproductionStepsGenerator {
  ...
  async generateReproductionSteps(
    ...
    try {
      ...
      const behaviorAnalysis = await this.analyzeUserBehavior(userJourney, frustrationMetrics);
      
      // Generate context-aware reproduction steps
      const steps = await this.generateAISteps(errorDetails, systemContext, behaviorAnalysis, userJourney);
      
      // Enhance with system-specific knowledge
      const enhancedSteps = await this.enhanceWithSystemKnowledge(steps, errorDetails);
  ...
  private async analyzeUserBehavior(
    ...
    return {
      ...
      userFrustration: frustrationMetrics ? {
        ...
        formAbandonment: frustrationMetrics.contextualClues.formAbandonmentRate
      } : null,
  ...
  private identifyCommonPatterns(steps: UserStep[]): string[] {
    ...
    const rapidClicks = steps.filter((step, index) => {
      ...
      return nextStep && nextStep.action === 'click' && 
             (new Date(nextStep.timestamp).getTime() - new Date(step.timestamp).getTime()) < 1000;
    });
    ...
    const formSubmissions = steps.filter(step => step.action === 'form_submit');
  ...
  private identifyCriticalSteps(steps: UserStep[]): UserStep[] {
    ...
    const lastSuccessfulSteps = steps
      .filter(step => step.success)
      .slice(-5);
    
    const firstFailedStep = steps.find(step => !step.success);
    
    return firstFailedStep ? [...lastSuccessfulSteps, firstFailedStep] : lastSuccessfulSteps;
  }
...
`;
    ...
    const response = await this.openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert QA engineer specializing in mobile-first web applications and AI-powered platforms. Create detailed, actionable reproduction steps in JSON format."
        },
        {
          role: "user",
          content: prompt
    ...
    if (errorDetails.code === '23514') {
      enhancements.troubleshootingTips = [
        ...steps.troubleshootingTips || [],
        {
          ...
          suggestion: 'Check if database schema allows the status value being used',
          explanation: 'Error 23514 indicates a check constraint violation in PostgreSQL'
        },
        {
          condition: 'Inspection creation fails consistently',
          suggestion: 'Verify static_safety_items table has required data',
          explanation: 'Inspection creation depends on static safety items for checklist generation'
        }
      ];
    }
   ...
   */
  private async optimizeSteps(steps: ReproductionSteps): Promise<ReproductionSteps> {
   ...
   */
  private inferTiming(step: ReproductionStep): { waitTime?: string; timeout?: string } {
   ...
   */
  private addValidation(step: ReproductionStep): { checkFor: string[]; avoidIf: string[] } {
    const action = step.action.toLowerCase();
    
    if (action.includes('login') || action.includes('authenticate')) {
      return {
        checkFor: ['User is redirected to dashboard', 'No error messages displayed'],
    ...
    if (action.includes('create') || action.includes('submit')) {
      return {
        ...
        avoidIf: ['Error message appears', 'Form still shows validation errors']
      };
    }
  ...
  private getFallbackSteps(
    ...
    return {
      ...
      environmentRequirements: [
        {
          type: 'browser',
          requirement: systemContext.browser.name + ' ' + systemContext.browser.version,
          critical: false
        }
      ],
      troubleshootingTips: [
  ...
  private generateCacheKey(errorDetails: ErrorDetails, systemContext: SystemContext): string {
    return `${errorDetails.message}-${errorDetails.type}-${systemContext.browser.name}`.slice(0, 100);
  }
  ...
  async validateSteps(steps: ReproductionSteps): Promise<{
    ...
    if (!steps.prerequisites || steps.prerequisites.length === 0) {
      ...
      suggestions.push('Add prerequisites like account requirements, test data, etc.');
    }

    // Check for missing environment requirements
    if (!steps.environmentRequirements || steps.environmentRequirements.length === 0) {
    ...
    const incompleteSteps = steps.steps.filter(step => 
      !step.action || !step.details || !step.expectedResult
    );
    
    if (incompleteSteps.length > 0) {
      issues.push(`${incompleteSteps.length} incomplete steps found`);
      suggestions.push('Ensure all steps have action, details, and expected result');
    }

    // Check for logical step ordering
    const navigationSteps = steps.steps.filter(step => 
    ...
    if (navigationSteps.length > 0 && navigationSteps[0].stepNumber !== 1) {
      ...
      suggestions.push('Reorder steps to start with authentication and navigation');
    }
