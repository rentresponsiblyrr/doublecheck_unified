...
export class AmenityComparisonEngine {
  ...
  async generateOptimizationReport(
    ...
    try {
      logger.info(
        "Starting comprehensive amenity comparison",
        {
      ...
      const [discoveryResult, missingResult] = await Promise.all([
        amenityDiscoveryService.compareInspectionToListing(
          inspection,
          scrapedData,
        ),
      ...
      logger.info(
        "Amenity comparison report generated",
        {
  ...
  private processDiscoveredOpportunities(
    ...
    const criticalMissing = missingResult.criticalMissing.map(
      (alert: MissingAmenityAlert) => ({
    ...
    const underUtilized = missingResult.underUtilized.map(
      (alert: UnderUtilizedAlert) => ({
        amenity: alert.amenityName,
  ...
  private extractRoomSpecificFindings(inspection: InspectionForReview) {
    ...
    roomTypes.forEach((roomType) => {
      const roomItems = inspection.checklist_items.filter(
        (item) =>
          item.title.toLowerCase().includes(roomType) ||
          ("room_type" in item &&
            typeof item.room_type === "string" &&
            item.room_type.toLowerCase() === roomType),
      );
        ...
        roomItems.forEach((item) => {
          ...
          if (item.ai_confidence > 80 && item.status === "completed") {
            recommendations.push(
              `Consider highlighting "${item.title}" in your ${roomType} description`,
            );
          }
        });
  ...
  private generateActionableRecommendations(
    ...
    discoveryResult.suggestions.descriptionAdditions.forEach(
      (addition: string) => {
        descriptionUpdates.push({
          section: "main_description",
          currentText: this.extractRelevantDescriptionSection(
            scrapedData.description,
            addition,
          ),
          suggestedAddition: addition,
          rationale:
            "Found evidence during inspection but not mentioned in description",
        });
      },
    );
   ...
   */
  private extractRelevantDescriptionSection(
    description: string,
    addition: string,
  ): string {
  ...
  private generateSummary(
    ...
    const totalOpportunities =
      missingResult.criticalMissing.length +
      missingResult.underUtilized.length +
      missingResult.opportunityAmenities.length;

    const highPriorityActions = missingResult.criticalMissing.filter(
      (alert: MissingAmenityAlert) =>
    ...
    if (missingResult.criticalMissing.length > 0) {
      const highValueMissing = missingResult.criticalMissing.filter(
        (alert: MissingAmenityAlert) => alert.category === "high_value",
      );
      if (highValueMissing.length > 0) {
        keyFindings.push(
          `${highValueMissing.length} high-value amenities found but not listed`,
        );
      }
    }
    ...
    if (missingResult.underUtilized.length > 0) {
      keyFindings.push(
        `${missingResult.underUtilized.length} amenities could be featured more prominently`,
      );
    }

    if (recommendations.photoSuggestions.length > 0) {
      keyFindings.push(
        `${recommendations.photoSuggestions.length} opportunities to improve photo presentation`,
      );
    }
  ...
  private calculateEstimatedImpact(missingResult: MissingResult): string {
    const highValueCount = missingResult.criticalMissing.filter(
      (alert: MissingAmenityAlert) => alert.category === "high_value",
    ).length;
  ...
  async calculateOptimizationMetrics(
    ...
    try {
      ...
      // Calculate metrics
      const totalPossibleAmenities =
        scrapedData.amenities.length + missingResult.summary.totalMissing;
      ...
      const listingCompleteness =
        totalPossibleAmenities > 0
          ? Math.round((accurateAmenities / totalPossibleAmenities) * 100)
          : 100;

      const amenityAccuracy =
        scrapedData.amenities.length > 0
          ? Math.round((accurateAmenities / scrapedData.amenities.length) * 100)
          : 100;

      const opportunityScore =
        missedOpportunities > 0
          ? Math.max(0, 100 - missedOpportunities * 10)
          : 100;
      ...
      logger.error(
        "Failed to calculate optimization metrics",
        error,
        "AMENITY_COMPARISON_ENGINE",
      );
      throw error;
  ...
  async getQuickActionableChecklist(
    ...
    const missingResult = await missingAmenityDetector.detectMissingAmenities(
      inspection,
      scrapedData,
    );
