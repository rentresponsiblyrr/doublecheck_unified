...
export class TransactionError extends DatabaseError {
  constructor(message: string, public operationIndex: number) {
...
export class UnifiedDatabaseService {
  ...
  private async ensureAuthenticated(): Promise<UserId> {
    ...
    if (error || !user) {
      throw new DatabaseError('Authentication required for database operations', 'AUTH_REQUIRED');
    }
  ...
  private async trackOperation<T>(
    ...
    try {
      logger.debug(`Starting database operation: ${operationName}`, { operationId });
  ...
  async getAllUsers(): Promise<DatabaseUser[]> {
    return this.trackOperation('getAllUsers', async () => {
      ...
      if (error) {
        throw new DatabaseError('Failed to fetch users', 'USER_FETCH_FAILED', error);
      }
  ...
  async getUserById(id: UserId): Promise<DatabaseUser | null> {
    return this.trackOperation('getUserById', async () => {
      ...
      if (error) {
        ...
        throw new DatabaseError('Failed to fetch user', 'USER_FETCH_FAILED', error);
      }
  ...
  async createUser(userData: Omit<DatabaseUser, 'id' | 'created_at' | 'updated_at'>): Promise<DatabaseUser> {
    return this.trackOperation('createUser', async () => {
      ...
      if (error) {
        throw new DatabaseError('Failed to create user', 'USER_CREATE_FAILED', error);
      }
   ...
   * Update user
   */
  async updateUser(id: UserId, updates: Partial<DatabaseUser>): Promise<DatabaseUser> {
    return this.trackOperation('updateUser', async () => {
      ...
      if (error) {
        throw new DatabaseError('Failed to update user', 'USER_UPDATE_FAILED', error);
      }
    ...
    return this.trackOperation('getPropertiesWithInspections', async () => {
      ...
      if (filters.has_inspections !== undefined) {
        properties = properties.filter(p => 
          filters.has_inspections ? p.inspection_count > 0 : p.inspection_count === 0
        );
      }
      
      if (filters.created_after) {
        properties = properties.filter(p => p.created_at >= filters.created_after!);
      }
      
      if (filters.created_before) {
        properties = properties.filter(p => p.created_at <= filters.created_before!);
      }
  ...
  async getPropertyById(id: number): Promise<DatabaseProperty | null> {
    return this.trackOperation('getPropertyById', async () => {
      ...
      if (error) {
        ...
        throw new DatabaseError('Failed to fetch property', 'PROPERTY_FETCH_FAILED', error);
      }
   ...
   */
  async getInspectionById(id: InspectionId): Promise<InspectionWithItems | null> {
  ...
  async getAllSafetyItems(): Promise<StaticSafetyItem[]> {
    return this.trackOperation('getAllSafetyItems', async () => {
      ...
      if (error) {
        throw new DatabaseError('Failed to fetch safety items', 'SAFETY_ITEMS_FETCH_FAILED', error);
      }
  ...
  async getHealthStatus(): Promise<DatabaseHealthStatus> {
    ...
    try {
      ...
      try {
        ...
        issues.push(`Authentication: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
