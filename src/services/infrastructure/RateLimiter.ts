...
export class RateLimiter {
  ...
  getAnalytics(): {
    ...
    let totalRequests = 0;
    let totalCost = 0;
    const operationStats = new Map<string, { requests: number; cost: number }>();
    ...
    const blockedRequests = Array.from(this.blocked.values())
      .filter(blockedUntil => blockedUntil > Date.now())
      .length;
