...
export interface AuditorFeedback {
  type: 'accuracy' | 'relevance' | 'completeness' | 'false_positive' | 'false_negative';
...
export class AILearningRepository {
  ...
  async recordFeedback(data: {
    ...
    try {
      ...
      logger.error('Failed to record auditor feedback', { error, analysisId: data.analysisId });
      throw error;
    }
  }
  ...
  private calculateProviderMetrics(): Record<string, ProviderMetrics> {
    ...
    for (const [provider, records] of providers) {
      ...
      const correctPredictions = withFeedback.filter(r => r.actualOutcome === 'correct');
      const totalCost = records.reduce((sum, r) => sum + r.cost, 0);
      const totalTime = records.reduce((sum, r) => sum + r.processingTime, 0);
      
      const accuracy = withFeedback.length > 0 ? correctPredictions.length / withFeedback.length : 0;
  ...
  private calculateLearningAnalytics(): LearningAnalytics {
    ...
    for (const provider of ['openai', 'claude']) {
      const providerRecords = withFeedback.filter(r => r.provider === provider);
      const providerCorrect = providerRecords.filter(r => r.actualOutcome === 'correct');
      
      accuracyByProvider[provider] = providerRecords.length > 0 
    ...
    const confidenceRanges = {
      ...
      'medium (0.6-0.8)': records.filter(r => r.confidence >= 0.6 && r.confidence < 0.8).length,
      'low (0.0-0.6)': records.filter(r => r.confidence < 0.6).length
    };
    
    // Simple improvement trend (placeholder)
    const improvementTrend = [
      { date: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], accuracy: 0.75 },
      { date: new Date().toISOString().split('T')[0], accuracy: withFeedback.length > 0 ? correct.length / withFeedback.length : 0 }
    ];
    
    return {
      totalAnalyses: records.length,
      totalFeedback: withFeedback.length,
      overallAccuracy: withFeedback.length > 0 ? correct.length / withFeedback.length : 0,
      accuracyByProvider,
  ...
  private determineOutcome(
    ...
    if ((predictedStatus === 'needs_review' && actualStatus !== 'pass') ||
