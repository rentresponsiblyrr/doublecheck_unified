...
export class AuditorService {
  ...
  async getInspectionsPendingReview(
    ...
    success: boolean;
    data?: InspectionForReview[];
    error?: string;
  }> {
    try {
      logger.info(
        "Fetching inspections pending review",
        { filters, limit },
        "AUDITOR_SERVICE",
      );
      ...
      if (error) {
        logger.error(
          "Failed to fetch inspections for review",
          error,
          "AUDITOR_SERVICE",
        );
      ...
      const enhancedInspections: InspectionForReview[] = (data || []).map(
        (inspection) => ({
          ...inspection,
          ai_analysis_summary: this.calculateAIAnalysisSummary(
      ...
      logger.info(
        "Successfully fetched inspections for review",
        {
      ...
      return {
        success: false,
        error: "Failed to fetch inspections for review",
      };
    }
  }
  ...
  async getInspectionForReview(
    ...
    try {
      logger.info(
        "Fetching detailed inspection for review",
        { inspectionId },
        "AUDITOR_SERVICE",
      );
  ...
  async submitReviewDecision(
    ...
    try {
      logger.info(
        "Submitting auditor review decision",
        {
      ...
      if (inspectionError) {
        throw new Error(
          `Failed to update inspection status: ${inspectionError.message}`,
        );
      }
      ...
      for (const override of reviewDecision.overrides) {
        ...
        if (itemError) {
          logger.error(
            "Failed to apply override to checklist item",
            {
      ...
      try {
        ...
        if (feedbackError) {
          logger.warn(
            "Failed to store audit feedback for learning",
            feedbackError,
            "AUDITOR_SERVICE",
          );
          ...
          logger.info(
            "Successfully stored audit feedback for AI learning",
            "AUDITOR_SERVICE",
          );
        }
      } catch (auditError) {
        logger.warn(
          "Audit feedback table may not exist, skipping AI learning storage",
          auditError,
          "AUDITOR_SERVICE",
        );
      ...
      logger.info(
        "Successfully submitted review decision",
        {
          inspectionId: reviewDecision.inspectionId,
          decision: reviewDecision.decision,
        },
  ...
  async getAuditorMetrics(
    ...
    try {
      ...
      if (metricsError) {
        throw new Error(
          `Failed to fetch inspection metrics: ${metricsError.message}`,
        );
      }
      ...
      if (feedbackError) {
        logger.warn(
          "Failed to fetch feedback data for metrics",
          feedbackError,
          "AUDITOR_SERVICE",
        );
      }
      ...
      const approvedCount = completedInspections.filter(
        (i) => i.status === "approved",
      ).length;
      const approvalRate =
        completedInspections.length > 0
          ? (approvedCount / completedInspections.length) * 100
          : 0;

      const avgReviewTime =
        feedbackData && feedbackData.length > 0
          ? feedbackData.reduce(
              (sum, f) => sum + (f.review_time_minutes || 0),
              0,
            ) / feedbackData.length
          : 0;

      const totalOverrides =
        feedbackData?.reduce((sum, f) => sum + (f.overrides_count || 0), 0) ||
        0;
      const totalItems = feedbackData?.length || 0;
      const overrideRate =
        totalItems > 0 ? (totalOverrides / totalItems) * 100 : 0;
      ...
      logger.info(
        "Successfully calculated auditor metrics",
        metrics,
        "AUDITOR_SERVICE",
      );
      ...
      logger.error(
        "Failed to calculate auditor metrics",
        error as Error,
        "AUDITOR_SERVICE",
      );
   ...
   */
  private calculateAIAnalysisSummary(
    ...
    const completedItems = inspectionChecklistItems.filter(
      (item) => item.status === "completed",
    ).length;
    const passedItems = inspectionChecklistItems.filter(
      (item) => item.ai_status === "pass",
    ).length;
    const failedItems = inspectionChecklistItems.filter(
      (item) => item.ai_status === "fail",
    ).length;
    const needsReviewItems = inspectionChecklistItems.filter(
    ...
    inspectionChecklistItems.forEach((item) => {
      if (item.media) {
        photoCount += item.media.filter(
          (media) => media.type === "photo",
        ).length;
        videoCount += item.media.filter(
