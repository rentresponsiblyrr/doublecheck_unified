...
const PROPERTY_SERVICE_CONFIG = {
  ...
  cacheKeys: {
    ...
    availableProperties: (inspectorId?: string) =>
      `available_properties${inspectorId ? `:${inspectorId}` : ""}`,
    propertyStats: (id: string, timeRange: string) =>
      `property_stats:${id}:${timeRange}`,
    propertyInspectionHistory: (id: string) => `property_inspections:${id}`,
    propertySearch: (query: string) =>
      `property_search:${Buffer.from(query).toString("base64")}`,
  },
...
export class PropertyDataService {
  ...
  async getPropertiesWithStatus(
    ...
    const cacheKey =
      PROPERTY_SERVICE_CONFIG.cacheKeys.propertiesList(filterKey);
    ...
    try {
        ...
        data.map(async (property) => {
          ...
          const latestInspection =
            inspections.length > 0
              ? inspections.sort(
                  (a: DatabaseInspection, b: DatabaseInspection) =>
          ...
          const transformedProperty: PropertyWithStatus = {
            ...
            inspectionStatus: this.determineInspectionStatus(
              property,
              inspections,
            ),
            stats: stats || this.getDefaultStats(),
            lastInspection: latestInspection
              ? this.transformInspectionSummary(latestInspection, property)
              : null,
            nextInspectionDue:
              this.calculateNextInspectionDue(latestInspection),
      ...
      logger.error("Failed to fetch properties with status", {
        error,
        options,
      });
  ...
  async getAvailableProperties(
    ...
    const startTime = performance.now();
    const cacheKey =
      PROPERTY_SERVICE_CONFIG.cacheKeys.availableProperties(inspectorId);
    ...
    try {
      ...
      const { data: properties, error } = await supabase
        ...
        .or(
          "inspections.status.is.null,inspections.status.in.(completed,cancelled)",
        )
        .limit(
          filters.limit || PROPERTY_SERVICE_CONFIG.performance.maxSearchResults,
        );
      ...
      logger.error("Failed to fetch available properties", {
        error,
        inspectorId,
        filters,
      });
  ...
  async searchProperties(
    ...
    const startTime = performance.now();
    const cacheKey = PROPERTY_SERVICE_CONFIG.cacheKeys.propertySearch(
      searchQuery + JSON.stringify(options),
    );
    ...
    try {
      ...
      query = query.limit(
        options.limit || PROPERTY_SERVICE_CONFIG.performance.maxSearchResults,
      );
      ...
      let searchResults = data.map((property) =>
        this.transformToPropertyWithStatus(property),
      );

      // Apply relevance ranking
      searchResults = this.rankSearchResults(
        searchResults,
        searchQuery,
        options.sortBy,
      );
  ...
  async getPropertyDetails(
    ...
    const cacheKey =
      PROPERTY_SERVICE_CONFIG.cacheKeys.propertyDetails(normalizedId);
    ...
    try {
      ...
      const propertyDetails: PropertyDetails = {
        ...
        inspectionHistory: this.transformInspectionHistory(
          (
            property as DatabaseProperty & {
              inspections?: DatabaseInspection[];
            }
        ...
        compliance: await this.calculatePropertyCompliance(
          property,
          (
            property as DatabaseProperty & {
              inspections?: DatabaseInspection[];
  ...
  async getPropertyStats(
    ...
    const cacheKey = PROPERTY_SERVICE_CONFIG.cacheKeys.propertyStats(
      normalizedId,
      timeKey,
    );
    ...
    try {
      ...
      logger.error("Failed to calculate property stats", {
        error,
        propertyId,
        timeRange,
      });
  ...
  private transformPropertyAddress(
  ...
  private calculateNextInspectionDue(
    lastInspection: DatabaseInspection | null,
  ): Date | null {
  ...
  private transformInspectionSummary(
  ...
  private isPropertyAvailable(
    property: DatabaseProperty & { inspections?: DatabaseInspection[] },
  ): boolean {
  ...
  private getLastInspectionDate(property: DatabaseProperty): Date | null {
    return property.last_inspection_date
      ? new Date(property.last_inspection_date)
      : null;
  }

  private calculatePropertyUrgency(
    property: DatabaseProperty,
  ...
  private transformToPropertyWithStatus(
    property: DatabaseProperty & { inspections?: DatabaseInspection[] },
  ): PropertyWithStatus {
    const inspections = property.inspections || [];
  ...
  private transformInspectionHistory(
    inspections: DatabaseInspection[],
  ...
  private async calculatePropertyCompliance(
    property: DatabaseProperty,
    inspections: DatabaseInspection[],
