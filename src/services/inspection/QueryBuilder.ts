...
export class QueryBuilder<T = Record<string, unknown>> {
   ...
   */
  static activeInspections(
    inspectorId?: string,
  ): QueryBuilder<DatabaseInspection> {
   ...
   */
  async withCount(): Promise<
    DatabaseQueryResult<T[]> & { totalCount: number }
  > {
 ...
 * @returns Configured QueryBuilder
 */
export function queryInspections(
  options?: QueryBuilderOptions,
): QueryBuilder<DatabaseInspection> {
 ...
 * @returns Configured QueryBuilder
 */
export function queryStaticSafetyItems(
  options?: QueryBuilderOptions,
): QueryBuilder<DatabaseStaticSafetyItem> {
...
export function analyzeQueryPerformance(metrics: QueryMetrics[]): {
  ...
  if (slowQueries.length > metrics.length * 0.1) {
    suggestions.push(
      "High percentage of slow queries - review query complexity",
    );
  }
