...
export interface DatabaseProperty {
  ...
  inspector_status: 'assigned' | 'in_progress' | 'completed' | 'unavailable' | null;
  ...
  audit_status: 'pending' | 'in_progress' | 'completed' | 'failed' | 'cancelled' | null;
...
export function isDatabaseError(error: any): error is DatabaseError {
  return error && typeof error.code === 'string' && error.code in DatabaseErrorCode;
}

/**
 * Type guard for successful query results
 */
export function isSuccessfulQuery<T>(result: DatabaseQueryResult<T>): result is DatabaseQueryResult<T> & { data: T } {
