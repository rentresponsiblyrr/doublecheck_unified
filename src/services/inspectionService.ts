...
export class InspectionService {
   ...
   */
  async createInspection(data: CreateInspectionData): Promise<{
    success: boolean;
    data?: InspectionWithDetails;
    error?: string;
  }> {
    try {
      logger.info(
        "Creating inspection with enterprise service",
        {
      ...
      const request: InspectionCreationRequest = {
        ...
        inspectorId: data.inspectorId
          ? createInspectorId(data.inspectorId)
          : undefined,
        status: "draft",
      };
      ...
      logger.info(
        "Enterprise inspection created successfully",
        {
          inspectionId,
          propertyId: data.propertyId,
      ...
      if (!fullInspection.success) {
        logger.warn(
          "Inspection created but failed to fetch details",
          { inspectionId },
          "INSPECTION_SERVICE",
        );
      ...
      logger.info(
        "Successfully created and fetched inspection",
        {
          inspectionId,
      ...
      logger.error(
        "Unexpected error in inspection creation",
        error,
        "INSPECTION_SERVICE",
      );
      return {
   ...
   */
  async getInspectionById(inspectionId: string): Promise<{
    success: boolean;
    data?: InspectionWithDetails;
    error?: string;
  }> {
    try {
      ...
      logger.error(
        "Unexpected error fetching inspection",
        error,
        "INSPECTION_SERVICE",
      );
   ...
   */
  async updateInspectionProgress(
    update: UpdateInspectionProgress,
  ): Promise<{ success: boolean; error?: string }> {
    try {
      logger.info(
        "Updating inspection progress",
        {
      ...
      logger.info(
        "Successfully updated inspection progress",
        { inspectionId: update.inspectionId },
        "INSPECTION_SERVICE",
      );
      return { success: true };
      ...
      logger.error(
        "Unexpected error updating inspection",
        error,
        "INSPECTION_SERVICE",
      );
      return { success: false, error: "Unexpected error occurred" };
   ...
   */
  async getInspectionsByProperty(propertyId: string): Promise<{
    success: boolean;
    data?: InspectionWithDetails[];
    error?: string;
  }> {
    try {
      logger.info(
        "Fetching inspections for property",
        { propertyId },
        "INSPECTION_SERVICE",
      );
      ...
      if (error) {
        logger.error(
          "Failed to fetch property inspections",
          error,
          "INSPECTION_SERVICE",
        );
        return { success: false, error: error.message };
      ...
      logger.error(
        "Unexpected error fetching property inspections",
        error,
        "INSPECTION_SERVICE",
      );
      return { success: false, error: "Unexpected error occurred" };
   ...
   */
  async getInspectionsForReview(): Promise<{
    success: boolean;
    data?: InspectionWithDetails[];
    error?: string;
  }> {
    try {
      ...
      logger.error(
        "Unexpected error fetching inspections for review",
        error,
        "INSPECTION_SERVICE",
      );
      return { success: false, error: "Unexpected error occurred" };
  ...
  async updateCertificationStatus(
    ...
    try {
      ...
      logger.info(
        "Successfully updated certification status",
        { inspectionId, status },
        "INSPECTION_SERVICE",
      );
      return { success: true };
      ...
      logger.error(
        "Unexpected error updating certification status",
        error,
        "INSPECTION_SERVICE",
      );
      return { success: false, error: "Unexpected error occurred" };
