...
export class MobileInspectionService {
  static async getOrCreateInspection(
    propertyId: string,
  ): Promise<MobileInspectionResult> {
    log.info(
      "Starting mobile inspection flow for property",
      {
    ...
    const hasAccess =
      await InspectionValidationService.validatePropertyAccess(propertyId);
    if (!hasAccess) {
      throw new Error(
    ...
    const activeInspectionId =
      await InspectionCreationOptimizer.findActiveInspectionSecure(propertyId);
    if (activeInspectionId) {
      log.info(
        "Joining existing inspection",
        {
          component: "MobileInspectionService",
          action: "getOrCreateInspection",
          propertyId,
    ...
    log.info(
      "Creating new inspection for property",
      {
        component: "MobileInspectionService",
        action: "getOrCreateInspection",
        propertyId,
    ...
    const newInspectionId =
      await InspectionCreationOptimizer.createInspectionWithRetry(propertyId);
  ...
  static async assignInspector(inspectionId: string): Promise<void> {
    return InspectionCreationOptimizer.assignInspectorToInspection(
      inspectionId,
    );
  }
}
