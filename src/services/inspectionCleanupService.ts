...
export class InspectionCleanupService {
  ...
  static async analyzeDuplicateInspections(): Promise<CleanupSummary> {
    ...
    inspections?.forEach((inspection) => {
      ...
      const completedItems =
        inspection.logs?.filter((item) => item.status === "completed").length ||
        0;
   ...
   */
  private static analyzePropertyInspections(
    ...
    ].includes(mostRecent.status as any);
    ...
    if (isActiveStatus || hasProgress) {
      ...
      sorted.slice(1).forEach((inspection) => {
        const isEarlyStage = [INSPECTION_STATUS.DRAFT, "available"].includes(
          inspection.status,
        );
      ...
      const withProgress = sorted.filter(
        (inspection) => inspection.completed_checklist_items > 0,
      );

      if (withProgress.length > 0) {
        // Keep the one with most progress
        const mostProgress = withProgress.reduce((best, current) =>
          current.completed_checklist_items > best.completed_checklist_items
            ? current
            : best,
        );
        ...
        sorted
          .filter(
            (i) =>
              i.id !== mostProgress.id && i.completed_checklist_items === 0,
          )
          .forEach((inspection) => {
            inspection.shouldKeep = false;
        ...
        sorted
          .filter(
            (i) => i.id !== mostProgress.id && i.completed_checklist_items > 0,
          )
          .forEach((inspection) => {
            inspection.shouldKeep = true;
   ...
   */
  static async cleanupDuplicateInspections(
    inspectionsToDelete: string[],
  ): Promise<void> {
    ...
    if (checklistError) {
      throw new Error(
        `Failed to delete inspection checklist items: ${checklistError.message}`,
      );
    }
    ...
    if (inspectionError) {
      throw new Error(
        `Failed to delete inspections: ${inspectionError.message}`,
      );
    }
  }
