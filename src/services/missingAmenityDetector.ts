...
export interface MissingAmenityAlert {
  ...
  recommendation: {
    action: 'add_to_amenities' | 'add_to_description' | 'feature_more_prominently';
...
export class MissingAmenityDetector {
  private readonly DETECTION_RULES: AmenityDetectionRule[] = [
    {
      ...
      detectionCriteria: {
        ...
        supportingKeywords: ['chlorine', 'diving', 'deck', 'poolside', 'swimming'],
        photoIndicators: ['pool', 'water', 'swimming', 'pool deck'],
        excludeIfPresent: ['no pool', 'shared pool']
      },
      commonMissedVariations: ['pool', 'swimming pool', 'private pool', 'outdoor pool'],
      marketImpact: 'high',
      recommendationTemplate: 'Add "Swimming Pool" to your amenities list and mention it prominently in your description. Pools are a major booking factor for vacation rentals.'
    },
    {
      name: 'Hot Tub',
      category: 'high_value',
      ...
      marketImpact: 'high',
      recommendationTemplate: 'Feature your hot tub/spa in both amenities and description. This is a premium feature that can significantly impact bookings.'
    },
    {
      name: 'Game Room',
      category: 'high_value',
      detectionCriteria: {
        ...
        supportingKeywords: ['entertainment', 'ping pong', 'foosball', 'arcade', 'billiards'],
        photoIndicators: ['pool table', 'games', 'entertainment room', 'rec room'],
        excludeIfPresent: ['no games']
      },
      commonMissedVariations: ['game room', 'games', 'entertainment room', 'rec room'],
      marketImpact: 'high',
      recommendationTemplate: 'Highlight your game room/entertainment space. Families and groups specifically search for properties with entertainment amenities.'
    },
    {
      name: 'Home Theater',
      category: 'high_value',
      detectionCriteria: {
        ...
        supportingKeywords: ['cinema', 'screening', 'surround sound', 'media room'],
        photoIndicators: ['projector', 'movie screen', 'theater', 'cinema'],
        excludeIfPresent: ['no theater']
      },
      commonMissedVariations: ['home theater', 'movie room', 'cinema', 'media room'],
      marketImpact: 'high',
      recommendationTemplate: 'Feature your home theater/movie room prominently. This is a luxury amenity that appeals to families and entertainment seekers.'
    },
    {
      ...
      detectionCriteria: {
        ...
        supportingKeywords: ['outdoor cooking', 'patio kitchen', 'outdoor grill', 'barbecue'],
        photoIndicators: ['outdoor grill', 'bbq', 'outdoor kitchen', 'patio cooking'],
        excludeIfPresent: ['no grill', 'no outdoor cooking']
      },
      commonMissedVariations: ['outdoor kitchen', 'grill', 'bbq', 'outdoor cooking'],
      marketImpact: 'medium',
      recommendationTemplate: 'Mention your outdoor kitchen/grilling area. Many guests look for properties where they can cook outdoors.'
    },
    {
      name: 'Fire Pit',
      category: 'standard',
      detectionCriteria: {
        ...
        supportingKeywords: ['outdoor fire', 'campfire', 'fire bowl', 'gathering'],
        photoIndicators: ['fire pit', 'outdoor fire', 'campfire', 'fire bowl'],
        excludeIfPresent: ['no fire pit', 'fire not allowed']
      },
      commonMissedVariations: ['fire pit', 'outdoor fireplace', 'fire bowl', 'campfire area'],
      marketImpact: 'medium',
      recommendationTemplate: 'Add your fire pit to the amenities and description. Great for creating memorable evening experiences for guests.'
    },
    {
      ...
      marketImpact: 'medium',
      recommendationTemplate: 'Highlight your workspace/office area. Many travelers need a dedicated work space for remote work or business travel.'
    },
    {
      name: 'Balcony/Patio',
      ...
      detectionCriteria: {
        ...
        supportingKeywords: ['outdoor seating', 'terrace', 'outdoor space', 'views'],
      ...
      marketImpact: 'low',
      recommendationTemplate: 'Mention your balcony/patio space. Outdoor access is valued by many guests, especially for morning coffee or evening relaxation.'
    },
    {
      ...
      commonMissedVariations: ['laundry', 'washer/dryer', 'washing machine', 'laundry facilities'],
      marketImpact: 'low',
      recommendationTemplate: 'Add laundry facilities to your amenities list. This is especially important for longer stays and families with children.'
    },
    {
      ...
      commonMissedVariations: ['gym', 'fitness equipment', 'workout room', 'exercise equipment'],
      marketImpact: 'medium',
      recommendationTemplate: 'Feature your fitness/gym equipment. Health-conscious travelers often specifically search for properties with exercise amenities.'
    }
  ];
  ...
  async detectMissingAmenities(
    ...
    try {
      logger.info('Starting missing amenity detection', { 
      ...
      for (const rule of this.DETECTION_RULES) {
        const alert = await this.analyzeDetectionRule(rule, inspection, scrapedData);
      ...
      const summary = {
        totalMissing: criticalMissing.length + underUtilized.length + opportunityAmenities.length,
        highValueMissing: criticalMissing.filter(a => a.category === 'high_value').length,
        actionableRecommendations: criticalMissing.length + underUtilized.length,
        estimatedListingImprovement: this.calculateListingImprovement(criticalMissing, underUtilized)
      };

      logger.info('Missing amenity detection completed', {
      ...
      logger.error('Failed to detect missing amenities', error, 'MISSING_AMENITY_DETECTOR');
      throw error;
    }
  }
  ...
  private async analyzeDetectionRule(
    ...
    const relevantItems = inspection.checklist_items.filter(item => {
      ...
      const hasRequiredKeyword = rule.detectionCriteria.mustHaveKeywords.some(keyword => 
        itemText.includes(keyword.toLowerCase())
      );
      
      // Should not have excluding keywords
      const hasExcludingKeyword = rule.detectionCriteria.excludeIfPresent.some(keyword => 
        itemText.includes(keyword.toLowerCase())
      );
    ...
    const scrapedText = `${scrapedData.description} ${scrapedData.amenities.map(a => a.name).join(' ')}`.toLowerCase();
    ...
    // No evidence found
    if (evidence.checklistItems.length === 0 && evidence.photoUrls.length === 0) {
      return null;
    }

    // Calculate confidence
    evidence.confidence = this.calculateConfidence(rule, evidence, relevantItems);

    // Generate reasoning
    evidence.reasoning = this.generateDetectionReasoning(rule, evidence, isAlreadyMentioned);

    // Determine recommendation
    const recommendation = this.generateRecommendation(rule, isAlreadyMentioned, isInAmenityList, evidence.confidence);
  ...
  private calculateConfidence(
    ...
    const avgAIConfidence = relevantItems.reduce((sum, item) => sum + (item.ai_confidence || 0), 0) / relevantItems.length;
  ...
  private generateDetectionReasoning(
    ...
    if (evidence.checklistItems.length > 0) {
      reasons.push(`Found ${evidence.checklistItems.length} inspection item(s) related to ${rule.name}`);
    }
    
    if (evidence.photoUrls.length > 0) {
      reasons.push(`Found ${evidence.photoUrls.length} photo(s) showing ${rule.name}`);
    }
    
    if (isAlreadyMentioned) {
      reasons.push(`Mentioned in current listing but may not be prominently featured`);
    } else {
  ...
  private generateRecommendation(
    ...
    let action: 'add_to_amenities' | 'add_to_description' | 'feature_more_prominently';
  ...
  private calculateListingImprovement(
    ...
    const highValueCount = criticalMissing.filter(a => a.category === 'high_value').length;
