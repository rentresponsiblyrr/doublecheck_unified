...
export class AtomicInspectionService {
  ...
  constructor() {
    logger.info('Atomic inspection service initialized', {}, 'ATOMIC_INSPECTION');
  }

  /**
   * Create inspection with atomic transaction guarantees
   */
  public async createInspectionAtomic(request: AtomicInspectionRequest): Promise<AtomicInspectionResult> {
    ...
    try {
      logger.info('Starting atomic inspection creation', {
      ...
      const inspectionData = await this.executeAtomicTransaction(request, context);
      context.stepCompleted++;
      ...
      const checklistItems = await this.populateChecklistItems(inspectionData, request, context);
      context.stepCompleted++;
      ...
      logger.info('Atomic inspection creation completed', {
      ...
      logger.error('Atomic inspection creation failed', {
        error,
        context,
   ...
   */
  private async validateRequest(request: AtomicInspectionRequest, context: TransactionContext): Promise<void> {
    logger.info('Validating inspection request', { step: 1 }, 'ATOMIC_INSPECTION');

    // Validate property ID format
    if (!request.propertyId || typeof request.propertyId !== 'string') {
      throw this.createValidationError('Invalid property ID format', 'PROPERTY_ID_INVALID');
    }

    // Validate inspector ID format  
    if (!request.inspectorId || typeof request.inspectorId !== 'string') {
      throw this.createValidationError('Invalid inspector ID format', 'INSPECTOR_ID_INVALID');
    }
    ...
    await this.checkForActiveInspection(request.propertyId, request.inspectorId);

    logger.info('Request validation completed', { step: 1 }, 'ATOMIC_INSPECTION');
  }
  ...
  private validateChecklistItem(item: ChecklistItemTemplate): void {
    if (!item.title || item.title.trim().length === 0) {
      throw this.createValidationError('Checklist item title is required', 'CHECKLIST_TITLE_REQUIRED');
    }

    if (!item.category || item.category.trim().length === 0) {
      throw this.createValidationError('Checklist item category is required', 'CHECKLIST_CATEGORY_REQUIRED');
    }

    if (!['photo', 'video', 'none'].includes(item.evidenceType)) {
      throw this.createValidationError('Invalid evidence type', 'EVIDENCE_TYPE_INVALID');
    }
  }
  ...
  private async verifyPropertyExists(propertyId: string): Promise<void> {
    ...
    if (!result) {
      throw this.createValidationError('Property not found', 'PROPERTY_NOT_FOUND');
    }
  }
   ...
   * Check for existing active inspection
   */
  private async checkForActiveInspection(propertyId: string, inspectorId: string): Promise<void> {
    const result = await executeWithResilience(
      async () => {
        const { data, error } = await supabase
          .from('inspections')
   ...
   */
  private async verifyUserAuthorization(request: AtomicInspectionRequest, context: TransactionContext): Promise<void> {
    logger.info('Verifying user authorization', { step: 2 }, 'ATOMIC_INSPECTION');

    // Check authentication state
    const sessionState = authGuard.getSessionState();
    if (!sessionState || sessionState.userId !== request.inspectorId) {
      throw this.createAuthorizationError('User not authenticated or ID mismatch', 'AUTH_MISMATCH');
    }
    ...
    if (result.status !== 'active') {
      throw this.createAuthorizationError('User account is not active', 'USER_INACTIVE');
    }

    if (!['inspector', 'admin'].includes(result.role)) {
      throw this.createAuthorizationError('User does not have inspection permissions', 'INSUFFICIENT_PERMISSIONS');
    }

    logger.info('User authorization verified', { step: 2 }, 'ATOMIC_INSPECTION');
  }
  ...
  private async executeAtomicTransaction(
  ...
  ): Promise<{ inspectionId: string; propertyId: string; inspectorId: string; status: string; createdAt: string }> {
    logger.info('Executing atomic transaction', { step: 3 }, 'ATOMIC_INSPECTION');
    ...
    return await executeWithResilience(
      async () => {
        ...
        logger.info('Atomic transaction completed', {
          step: 3,
          inspectionId
        }, 'ATOMIC_INSPECTION');
   ...
   */
  private async initializeMediaStorage(inspectionId: string, context: TransactionContext): Promise<void> {
    logger.info('Initializing media storage', { step: 4, inspectionId }, 'ATOMIC_INSPECTION');
    ...
    try {
      ...
      for (const path of storagePaths) {
        ...
        if (error && !error.message.includes('already exists')) {
          logger.warn('Failed to create storage path', { path, error }, 'ATOMIC_INSPECTION');
        } else {
      ...
      logger.warn('Media storage initialization failed', error, 'ATOMIC_INSPECTION');
  ...
  private async populateChecklistItems(
    ...
    logger.info('Checklist items populated', {
      step: 5,
   ...
   */
  private async finalizeInspection(inspectionId: string, context: TransactionContext): Promise<void> {
    logger.info('Finalizing inspection', { step: 6, inspectionId }, 'ATOMIC_INSPECTION');
    ...
    logger.info('Inspection finalized', { step: 6, inspectionId }, 'ATOMIC_INSPECTION');
  }
  ...
  private async rollbackTransaction(context: TransactionContext): Promise<void> {
    logger.info('Attempting transaction rollback', { context }, 'ATOMIC_INSPECTION');
    ...
    try {
      ...
      logger.error('Transaction rollback failed', rollbackError, 'ATOMIC_INSPECTION');
    }
  }
   ...
   * Create error result
   */
  private createErrorResult(error: any, context: TransactionContext): AtomicInspectionResult {
