...
export class MediaService {
  ...
  async getMediaFile(
    ...
    try {
      ...
      const filePath =
        mediaRecord.file_path || mediaRecord.path || mediaRecord.url || "";
      ...
      logger.info(
        "Successfully fetched media file",
        {
          mediaId,
          type: mediaFile.type,
      ...
      logger.error(
        "Unexpected error fetching media file",
        error,
        "MEDIA_SERVICE",
      );
  ...
  async getMediaFilesForItem(checklistItemId: string): Promise<{
    ...
    try {
      logger.info(
        "Fetching media files for checklist item",
        { checklistItemId },
        "MEDIA_SERVICE",
      );
      ...
      logger.info(
        "Successfully fetched media files",
        {
          checklistItemId,
      ...
      logger.error(
        "Unexpected error fetching media files",
        error,
        "MEDIA_SERVICE",
      );
      return { success: false, error: "Unexpected error occurred" };
  ...
  private async checkFileAccessibility(url: string): Promise<boolean> {
    try {
      ...
      logger.warn(
        "File accessibility check failed",
        { url, error },
        "MEDIA_SERVICE",
      );
      return false;
  ...
  async getOptimizedVideoUrl(
    ...
    logger.info(
      "Getting optimized video URL",
      { originalUrl, targetQuality },
      "MEDIA_SERVICE",
    );
    return originalUrl;
  }
