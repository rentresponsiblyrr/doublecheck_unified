...
class AIIssueClassificationService {
  ...
  async classifyIssue(context: IssueContext): Promise<AIClassificationResult> {
    ...
    try {
      ...
      if (this.cache.has(cacheKey)) {
        logger.info('Using cached AI classification', { cacheKey }, 'AI_CLASSIFICATION');
      ...
      if (!this.openAIApiKey) {
        return this.enhanceWithRuleBasedAnalysis(ruleBasedAnalysis, context, startTime);
      }
   ...
   */
  private performRuleBasedAnalysis(context: IssueContext): Partial<AIClassificationResult> {
    const { bugReport, errorContext } = context;
    
    // Analyze error patterns
    const hasCompatibilityErrors = errorContext.databaseErrors.some(e => e.isCompatibilityLayerIssue);
    const hasNetworkErrors = errorContext.networkErrors.length > 0;
    const hasPerformanceIssues = errorContext.performanceMetrics.some(m => 
    ...
    if (bugReport.description.toLowerCase().includes('request') || 
        bugReport.description.toLowerCase().includes('feature')) {
      issueType = 'feature_request';
    } else if (bugReport.description.toLowerCase().includes('improve') || 
    ...
    if (hasCompatibilityErrors || errorContext.affectedFeatures.length > 2) {
      ...
      complexity = 'complex';
    } else if (errorContext.consoleErrors.length <= 1 && errorContext.networkErrors.length <= 1) {
      complexity = 'simple';
    }
   ...
   */
  private buildAnalysisPrompt(context: IssueContext, ruleBasedAnalysis: Partial<AIClassificationResult>): string {
    const { bugReport, errorContext } = context;
...
## Technical Context
**Console Errors**: ${errorContext.consoleErrors.length} errors (Recent: ${errorContext.consoleErrors.slice(-3).map(e => e.message).join('; ')})
...
5. AI service integration issues`;
  ...
  private async callOpenAI(prompt: string): Promise<any> {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        const response = await fetch(`${this.baseUrl}/chat/completions`, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are an expert software engineer specializing in bug analysis and classification. Always respond with valid JSON.'
              },
              {
                role: 'user',
                content: prompt
        ...
        logger.warn(`OpenAI API attempt ${attempt} failed`, { error, attempt }, 'AI_CLASSIFICATION');
   ...
   * Parse and validate AI response
   */
  private parseAIResponse(aiResponse: string, context: IssueContext, startTime: number): AIClassificationResult {
    try {
      ...
      const result: AIClassificationResult = {
        ...
        requiredSkills: Array.isArray(parsed.requiredSkills) ? parsed.requiredSkills : ['frontend'],
        ...
        recommendedAssignee: parsed.recommendedAssignee || 'Full-stack developer',
        relatedIssues: Array.isArray(parsed.relatedIssues) ? parsed.relatedIssues : [],
        suggestedLabels: Array.isArray(parsed.suggestedLabels) ? parsed.suggestedLabels : ['bug'],
        debuggingHints: Array.isArray(parsed.debuggingHints) ? parsed.debuggingHints : [],
        relevantDocumentation: Array.isArray(parsed.relevantDocumentation) ? parsed.relevantDocumentation : [],
      ...
      logger.error('Failed to parse AI response', { error, aiResponse }, 'AI_CLASSIFICATION');
  ...
  private enhanceWithRuleBasedAnalysis(
    ...
    return {
      ...
      estimatedEffortHours: this.estimateEffort(ruleBasedAnalysis.complexity || 'moderate'),
  ...
  private identifyRootCause(context: IssueContext): string {
    ...
    if (errorContext.networkErrors.some(e => e.isSupabaseCall && e.status >= 500)) {
  ...
  private categorizeRootCause(context: IssueContext): AIClassificationResult['rootCauseCategory'] {
    const { errorContext } = context;
    
    if (errorContext.databaseErrors.length > 0) return 'database';
    if (errorContext.networkErrors.length > 0) return 'backend';
    if (errorContext.performanceMetrics.some(m => m.metrics.largestContentfulPaint && m.metrics.largestContentfulPaint > 4000)) return 'frontend';
    
    return 'frontend';
  }

  private assessBusinessImpact(context: IssueContext): AIClassificationResult['businessImpact'] {
    const { errorContext, bugReport } = context;
    
    if (errorContext.userFrustrationLevel >= 8 || 
        errorContext.affectedFeatures.includes('Authentication') ||
        bugReport.userInfo.userRole === 'admin') {
      return 'high';
    }
    
    if (errorContext.userFrustrationLevel >= 5 || 
  ...
  private assessUserImpact(context: IssueContext): AIClassificationResult['userImpact'] {
    const { errorContext } = context;
  ...
  private estimateEffort(complexity: AIClassificationResult['complexity']): number {
    const effortMap = {
      simple: 2,
      moderate: 8,
      complex: 24,
  ...
  private identifyRequiredSkills(context: IssueContext): string[] {
    ...
    if (errorContext.affectedFeatures.includes('Media Upload')) skills.add('mobile');
  ...
  private recommendAssignee(context: IssueContext): string {
    ...
    if (errorContext.databaseErrors.length > 0) return 'Database/Backend Developer';
    if (errorContext.affectedFeatures.includes('Mobile')) return 'Mobile/Frontend Developer';
  ...
  private generateCacheKey(context: IssueContext): string {
    ...
    const keyData = {
      ...
      errorCount: errorContext.consoleErrors.length + errorContext.networkErrors.length,
  ...
  private extractConfidenceFactors(context: IssueContext, parsed: any): string[] {
    ...
    if (context.errorContext.networkErrors.length > 0) factors.push('network errors available');
    if (context.errorContext.performanceMetrics.length > 0) factors.push('performance data available');
    if (context.bugReport.steps.length > 2) factors.push('detailed reproduction steps');
