...
type EnvironmentalFactorsData = Record<
  string,
...
export class LearningSystem {
  ...
  constructor() {
    ...
    log.warn(
      "LearningSystem: Direct AI integration disabled",
      {
  ...
  async analyzeResolutionOutcome(
    ...
    try {
      ...
      const resolutionLearnings = await this.extractResolutionLearnings(
        errorId,
        resolutionHistory,
      );
      ...
      const recommendations =
        await this.generateImprovementRecommendations(resolutionLearnings);

      // Store recommendations
      this.recommendations.push(...recommendations);
    } catch (error) {
      log.error(
        "Failed to analyze resolution outcome",
        error as Error,
        {
          component: "LearningSystem",
  ...
  async generateModelRecommendations(): Promise<LearningRecommendation[]> {
    try {
      ...
      const recommendations =
        await this.generateAIRecommendations(performanceAnalysis);
      ...
      log.error(
        "Failed to generate model recommendations",
        error as Error,
        {
          component: "LearningSystem",
  ...
  private async calculateAccuracy(
    ...
    switch (category) {
      case "classification":
        overall = this.calculateClassificationAccuracy(
          aiPrediction,
          actualOutcome,
        );
      ...
      case "reproduction":
        overall = this.calculateReproductionAccuracy(
          aiPrediction,
          actualOutcome,
        );
        categorySpecific = overall;
   ...
   */
  private calculateRootCauseAccuracy(
    aiPrediction: AIResult,
    actualOutcome: AIResult,
  ): number {
    ...
    const intersection = new Set(
      [...predictedWords].filter((x) => actualWords.has(x)),
    );

    const similarity =
      intersection.size / Math.max(predictedWords.size, actualWords.size);
    return similarity;
  }
   ...
   */
  private calculateReproductionAccuracy(
    aiPrediction: AIResult,
    actualOutcome: AIResult,
  ): number {
    ...
    const completeness =
      aiPrediction.steps.length > 5 ? 1 : aiPrediction.steps.length / 5;
   ...
   */
  private calculateHealingAccuracy(
    aiPrediction: AIResult,
    actualOutcome: AIResult,
  ): number {
...
`;
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an AI learning analyst. Provide specific, actionable insights about AI performance. Respond with valid JSON only.",
          },
          {
            role: "user",
            content: prompt,
      ...
      log.error(
        "Failed to generate insights from AI prediction analysis",
        error as Error,
        {
          component: "LearningSystem",
  ...
  private async analyzeLearningData(category: string): Promise<void> {
    ...
    if (metrics.metrics.accuracy < 0.8) {
      const recommendation = await this.generateCategoryRecommendation(
        category,
        metrics,
      );
  ...
  private calculatePerformanceMetrics(
    ...
    const accuracy =
      recent.reduce((sum, d) => sum + d.accuracy.overall, 0) / recent.length;
    ...
    return {
      ...
      metrics: {
        ...
        confidenceCalibration:
          recent.reduce((sum, d) => sum + d.accuracy.confidenceCalibration, 0) /
          recent.length,
        avgResponseTime,
      },
      trends: {
        accuracyTrend: this.calculateTrend(
          recent.map((d) => d.accuracy.overall),
        ),
        volumeTrend: "stable",
   ...
   */
  private identifyTopErrors(
    data: LearningDataPoint[],
  ): ModelPerformanceMetrics["topErrors"] {
    const errorMap = new Map<string, { count: number; accuracySum: number }>();

    data.forEach((d) => {
      if (d.accuracy.overall < 0.7) {
        // Consider as error if accuracy < 70%
  ...
  private async storeLearningData(dataPoint: LearningDataPoint): Promise<void> {
    try {
      ...
      log.error(
        "Failed to store learning data in database",
        error as Error,
        {
          component: "LearningSystem",
  ...
  private async storeUserFeedback(feedback: UserFeedback): Promise<void> {
    try {
      ...
      log.error(
        "Failed to store user feedback in database",
        error as Error,
        {
          component: "LearningSystem",
  ...
  private async extractResolutionLearnings(
    ...
    return {
      ...
      resolutionTime: resolutionHistory.timeline.closed
        ? new Date(resolutionHistory.timeline.closed).getTime() -
          new Date(resolutionHistory.timeline.reported).getTime()
        : null,
  ...
  private async updatePerformanceMetrics(
    ...
    log.info(
      "Updating performance metrics with new learnings",
      {
        component: "LearningSystem",
  ...
  private async generateImprovementRecommendations(
    ...
    if (learnings.resolutionTime > 3600000) {
      // > 1 hour
      recommendations.push({
        ...
        technicalDetails:
          "Review and streamline prompts to reduce complexity while maintaining accuracy",
        expectedImpact: {
  ...
  private async analyzeOverallPerformance(): Promise<OverallPerformanceAnalysis> {
    ...
    return {
      avgAccuracy:
        ...
        allMetrics.length,
      avgResponseTime:
        allMetrics.reduce((sum, m) => sum + m.metrics.avgResponseTime, 0) /
        allMetrics.length,
      categoriesNeedingImprovement: allMetrics
        .filter((m) => m.metrics.accuracy < 0.8)
        .map((m) => m.category),
  ...
  private calculateOverallTrend(metrics: ModelPerformanceMetrics[]): string {
    const improvingCount = metrics.filter(
      (m) => m.trends.accuracyTrend === "improving",
    ).length;
    const degradingCount = metrics.filter(
      (m) => m.trends.accuracyTrend === "degrading",
    ).length;
  ...
  private async generateAIRecommendations(
    ...
    if (performanceAnalysis.avgAccuracy < 0.8) {
      recommendations.push({
        ...
        priority: "high",
        description:
          "Retrain models with recent learning data to improve accuracy",
        technicalDetails:
          "Use accumulated learning data to fine-tune model parameters",
        expectedImpact: {
  ...
  private prioritizeRecommendations(
    ...
    return recommendations.sort((a, b) => {
      ...
      const scoreA =
        a.expectedImpact.accuracyImprovement *
        this.getEffortMultiplier(a.expectedImpact.implementationEffort);
      const scoreB =
        b.expectedImpact.accuracyImprovement *
        this.getEffortMultiplier(b.expectedImpact.implementationEffort);
  ...
  private async analyzeFeedback(feedback: UserFeedback): Promise<void> {
    ...
    log.info(
      "Analyzing user feedback for insights",
      {
        ...
        errorId: feedback.errorId,
        overallRating:
          Object.values(feedback.feedback).reduce((sum, val) => sum + val, 0) /
          Object.values(feedback.feedback).length,
  ...
  getLearningStatus(): {
    ...
    const totalDataPoints = Array.from(this.learningData.values()).reduce(
