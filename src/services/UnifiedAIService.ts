...
class OpenAIProvider extends BaseAIProvider {
  ...
  async execute<T extends AIRequest>(request: T): Promise<AIResponse<T>> {
    return this.trackOperation("execute", async () => {
      ...
      try {
        switch (request.type) {
          case "photo_analysis":
            result = await this.executePhotoAnalysis(
              request as PhotoAnalysisRequest,
            );
            break;
          case "issue_classification":
            result = await this.executeIssueClassification(
              request as IssueClassificationRequest,
            );
            break;
          case "text_generation":
            result = await this.executeTextGeneration(
              request as TextGenerationRequest,
            );
            break;
          case "code_review":
  ...
  private async executePhotoAnalysis(
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert property inspector analyzing photos for vacation rental compliance.",
          },
          {
            role: "user",
            content: [
  ...
  private async executeIssueClassification(
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert software engineer analyzing bug reports and issues for STR Certified platform.",
          },
          {
            role: "user",
    ...
    return {
      data: this.parseIssueClassificationResponse(
  ...
  private parseIssueClassificationResponse(
    content: string,
...
export class UnifiedAIService {
  ...
  async analyzePhoto(
    request: PhotoAnalysisRequest,
  ): Promise<AIResponse<PhotoAnalysisRequest>> {
  ...
  async classifyIssue(
    request: IssueClassificationRequest,
  ): Promise<AIResponse<IssueClassificationRequest>> {
  ...
  async generateText(
    request: TextGenerationRequest,
  ): Promise<AIResponse<TextGenerationRequest>> {
  ...
  ): Promise<AIResponse<CodeReviewRequest>> {
  ...
  async recordFeedback(feedback: AuditorFeedback): Promise<void> {
    ...
    const learningRecord = this.learningData.find(
      (record) =>
  ...
  async getMetrics(): Promise<AIMetrics> {
    ...
    const withFeedback = this.learningData.filter(
      (d) => d.auditorFeedback,
    ).length;
    ...
    for (const provider of this.providers.keys()) {
      const providerData = this.learningData.filter(
        (d) => d.provider === provider && d.auditorFeedback,
      );
    ...
    return {
      accuracy: {
        overall:
          withFeedback > 0
            ? Array.from(accuracyByProvider.values()).reduce(
                (a, b) => a + b,
                0,
              ) / accuracyByProvider.size
            : 0,
      ...
      performance: {
        ...
        successRate:
          ...
          Math.max(serviceMetrics.totalRequests, 1),
  ...
  private calculateAccuracy(
    prediction: AIResponseData,
    actual: AIResponseData,
  ): number {
  ...
  private calculateTotalTokens(): number {
    return this.learningData
      ...
      .reduce(
        (sum, d) =>
          sum +
          (d.aiPrediction.tokens?.input || 0) +
          (d.aiPrediction.tokens?.output || 0),
        0,
      );
  }
  ...
  private calculateImprovementRate(): number {
    ...
    const recentAccuracy =
      recentData
        .filter((d) => d.auditorFeedback)
        .reduce(
          (sum, d) =>
            sum + this.calculateAccuracy(d.aiPrediction, d.actualOutcome),
          0,
        ) / recentData.length;
  ...
  private calculateConfidenceCalibration(): number {
    ...
    for (const data of dataWithFeedback) {
      ...
      const actualAccuracy = this.calculateAccuracy(
        data.aiPrediction,
        data.actualOutcome,
      );
