...
export class AuthenticationGuard {
    ...
    try {
      this.sessionState = {
        ...
        inspectionInProgress:
          useInspectionStore.getState().inspectionId || undefined,
      };

      const refreshTime =
        this.sessionState.tokenExpiry.getTime() - Date.now() - 5 * 60 * 1000;
      if (refreshTime > 0) {
  ...
  private async refreshSession(): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.warn(
          "Session refresh failed, attempting recovery",
          { error: error.message },
          "AUTH_GUARD",
        );
  ...
  private async recoverFromStoredSession(): Promise<AuthRecoveryResult> {
    try {
      ...
      logger.error(
        "Failed to recover from stored session",
        error,
        "AUTH_GUARD",
      );
      return {
  ...
  private async enableOfflineMode(): Promise<AuthRecoveryResult> {
    try {
      ...
      const preservedState = {
        ...
        photosCaptured: inspectionState.photosCaptured.filter(
      ...
      logger.info(
        ...
        { preservedState: !!preservedState.inspectionId },
        "AUTH_GUARD",
      );
  ...
  private async preserveWorkflowState(): Promise<void> {
    try {
      ...
      localStorage.setItem(
        "str_workflow_recovery",
        JSON.stringify(workflowState),
      );

      logger.info(
        "Workflow state preserved for recovery",
        {
  ...
  private monitorSession(): void {
    ...
    try {
      ...
      const timeUntilExpiry =
        this.sessionState.tokenExpiry.getTime() - now.getTime();
