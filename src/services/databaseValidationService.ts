...
export class DatabaseValidationService {
  ...
  static async validateInspectionCreationSetup(): Promise<{
    ...
    if (!rpcTest.exists) {
      issues.push(
        "create_inspection_compatibility RPC function does not exist",
      );
      recommendations.push(
        "Create the RPC function in the database or rely on direct inserts",
      );
    }
    ...
    if (!auditTest.accessible) {
      issues.push(
        `Checklist operations audit table not accessible: ${auditTest.error}`,
      );
      recommendations.push(
        "Create checklist_operations_audit table for proper error tracking",
      );
    }
