...
export class ReportService {
  ...
  async generateInspectionReport(
    ...
    try {
      logger.info(
        "Starting inspection report generation",
        { inspectionId, options },
        "REPORT_SERVICE",
      );
      ...
      const inspectionResult =
        await auditorService.getInspectionForReview(inspectionId);
  ...
  private async generatePDFReport(
    ...
    // Add header
    if (options.branding) {
      yPosition = this.addReportHeader(
        pdf,
        yPosition,
        reportData,
        pageWidth,
    ...
    if (options.includeSummary) {
      yPosition = await this.addSummarySection(
        pdf,
        yPosition,
        reportData,
        pageWidth,
    ...
    yPosition = this.addPropertySection(
      pdf,
      yPosition,
      reportData,
      pageWidth,
    ...
    yPosition = await this.addInspectionDetailsSection(
      pdf,
      yPosition,
      reportData,
      pageWidth,
    ...
    yPosition = await this.addChecklistSection(
      pdf,
      yPosition,
      reportData,
      pageWidth,
    ...
    if (options.includeListingOptimization) {
      yPosition = await this.addListingOptimizationSection(
        pdf,
        yPosition,
        reportData,
        pageWidth,
    ...
    if (options.includeAuditTrail) {
      yPosition = await this.addAuditTrailSection(
        pdf,
        yPosition,
        reportData,
        pageWidth,
  ...
  private addReportHeader(
    ...
    pdf.text(this.COMPANY_SUBTITLE, pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 20;
    ...
    pdf.text("Property Inspection Report", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 15;
  ...
  private async addSummarySection(
    ...
    if (summary) {
      ...
      // Confidence rating
      pdf.text(
        `Average AI Confidence: ${summary.confidence_average}%`,
        margin,
        yPosition,
      );
      yPosition += 15;
  ...
  private addPropertySection(
    ...
    // Inspection details
    pdf.text(
      `Inspector: ${inspection.users.name} (${inspection.users.email})`,
      margin,
      yPosition,
    );
    yPosition += 8;
    pdf.text(
      `Inspection Date: ${new Date(inspection.start_time).toLocaleDateString()}`,
      margin,
      yPosition,
    );
    yPosition += 8;
    ...
    if (inspection.end_time) {
      const duration = Math.round(
        (new Date(inspection.end_time).getTime() -
          new Date(inspection.start_time).getTime()) /
          (1000 * 60),
      );
  ...
  private async addInspectionDetailsSection(
    ...
    categories.forEach((category) => {
      ...
      const failed = categoryItems.filter(
      ...
      pdf.text(
        `${completed}/${categoryItems.length} completed`,
        margin + 60,
        yPosition,
      );
  ...
  private async addChecklistSection(
    ...
    for (const item of items) {
      ...
      if (item.media && item.media.length > 0) {
        ...
        pdf.text(
          `Media: ${photoCount} photos, ${videoCount} videos`,
          margin,
          yPosition,
        );
        yPosition += 8;
  ...
  async generatePhotoComparisonReport(
    ...
    try {
      logger.info(
        "Generating photo comparison report",
        { inspectionId, comparisons: comparisons.length },
        "REPORT_SERVICE",
      );
      ...
      pdf.setFontSize(18);
      pdf.setTextColor(17, 24, 39);
      pdf.text("Photo Comparison Report", pageWidth / 2, yPosition, {
        align: "center",
      });
      yPosition += 30;
      ...
      for (const comparison of comparisons) {
        ...
        pdf.setTextColor(75, 85, 99);
        pdf.text(
          `AI Score: ${comparison.aiAnalysis.score}% (Confidence: ${comparison.aiAnalysis.confidence}%)`,
          margin,
          yPosition,
        );
        yPosition += 10;
        ...
        pdf.setFontSize(10);
        pdf.text(
          `Analysis: ${comparison.aiAnalysis.reasoning}`,
          margin,
          yPosition,
        );
        yPosition += 8;
        ...
        if (comparison.aiAnalysis.issues.length > 0) {
          pdf.text(
            `Issues: ${comparison.aiAnalysis.issues.join(", ")}`,
            margin,
            yPosition,
          );
          yPosition += 8;
        ...
        if (comparison.auditorFeedback) {
          pdf.text(
            `Auditor Feedback: ${comparison.auditorFeedback}`,
            margin,
            yPosition,
          );
          yPosition += 8;
      ...
      logger.error(
        "Failed to generate photo comparison report",
        error,
        "REPORT_SERVICE",
      );
      return {
  ...
  private async addListingOptimizationSection(
    ...
    try {
      ...
      const mockScrapedData = {
        description:
          reportData.inspection.properties.name + " property description",
        amenities: [],
        photos: [],
        rooms: [],
        specifications: {
      ...
      const optimizationReport =
        await amenityComparisonEngine.generateOptimizationReport(
      ...
      pdf.setTextColor(75, 85, 99);
      pdf.text(
        `Total opportunities identified: ${optimizationReport.summary.totalOpportunities}`,
        margin,
        yPosition,
      );
      yPosition += 6;
      pdf.text(
        `High priority actions: ${optimizationReport.summary.highPriorityActions}`,
        margin,
        yPosition,
      );
      yPosition += 6;
      pdf.text(
        `Impact: ${optimizationReport.summary.estimatedImpact}`,
        margin,
        yPosition,
      );
      yPosition += 15;
      ...
      if (
        optimizationReport.discoveredOpportunities.criticalMissing.length > 0
      ) {
        pdf.setFontSize(12);
        ...
        optimizationReport.discoveredOpportunities.criticalMissing.forEach(
          (missing) => {
            if (yPosition > pageHeight - 40) {
              pdf.addPage();
              yPosition = margin;
      ...
      if (optimizationReport.recommendations.amenityCheckboxes.length > 0) {
        ...
        optimizationReport.recommendations.amenityCheckboxes
          .slice(0, 5)
          .forEach((checkbox) => {
            if (yPosition > pageHeight - 40) {
              pdf.addPage();
      ...
      if (optimizationReport.recommendations.descriptionUpdates.length > 0) {
        ...
        optimizationReport.recommendations.descriptionUpdates
          .slice(0, 3)
          .forEach((update) => {
            if (yPosition > pageHeight - 60) {
              pdf.addPage();
      ...
      pdf.setTextColor(107, 114, 128);
      const disclaimer =
        "These suggestions are based on our inspection findings compared to your current listing. Implementing these changes may help improve your listing's appeal and discoverability.";
      ...
      logger.error(
        "Failed to add listing optimization section",
        error,
        "REPORT_SERVICE",
      );
      ...
      pdf.setTextColor(107, 114, 128);
      pdf.text(
        "Analysis temporarily unavailable. Please contact support.",
        margin,
        yPosition,
      );
      yPosition += 20;
