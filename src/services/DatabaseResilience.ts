...
export class DatabaseResilience {
   ...
   */
  public async executeWithResilience<T>(operation: DatabaseOperation<T>): Promise<T> {
    ...
    try {
      ...
      if (this.circuitBreaker.state === 'open') {
        if (this.shouldAttemptReset()) {
          ...
          logger.info('Circuit breaker moving to half-open state', {}, 'DB_RESILIENCE');
        } else {
      ...
      if (operation.fallback) {
        try {
          logger.info('Attempting fallback operation', { operation: operation.operationName }, 'DB_RESILIENCE');
          const fallbackResult = await operation.fallback();
          this.recordSuccess(Date.now() - startTime);
          return fallbackResult;
        } catch (fallbackError) {
          logger.error('Fallback operation failed', fallbackError, 'DB_RESILIENCE');
        }
      }
  ...
  private classifyError(error: Error | { message?: string; code?: string | number; status?: string | number }): DatabaseError {
    ...
    if (code === 400 || message.includes('syntax') || message.includes('constraint')) {
      return {
        type: 'query',
  ...
  private recordSuccess(responseTime: number): void {
    ...
    if (this.circuitBreaker.state === 'half-open') {
      ...
      if (this.circuitBreaker.successCount >= this.halfOpenSuccessThreshold) {
        ...
        logger.info('Circuit breaker closed - connection restored', {}, 'DB_RESILIENCE');
      }
  ...
  private recordFailure(error: Error, responseTime: number): void {
    ...
    if (this.circuitBreaker.state === 'half-open') {
      ...
      this.circuitBreaker.successCount = 0;
      this.circuitBreaker.nextAttemptTime = new Date(Date.now() + this.recoveryTimeout);
      logger.warn('Circuit breaker opened after half-open failure', {}, 'DB_RESILIENCE');
    } else if (this.circuitBreaker.state === 'closed' && 
  ...
  private createCircuitBreakerError(): Error {
    ...
    const waitTime = nextAttempt ? Math.ceil((nextAttempt.getTime() - Date.now()) / 1000) : 0;
  ...
  private async exponentialBackoffWithJitter(attempt: number): Promise<void> {
    ...
    logger.info('Waiting before retry', { attempt, delay: Math.round(delay) }, 'DB_RESILIENCE');
  ...
  private updateConnectionHealth(): void {
    ...
    const recentFailureRate = this.metrics.failedOperations / Math.max(recentOperations, 1);
    ...
    if (health !== this.metrics.connectionHealth) {
      logger.info('Database connection health changed', {
