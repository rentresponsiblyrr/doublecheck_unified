...
export const createPropertyId = (id: string): PropertyId => {
  ...
  if (!result.success) {
    throw new ValidationError('Invalid PropertyId', { id, errors: result.error.errors });
  }
...
export const createInspectionId = (id: string): InspectionId => {
  ...
  if (!result.success) {
    throw new ValidationError('Invalid InspectionId', { id, errors: result.error.errors });
  }
...
export const createUserId = (id: string): UserId => {
  ...
  if (!result.success) {
    throw new ValidationError('Invalid UserId', { id, errors: result.error.errors });
  }
  return id as UserId;
};
...
export const createChecklistItemId = (id: string): ChecklistItemId => {
  ...
  if (!result.success) {
    throw new ValidationError('Invalid ChecklistItemId', { id, errors: result.error.errors });
  }
...
export const createStaticSafetyItemId = (id: string): StaticSafetyItemId => {
  ...
  if (!result.success) {
    throw new ValidationError('Invalid StaticSafetyItemId', { id, errors: result.error.errors });
  }
...
export interface EnhancedServiceError {
  type: 'validation' | 'database' | 'business_logic' | 'concurrency' | 'network' | 'unknown';
  code: string;
  message: string;
  userMessage: string;
...
abstract class EnhancedBaseService {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
          ...
          if (options.validateResult && !options.validateResult(result)) {
            throw new ValidationError('Result validation failed', { result, operationId });
          }
  ...
  private createEnhancedServiceError(error: Error, operationName: string): EnhancedServiceError {
  ...
  private getUserFriendlyMessage(error: Error): string {
    const friendlyMessages: Record<string, string> = {
      'ValidationError': 'The provided information is not valid. Please check your input.',
      'DatabaseError': 'We encountered a database issue. Please try again in a moment.',
      'BusinessLogicError': 'This operation is not allowed due to business rules.',
      'ConcurrencyError': 'Another user modified this data. Please refresh and try again.',
...
export class EnhancedPropertyService extends EnhancedBaseService {
  ...
  async getProperty(id: PropertyId): Promise<EnhancedServiceResult<EnhancedProperty>> {
    ...
    return this.executeEnhanced(
      () => {
        ...
        if (options.search) {
          ...
          query = query.or(`property_name.ilike.%${search}%,street_address.ilike.%${search}%`);
        }
  ...
  async createProperty(
    ...
    if (!validationResult.valid) {
      throw new ValidationError('Invalid property data', validationResult.errors);
    }
...
export class EnhancedChecklistService extends EnhancedBaseService {
  ...
  async updateChecklistItem(
    id: ChecklistItemId,
    updates: Partial<Pick<EnhancedChecklistItem, 'aiResult' | 'inspectorRemarks' | 'pass' | 'inspectorId'>>
  ): Promise<EnhancedServiceResult<EnhancedChecklistItem>> {
    // Validate updates
    const validationResult = this.validateChecklistItemUpdates(updates);
    if (!validationResult.valid) {
      throw new ValidationError('Invalid checklist item updates', validationResult.errors);
    }
...
export const enhancedPropertyService = EnhancedServiceFactory.getPropertyService();
export const enhancedChecklistService = EnhancedServiceFactory.getChecklistService();