...
export class PerformanceMonitor {
  ...
  trackQuery(
    metrics: Omit<
      QueryMetrics,
      "queryId" | "duration" | "dataSize" | "userAgent" | "connectionType"
    >,
  ): void {
  ...
  private generateRecommendations(
    ...
    const lowCacheServices = services.filter(
      (s) => s.cacheHitRate < PERFORMANCE_THRESHOLDS.MIN_CACHE_HIT_RATE,
    );
    ...
    const slowQueryServices = services.filter(
      (s) => s.averageDuration > PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS,
    );
    ...
    const highMemoryServices = services.filter(
      (s) => s.memoryUsage > PERFORMANCE_THRESHOLDS.MAX_MEMORY_MB,
    );
  ...
  private checkPerformanceThresholds(metrics: QueryMetrics): void {
    ...
    if (serviceMetrics) {
      ...
      if (
        ...
        PERFORMANCE_THRESHOLDS.ALERT_THRESHOLD_MS
      ) {
        this.sendAlert(
          "warning",
          `${metrics.service} average response time exceeded threshold`,
          {
            service: metrics.service,
      ...
      if (
        serviceMetrics.cacheHitRate < PERFORMANCE_THRESHOLDS.MIN_CACHE_HIT_RATE
      ) {
        this.sendAlert(
          "warning",
          `${metrics.service} cache hit rate below threshold`,
          {
            service: metrics.service,
  ...
  private updateServiceStats(queryMetrics: QueryMetrics): void {
    ...
    if (existing) {
      ...
      const newErrorCount =
        existing.errorRate * existing.totalQueries +
        (queryMetrics.success ? 0 : 1);
      ...
      this.serviceStats.set(serviceName, {
        ...
        slowQueryCount:
          ...
          (queryMetrics.duration > PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS
            ? 1
            : 0),
      ...
      this.serviceStats.set(serviceName, {
        ...
        slowQueryCount:
          queryMetrics.duration > PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS ? 1 : 0,
    ...
    const recentQueries = this.metrics.filter(
      (m) =>
        m.service === serviceName && Date.now() - m.endTime < 60 * 60 * 1000, // Last hour
    );
  ...
  private calculateSummary(metrics: QueryMetrics[]) {
    ...
    const slowQueries = metrics.filter(
      (m) => m.duration > PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS,
    );
    ...
    return {
      ...
      averageResponseTime:
        totalQueries > 0
          ? metrics.reduce((sum, m) => sum + m.duration, 0) / totalQueries
          : 0,
      cacheHitRate:
  ...
  private calculateTrends(metrics: QueryMetrics[]) {
    ...
    const responseTimesByHour = hours.map((hour) => {
      ...
      const hourMetrics = metrics.filter(
        (m) => m.endTime >= hourStart && m.endTime < hourEnd,
      );
      ...
      return {
        ...
        avgTime:
          hourMetrics.length > 0
            ? hourMetrics.reduce((sum, m) => sum + m.duration, 0) /
              hourMetrics.length
            : 0,
      };
    });
    ...
    const cacheHitsByHour = hours.map((hour) => {
      ...
      return {
        hour: 23 - hour,
        hitRate:
          hourMetrics.length > 0 ? (cacheHits / hourMetrics.length) * 100 : 0,
      };
    });
    ...
    const errorsByHour = hours.map((hour) => {
      ...
      return {
        hour: 23 - hour,
        errorRate:
          hourMetrics.length > 0 ? (errors / hourMetrics.length) * 100 : 0,
      };
    });
  ...
  private estimateMemoryUsage(): number {
    ...
    return (
      Math.round(((metricsSize + serviceStatsSize) / (1024 * 1024)) * 100) / 100
    );
  }
  ...
  private isMobileRequest(metrics: QueryMetrics): boolean {
    return (
      metrics.userAgent?.includes("Mobile") ||
    ...
    this.reportTimer = setInterval(
      () => {
        ...
        if (
          ...
          PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS
        ) {
          this.sendAlert(
            "warning",
