...
import type { HealthCheckReport, HealthCheckResult } from '../types/health-check';
...
class HealthCheckService {
  async runCompleteHealthCheck(): Promise<HealthCheckReport> {
    ...
    const [databaseResults, authResults, serviceResults, workflowResults] = await Promise.allSettled([
      this.checkDatabase(),
    ...
    const overall = failedChecks > 0 ? 'critical' : 
                   passedChecks < totalChecks ? 'degraded' : 'healthy';

    const duration = Date.now() - startTime;
    log('info', 'Health check completed', { duration, overall, passedChecks, totalChecks });
  ...
  private extractResults(settledResult: PromiseSettledResult<HealthCheckResult[]>): HealthCheckResult[] {
  ...
  private async checkDatabase(): Promise<HealthCheckResult[]> {
    ...
    // Table Access
    const tables = ['profiles', 'properties', 'inspections', 'logs', 'static_safety_items'];
