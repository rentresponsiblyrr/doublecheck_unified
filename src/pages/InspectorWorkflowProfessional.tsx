...
export function InspectorWorkflowProfessional() {
  ...
  const handlePropertySelected = useCallback(async (property: Property) => {
    try {
      setSelectedProperty(property);
      updateStepStatus('property_selection', 'completed', property);
      setCurrentStep(1);
    } catch (error) {
      await recoverFromError(error as Error, { context: 'property_selection' });
    }
  ...
  const handleChecklistGenerated = useCallback(async (checklist: ChecklistData) => {
    try {
      ...
      await recoverFromError(error as Error, { context: 'checklist_generation' });
    }
  }, [setGeneratedChecklist, selectedProperty, user, setCurrentInspectionId, updateStepStatus, setCurrentStep, recoverFromError]);
  ...
  }, [updateStepStatus, setSyncProgress, navigate, currentInspectionId, recoverFromError]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          <Alert className="border-red-200 bg-red-50">
            ...
            <AlertDescription className="text-red-700 mb-4">
              {error.message || 'An error occurred during the inspection process.'}
            </AlertDescription>
