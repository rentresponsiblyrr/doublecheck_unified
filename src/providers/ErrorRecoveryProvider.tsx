...
interface ErrorRecoveryContextType {
  ...
  retryOperation: (operation: () => Promise<void>, config?: Partial<RetryConfig>) => Promise<boolean>;
...
export const ErrorRecoveryProvider: React.FC<ErrorRecoveryProviderProps> = ({
  ...
  const retryOperation = useCallback(async (
    ...
    logger.logError('Operation retry exhausted', lastError || new Error('Unknown error'), {
      component: 'ErrorRecoveryProvider',
      totalAttempts: effectiveConfig.maxAttempts
    });
  ...
  const recoverFromError = useCallback(async (
    ...
    if (errorMessage.includes('auth') || errorMessage.includes('unauthorized') || errorMessage.includes('token')) {
      return await retryOperation(async () => {
        window.dispatchEvent(new CustomEvent('auth-recovery', { 
    ...
    if (errorMessage.includes('database') || errorMessage.includes('api') || errorMessage.includes('server')) {
      return await retryOperation(async () => {
    ...
    return await retryOperation(async () => {
      window.dispatchEvent(new CustomEvent('state-recovery', { 
