...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const bridgeHealth = (global as Record<string, unknown>)
      .__INTEGRATION_BRIDGE_HEALTH__;
    const bridgeInstance = (global as Record<string, unknown>)
      .__PWA_ENHANCED_BRIDGE__;
    ...
    const enhancedServices = (global as Record<string, unknown>)
      .__ENHANCED_SERVICES__;
    ...
    const backgroundSyncManager = (global as Record<string, unknown>)
      .__BACKGROUND_SYNC_MANAGER__ as { isActive?: () => boolean } | undefined;
    const pushNotificationManager = (global as Record<string, unknown>)
      .__PUSH_NOTIFICATION_MANAGER__ as
      | { isActive?: () => boolean }
      | undefined;

    healthData.services.pwa = {
      allSystemsReady:
        (pwaStatus as { allSystemsReady?: boolean })?.allSystemsReady || false,
      serviceWorkerActive:
        (pwaStatus as { serviceWorkerActive?: boolean })?.serviceWorkerActive ||
        false,
