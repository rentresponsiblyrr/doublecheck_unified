...
describe("Inspection Monitoring Integration", () => {
  ...
  describe("Error Tracking", () => {
    it("should track inspection errors with full context", async () => {
      ...
      expect(metrics.errorsByCode[InspectionErrorCode.PROPERTY_NOT_FOUND]).toBe(
        1,
      );
      expect(metrics.recentErrors).toHaveLength(1);
      expect(metrics.recentErrors[0].errorCode).toBe(
  ...
  describe("Alert Generation", () => {
    ...
    it("should generate alerts for low success rates", () => {
      ...
      expect(alerts.some((alert) => alert.type === "success_rate_drop")).toBe(
        true,
      );
    });
    ...
    it("should generate alerts for critical errors", () => {
      ...
      expect(alerts.some((alert) => alert.type === "critical_failure")).toBe(
        true,
      );
  ...
  describe("Real-time Updates", () => {
    it("should provide real-time metrics updates", () => {
      ...
      expect(updatedMetrics.totalErrors).toBeGreaterThan(
