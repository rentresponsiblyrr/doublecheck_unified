...
export const deletePropertyData = async (propertyId: string): Promise<void> => {
  log.info(
    "Starting comprehensive property deletion",
    {
      propertyId,
  ...
  if (deletionInProgress.has(propertyId)) {
    log.warn(
      "Property deletion already in progress",
      {
        propertyId,
        component: "propertyDeletion",
      },
    ...
    throw new Error(
      "Property deletion is already in progress. Please wait for the current operation to complete.",
    );
  }
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          log.warn(
            "Network error during deletion, skipping",
            {
              tableName,
              description,
              component: "propertyDeletion",
      ...
      throw new Error(
        `Failed to delete ${description}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  };
  ...
  try {
    ...
    if (inspectionsQueryError) {
      // Handle network errors gracefully
      if (
        inspectionsQueryError.message.includes("ERR_INTERNET_DISCONNECTED") ||
      ...
      throw new Error(
        `Failed to query inspections: ${inspectionsQueryError.message}`,
      );
    }
    ...
    if (inspections && inspections.length > 0) {
      ...
      const { data: checklistItems, error: checklistQueryError } =
        await supabase
      ...
      if (checklistQueryError) {
        // Handle network errors gracefully
        if (
          checklistQueryError.message.includes("ERR_INTERNET_DISCONNECTED") ||
        ...
        throw new Error(
          `Failed to query checklist items: ${checklistQueryError.message}`,
        );
      }
      ...
      if (checklistItems && checklistItems.length > 0) {
        ...
        if (checklistItemIds.length > 0) {
          ...
          for (let i = 0; i < checklistItemIds.length; i += BATCH_SIZE) {
            ...
            await safeDelete(
              "checklist_audit_log",
              { checklist_item_id: batch },
              `checklist audit logs batch ${Math.floor(i / BATCH_SIZE) + 1}`,
            );
          }
        }
        ...
        if (checklistItemIds.length > 0) {
          ...
          for (let i = 0; i < checklistItemIds.length; i += BATCH_SIZE) {
            ...
            await safeDelete(
              "media",
              { checklist_item_id: batch },
              `media batch ${Math.floor(i / BATCH_SIZE) + 1}`,
            );
          }
        }
      }
      ...
      if (inspectionIds.length > 0) {
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            ...
            `checklist operations audit for inspection ${inspectionId}`,
          );
        }
      }
      ...
      if (inspectionIds.length > 0) {
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            "auditor_feedback",
            { inspection_id: inspectionId },
            `auditor feedback for inspection ${inspectionId}`,
          );
        }
      }
      ...
      if (inspectionIds.length > 0) {
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            "rag_query_log",
            { inspection_id: inspectionId },
            `RAG query logs for inspection ${inspectionId}`,
          );
        }
      }
      ...
      if (inspectionIds && inspectionIds.length > 0) {
        ...
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            "audit_feedback",
            { inspection_id: inspectionId },
            `audit feedback for inspection ${inspectionId}`,
          );
        }
      } else {
      }
      ...
      if (inspectionIds && inspectionIds.length > 0) {
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            "report_deliveries",
            { inspection_id: inspectionId },
            `report deliveries for inspection ${inspectionId}`,
          );
        }
      }
      ...
      if (inspectionIds && inspectionIds.length > 0) {
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            "inspection_reports",
            { inspection_id: inspectionId },
            `inspection reports for inspection ${inspectionId}`,
          );
        }
      }
      ...
      if (inspectionIds && inspectionIds.length > 0) {
        for (const inspectionId of inspectionIds) {
          await safeDelete(
            "checklist_items",
            { inspection_id: inspectionId },
            `checklist items for inspection ${inspectionId}`,
          );
        }
      }
    }
    ...
    if (listingPhotosError) {
      throw new Error(
        `Failed to delete listing photos: ${listingPhotosError.message}`,
      );
    }
    ...
    if (webhookError) {
      throw new Error(
        `Failed to delete webhook notifications: ${webhookError.message}`,
      );
    }
    ...
    if (inspectionsError) {
      throw new Error(
        `Failed to delete inspections: ${inspectionsError.message}`,
      );
    }
    ...
    if (propertyError) {
      ...
      if (propertyError.message.includes("violates row-level security")) {
        throw new Error(
          "You can only delete properties you created or you need admin permissions.",
        );
      }
