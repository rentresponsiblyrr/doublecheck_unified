...
export class PerformanceMetrics {
  ...
  getSummary(): {
    ...
    const renderMeasurements = this.measurements.filter(m => 
      m.name.includes('render') || m.name.includes('component')
    );

    const averageRenderTime = renderMeasurements.length > 0
...
// Network performance simulation
export function simulateSlowNetwork(conditions: Partial<NetworkConditions>): void {
  ...
  global.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (contentLength && method === 'GET') {
      ...
      const downloadTime = (sizeBytes / networkConditions.downloadThroughput) * 1000;
...
export async function measureRenderTime(renderFunction: () => void): Promise<number> {
  const startTime = performance.now();
...
export function detectPerformanceRegression(
  ...
  const baselineMap = new Map(
    baseline.map(m => [m.name, m.duration])
  );
  ...
  current.forEach(currentMeasurement => {
    ...
    if (baselineTime) {
      const regressionPercent = (currentMeasurement.duration - baselineTime) / baselineTime;
