...
class ScreenshotCaptureService {
   ...
   */
  async captureScreenUsingAPI(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    try {
   ...
   */
  async captureViewportUsingHTML2Canvas(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    try {
  ...
  async captureScreenshot(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    ...
    try {
      ...
      try {
        ...
        logger.info('Screenshot captured using Screen Capture API', { 
          dimensions: result.dimensions 
        }, 'SCREENSHOT_CAPTURE');
        return result;
      } catch (screenApiError) {
        logger.warn('Screen Capture API failed, falling back to HTML2Canvas', screenApiError, 'SCREENSHOT_CAPTURE');
        
        // Fallback to HTML2Canvas (viewport only)
        const result = await this.captureViewportUsingHTML2Canvas(options);
        logger.info('Screenshot captured using HTML2Canvas fallback', { 
          dimensions: result.dimensions 
        }, 'SCREENSHOT_CAPTURE');
        return result;
      }
      ...
      logger.error('All screenshot methods failed', error, 'SCREENSHOT_CAPTURE');
   ...
   */
  async compressScreenshot(screenshot: ScreenshotResult, maxSizeKB: number = 500): Promise<ScreenshotResult> {
