...
export class DatabaseDiagnosticService {
  ...
  static async runComprehensiveDiagnostic(): Promise<DatabaseDiagnostic> {
    logger.info('🔍 Starting comprehensive database diagnostic...', {}, 'DB_DIAGNOSTIC');
    ...
    try {
      ...
      diagnostic.recommendations.push('Database diagnostic failed completely - check connection and permissions');
      return diagnostic;
    }
  }

  private static async getCurrentUserInfo(): Promise<DatabaseDiagnostic['currentUser']> {
    try {
  ...
  private static async checkRLSPolicies(tableName: string): Promise<DatabaseDiagnostic['rlsPolicies'][0]> {
    const rlsInfo = {
      table: tableName,
      hasRLS: false,
      policies: [] as string[]
    ...
    try {
      ...
      logger.warn(`Failed to check RLS for ${tableName}`, error, 'DB_DIAGNOSTIC');
    }

    return rlsInfo;
  }

  private static determineOverallHealth(diagnostic: DatabaseDiagnostic): 'healthy' | 'degraded' | 'critical' {
    const criticalTableResults = diagnostic.tables.filter(t => CRITICAL_TABLES.includes(t.name));
    
    // Count critical issues
    const missingCriticalTables = criticalTableResults.filter(t => !t.exists).length;
    const inaccessibleCriticalTables = criticalTableResults.filter(t => t.exists && !t.accessible).length;
  ...
  private static generateRecommendations(diagnostic: DatabaseDiagnostic): string[] {
    ...
    if (missingTables.length > 0) {
      recommendations.push(`Critical tables missing: ${missingTables.join(', ')}. Run database migrations.`);
    }
    ...
    if (inaccessibleTables.length > 0) {
      recommendations.push(`Tables exist but not accessible: ${inaccessibleTables.join(', ')}. Check RLS policies and user permissions.`);
    }

    // Check user role
    if (!diagnostic.currentUser.role) {
      recommendations.push('Current user has no role assigned. Assign appropriate role for access to admin features.');
    }

    // Check for empty critical tables
    const emptyCriticalTables = diagnostic.tables
      .filter(t => CRITICAL_TABLES.includes(t.name) && t.accessible && t.rowCount === 0)
      .map(t => t.name);
    
    if (emptyCriticalTables.includes('users')) {
      recommendations.push('Users table is empty. Create initial admin users or check user sync from auth.users.');
    }

    if (emptyCriticalTables.includes('properties')) {
      recommendations.push('No properties found. Add test properties for development/testing.');
    }
    ...
    if (largeAccessibleTables.length > 0) {
      recommendations.push(`Large tables detected: ${largeAccessibleTables.join(', ')}. Consider implementing pagination and indexing optimization.`);
    }

    if (recommendations.length === 0) {
      recommendations.push('Database appears healthy. All critical systems are functioning normally.');
    }
...
export const runDatabaseDiagnostic = DatabaseDiagnosticService.runComprehensiveDiagnostic;
