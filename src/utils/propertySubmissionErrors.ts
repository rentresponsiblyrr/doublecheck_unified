...
export const usePropertyErrorHandler = () => {
  ...
  const handleSubmissionError = (
    ...
    // Authentication errors
      ...
      errorDetails.message?.includes("JWT")
    ) {
    ...
    // Permission errors
      ...
      errorMessage =
        "You don't have permission to access this property. Please contact your administrator.";
    }
    // Unique constraint violations
      ...
      if (errorDetails.message?.includes("properties_name")) {
        ...
        errorMessage =
          "This Vrbo URL is already registered. Please check if the property already exists.";
      } else if (errorDetails.message?.includes("properties_airbnb_url")) {
        errorMessage =
          "This Airbnb URL is already registered. Please check if the property already exists.";
      } else {
        errorMessage =
          "This property information conflicts with an existing property.";
      }
    }
    // Not found errors
    else if (errorDetails.code === "PGRST116") {
      errorMessage =
        "The property could not be found. It may have been deleted by another user.";
    }
    // Foreign key violations
    else if (errorDetails.code === "23503") {
      errorMessage =
        "Invalid reference data. Please refresh the page and try again.";
      isTemporary = true;
    }
