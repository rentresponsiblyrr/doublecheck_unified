...
export class DatabaseVerification {
  ...
  static async runFullVerification(): Promise<DatabaseStatus> {
    ...
    if (missingTables.length > 0) {
      recommendations.push(`Missing tables detected: ${missingTables.map(t => t.name).join(', ')}`);
    }
    ...
    if (!logsTable?.exists && checklistTable?.exists) {
      recommendations.push('Use "checklist_items" table instead of "logs" for checklist data');
    } else if (logsTable?.exists && !checklistTable?.exists) {
      recommendations.push('Use "logs" table for checklist data (preferred production schema)');
    } else if (!logsTable?.exists && !checklistTable?.exists) {
      recommendations.push('CRITICAL: No checklist table found - database may need setup');
    }
    
    // Status recommendations
    if (validStatusValues.length === 0) {
      recommendations.push('CRITICAL: No valid status values found - check database constraints');
    } else if (!validStatusValues.includes('draft') && !validStatusValues.includes('in_progress')) {
