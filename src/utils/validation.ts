...
export const propertyIdSchema = z
  ...
  .regex(
    /^[a-zA-Z0-9_-]+$/,
    "Property ID can only contain letters, numbers, hyphens, and underscores",
  );

// Inspection status validation
export const inspectionStatusSchema = z.enum(
  ["draft", "in_progress", "completed", "auditing", "approved", "rejected"],
  {
...
export const userRoleSchema = z.enum(
  ["inspector", "auditor", "admin", "super_admin"],
  {
...
export const inspectionCreationSchema = z.object({
  ...
  priorityAreas: z
    .array(stringSchema)
    .max(20, "Too many priority areas")
    .optional(),
});
...
export const checklistItemSchema = z.object({
  ...
  required: z.boolean(),
  gptPrompt: optionalStringSchema
    .max(2000, "GPT prompt is too long")
    .optional(),
...
export const useFormValidation = <T>(schema: z.ZodSchema<T>) => {
  ...
  const validateField = React.useCallback(
    (fieldName: string, value: unknown) => {
      try {
        const fieldSchema = (schema as { shape: Record<string, unknown> })
          .shape[fieldName];
        if (fieldSchema) {
