...
export const IdConverter = {
  /**
   * Properties use integers in the database, but frontend may send UUID-like strings
   */
  property: {
    toDatabase: convertUUIDLikePropertyIdToInt, // Convert to integer
    validate: (id: string) => {
      try {
        convertUUIDLikePropertyIdToInt(id);
        return true;
      } catch {
        return false;
      }
