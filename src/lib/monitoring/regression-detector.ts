...
export class PerformanceRegressionDetector {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      log.info(
        "Performance Regression Detector initialized",
        {
      ...
      log.error(
        "Failed to initialize Performance Regression Detector",
        error as Error,
        {
          component: "PerformanceRegressionDetector",
          action: "initialize",
    ...
    try {
      ...
      for (const [metricName, values] of metricGroups) {
        ...
        if (regressionAnalysis.isRegression) {
          const alert = await this.createRegressionAlert(
            metricName,
            regressionAnalysis,
            baseline,
          );
    ...
    budgets.forEach((budget) => {
      if (
        ...
        const violationPercentage =
          ((budget.current - budget.budget) / budget.budget) * 100;
        ...
        const violation: PerformanceBudgetViolation = {
          ...
          impact: this.calculateViolationImpact(
            violationPercentage,
            budget.trend,
          ),
        };
  ...
  configureAutoRollback(config: Partial<AutoRollbackConfig>): void {
    ...
    log.info(
      "Auto-rollback configuration updated",
      {
        component: "PerformanceRegressionDetector",
  ...
  private analyzeRegression(
    ...
    const degradationPercentage =
      ((currentMean - baseline.mean) / baseline.mean) * 100;
  ...
  private async createBaseline(
    ...
    if (values.length < this.DETECTION_THRESHOLDS.sampleSizeMinimum) {
      throw new Error(
        `Insufficient data for baseline creation: ${values.length} < ${this.DETECTION_THRESHOLDS.sampleSizeMinimum}`,
      );
    }
  ...
  private shouldTriggerAutoRollback(alert: RegressionAlert): boolean {
    return (
      (this.autoRollbackConfig.enabled &&
        ...
        alert.degradationPercentage >=
          this.autoRollbackConfig.degradationThreshold &&
  ...
  private async waitForConfirmation(alert: RegressionAlert): Promise<void> {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const isStillRegression = this.verifyRegressionPersistence(
          alert.metric,
        );
  ...
  private generateSuggestedActions(
    metricName: string,
    analysis: { severity: string; degradationPercentage: number },
  ): string[] {
  ...
  private async loadBaselines(): Promise<void> {
    try {
      ...
      log.warn(
        "Failed to load stored baselines",
        {
          component: "PerformanceRegressionDetector",
  ...
  private cleanupOldAlerts(): void {
    ...
    this.alerts = this.alerts.filter(
      (alert) => now - alert.detectedAt < maxAge,
    );
  }
  ...
  stop(): void {
    ...
    log.info(
      "Performance Regression Detector stopped",
      {
        component: "PerformanceRegressionDetector",
        action: "stop",
