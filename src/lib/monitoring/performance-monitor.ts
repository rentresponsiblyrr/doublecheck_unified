...
class PerformanceMonitor {
  ...
  trackInteraction(
    interaction: Omit<UserInteraction, "timestamp" | "page">,
  ): void {
  ...
  measureFunction<T>(
    ...
    try {
      ...
      this.trackMetric(
        ...
        performance.now() - start,
        "ms",
        { success: false, error: errorObj.message },
        tags,
      );
      throw error;
    }
  ...
  private trackNavigationTiming(timing: PerformanceNavigationTiming): void {
    ...
    this.trackMetric(
      "page.domReady",
      timing.domComplete - timing.fetchStart,
      "ms",
    );
    this.trackMetric(
      "page.loadComplete",
  ...
  trackFileUpload(
    ...
    this.trackMetric("upload.duration", duration, "ms", {
      success,
      fileName,
      fileSize,
    });
  ...
  trackDatabaseOperation(
    ...
    this.trackMetric(`db.${operation}.duration`, duration, "ms", {
      success,
      table,
    });

    if (rowsAffected !== undefined) {
      this.trackMetric(`db.${operation}.rows`, rowsAffected, "count", {
        table,
      });
    }
  }
  ...
  private startBatchFlush(): void {
    setInterval(() => {
      if (
        ...
        this.interactions.length >= this.batchSize
      ) {
        this.flushMetrics();
      }
    }, this.flushInterval);
