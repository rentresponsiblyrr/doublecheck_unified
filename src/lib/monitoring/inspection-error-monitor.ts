...
export interface MonitoringAlert {
  id: string;
  type: 'error_spike' | 'performance_degradation' | 'critical_failure' | 'success_rate_drop';
...
export class InspectionErrorMonitor {
  ...
  private performanceBuffer: Array<{ timestamp: string; processingTime: number }> = [];
  ...
  public static getInstance(): InspectionErrorMonitor {
    if (!InspectionErrorMonitor.instance) {
      ...
      logger.info('InspectionErrorMonitor initialized', {
   ...
   */
  trackInspectionError(errorEvent: Partial<InspectionErrorEvent> & { 
    errorCode: InspectionErrorCode; 
    message: string; 
  }): void {
    ...
    const enrichedEvent: InspectionErrorEvent = {
      ...
      technicalContext: {
        ...
        component: errorEvent.technicalContext?.component || 'InspectionCreationService',
    ...
    const logLevel = enrichedEvent.businessImpact?.severity === 'critical' ? 'error' : 'warn';
    logger[logLevel]('Inspection error tracked', {
  ...
  trackInspectionSuccess(data: {
    ...
    logger.debug('Inspection success tracked', {
      processingTime: data.processingTime,
  ...
  getErrorMetrics(timeWindowHours = 24): ErrorMetrics {
    ...
    const successRate = totalAttempts > 0 ? recentSuccesses.length / totalAttempts : 1;

    const recentPerformance = this.performanceBuffer.filter(
      entry => new Date(entry.timestamp) > cutoffTime
    );

    const averageProcessingTime = recentPerformance.length > 0
      ? recentPerformance.reduce((sum, entry) => sum + entry.processingTime, 0) / recentPerformance.length
      : 0;
  ...
  getActiveAlerts(): MonitoringAlert[] {
    ...
    // Performance alert
    if (metrics.averageProcessingTime > this.alertThresholds.avgProcessingTime) {
      alerts.push({
  ...
  private calculateSeverity(errorCode: InspectionErrorCode): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private sanitizeUserContext(context?: InspectionErrorEvent['userContext']): any {
  ...
  private getErrorMetrics(timeWindowHours: number, previousPeriod = false): ErrorMetrics {
    ...
    const periodSuccesses = this.successBuffer.filter(
      entry => {
    ...
    const errorsByCode = periodErrors.reduce((acc, error) => {
      acc[error.errorCode] = (acc[error.errorCode] || 0) + 1;
      return acc;
    }, {} as Record<InspectionErrorCode, number>);

    const totalAttempts = periodErrors.length + periodSuccesses.length;
    const successRate = totalAttempts > 0 ? periodSuccesses.length / totalAttempts : 1;
    ...
    return {
      ...
      successRate,
      criticalErrors: periodErrors.filter(e => e.businessImpact?.severity === 'critical').length,
      recentErrors: [],
      performanceTrends: {
  ...
  private checkForAlerts(errorEvent: InspectionErrorEvent): void {
    ...
    if (alerts.length > 0) {
      logger.warn('Monitoring alerts triggered', {
  ...
  private checkPerformanceAlerts(processingTime: number): void {
    if (processingTime > this.alertThresholds.avgProcessingTime) {
      logger.warn('Slow processing time detected', {
        processingTime,
        threshold: this.alertThresholds.avgProcessingTime
      }, 'INSPECTION_ERROR_MONITOR');
    }
  ...
  private async sendToExternalMonitoring(errorEvent: InspectionErrorEvent): Promise<void> {
    try {
      ...
      logger.error('Failed to send to external monitoring', monitoringError, 'INSPECTION_ERROR_MONITOR');
    }
  }

  private async sendAlertsToExternalSystems(alerts: MonitoringAlert[]): Promise<void> {
    try {
      ...
      logger.info('Alerts would be sent to external systems', {
        alertCount: alerts.length,
        criticalAlerts: alerts.filter(a => a.severity === 'critical').length
      }, 'INSPECTION_ERROR_MONITOR');
      
    } catch (alertError) {
      logger.error('Failed to send alerts to external systems', alertError, 'INSPECTION_ERROR_MONITOR');
    }
  }

  private async storeInDatabase(errorEvent: InspectionErrorEvent): Promise<void> {
    try {
      ...
      logger.debug('Failed to store monitoring event in database', dbError, 'INSPECTION_ERROR_MONITOR');
    }
  }
}
...
export const trackInspectionError = (errorEvent: Parameters<typeof inspectionErrorMonitor.trackInspectionError>[0]) => {
  return inspectionErrorMonitor.trackInspectionError(errorEvent);
};

export const trackInspectionSuccess = (data: Parameters<typeof inspectionErrorMonitor.trackInspectionSuccess>[0]) => {
