...
class CircularDependencyDetector {
  ...
  async analyzeDependencies(): Promise<CircularDependencyReport> {
    ...
    const report: CircularDependencyReport = {
      ...
      criticalCount: this.detectedCircularDependencies.filter(
        (d) => d.severity === "critical",
      ).length,
      highCount: this.detectedCircularDependencies.filter(
        (d) => d.severity === "high",
      ).length,
      mediumCount: this.detectedCircularDependencies.filter(
        (d) => d.severity === "medium",
      ).length,
      lowCount: this.detectedCircularDependencies.filter(
        (d) => d.severity === "low",
      ).length,
  ...
  private reportCircularDependency(cycle: string[]): void {
    ...
    if (
      this.detectedCircularDependencies.some(
        (d) => d.cycle.join(" -> ") === cycleId,
      )
    ) {
      return;
    }
  ...
  private calculateSeverity(cycle: string[]): CircularDependency["severity"] {
    ...
    if (
      cycle.some(
        (module) => module.includes("component") || module.includes("page"),
      )
    ) {
      return "medium";
    }
  ...
  private determineType(cycle: string[]): CircularDependency["type"] {
    ...
    if (cycle.some((module) => module.includes("component")))
      return "component";
    return "module";
  }
  ...
  private generateRecommendations(): string[] {
    ...
    const criticalCount = this.detectedCircularDependencies.filter(
      (d) => d.severity === "critical",
    ).length;
    const highCount = this.detectedCircularDependencies.filter(
      (d) => d.severity === "high",
    ).length;
  ...
  exportToDot(): string {
    ...
    for (const circularDep of this.detectedCircularDependencies) {
      const color =
        circularDep.severity === "critical"
          ? "red"
          : circularDep.severity === "high"
            ? "orange"
            : circularDep.severity === "medium"
              ? "yellow"
              : "gray";
