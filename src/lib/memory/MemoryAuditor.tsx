...
import { memoryLeakDetector, MemoryStats, MemoryLeak } from './MemoryLeakDetector';
import { circularDependencyDetector, CircularDependencyReport } from './CircularDependencyDetector';
...
export const MemoryAuditor: React.FC<MemoryAuditorProps> = ({ 
  ...
  const [circularReport, setCircularReport] = useState<CircularDependencyReport | null>(null);
        ...
        <Tabs defaultValue="overview" className="h-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="circular">
              Circular Deps
              {circularReport && circularReport.totalCircularDependencies > 0 && (
              ...
              <Card>
                ...
                <CardContent>
                  <div className={`text-2xl font-bold ${
                    memoryStats.growthRate > 10 ? 'text-red-600' : 
                    memoryStats.growthRate > 5 ? 'text-orange-600' : 
                    'text-green-600'
                  }`}>
                    {memoryStats.growthRate > 0 ? '+' : ''}{memoryStats.growthRate.toFixed(1)} MB/min
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>Current: {memoryStats.currentUsage.toFixed(1)} MB</span>
                    <span>Peak: {memoryStats.peakUsage.toFixed(1)} MB</span>
                  </div>
                  <Progress 
                    value={(memoryStats.currentUsage / Math.max(memoryStats.peakUsage, 100)) * 100}
                    className="w-full"
                  />
                </div>
                  ...
                  <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded">
                    <div className="flex items-center">
                      ...
                      <span className="text-sm text-orange-800">
                        High memory growth rate detected. Consider reviewing recent changes.
                      </span>
                    </div>
                  </div>
                )}
          ...
          <TabsContent value="circular" className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium">Circular Dependency Analysis</h3>
              <Button
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-sm text-gray-600">
                    Snapshots taken over {Math.round(memoryStats.monitoringDuration)} minutes
                  </p>
                </CardContent>
              </Card>
    ...
    <Card className={`border-l-4 ${getSeverityColor(leak.severity)}`}>
      ...
      <CardContent>
        ...
        {leak.suggestions.length > 0 && (
          <div>
            <h4 className="text-xs font-medium text-gray-600 mb-1">Suggestions:</h4>
...
const CircularDependencyReport: React.FC<{ report: CircularDependencyReport }> = ({ report }) => {
  return (
    <div className="space-y-4">
      ...
      {report.dependencies.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Detected Circular Dependencies</CardTitle>
          </CardHeader>
          <CardContent>
