...
type WorkerOptions = CompressionOptions | PhotoCompressionOptions | { maxSize: number };
...
export class MediaCompressionWorkerInterface {
  ...
  private pendingPromises: Map<string, { resolve: ResolverFunction<unknown>; reject: RejectorFunction }> = new Map();
  ...
  constructor() {
    ...
    if (this.workerSupported) {
      try {
        ...
        this.isReady = true;
        console.log('MediaCompressionWorker: Web Worker initialized successfully');
      } catch (error) {
        console.warn('MediaCompressionWorker: Failed to initialize Web Worker, falling back to main thread:', error);
        this.worker = null;
        this.isReady = false;
        this.workerSupported = false;
      }
    } else {
      console.log('MediaCompressionWorker: Web Workers not supported, using fallback processing');
      this.isReady = false;
    }
  }
  ...
  private postMessage<T>(type: WorkerMessage['type'], file: File, options: WorkerOptions): Promise<T> {
   ...
   */
  private async fallbackProcessing<T>(type: WorkerMessage['type'], file: File, options: WorkerOptions): Promise<T> {
    console.log(`MediaCompressionWorker: Using fallback processing for ${type}`);
    ...
    switch (type) {
      ...
      case 'SHOULD_COMPRESS':
        return Promise.resolve(file.size > (options.maxSize || 10 * 1024 * 1024)) as Promise<T>;
      default:
   ...
   */
  private async fallbackVideoCompression(file: File, options: VideoCompressionOptions): Promise<CompressionResult> {
   ...
   */
  private async fallbackPhotoCompression(file: File, options: PhotoCompressionFallbackOptions): Promise<CompressionResult> {
    const {
      quality = 0.8,
      maxWidth = 1920,
      maxHeight = 1080,
   ...
   */
  private calculateOptimalDimensions(origWidth: number, origHeight: number, maxWidth: number, maxHeight: number) {
   ...
   */
  async compressPhoto(file: File, options: PhotoCompressionOptions = {}): Promise<CompressionResult> {
   ...
   */
  async shouldCompress(file: File, maxSize: number = 10 * 1024 * 1024): Promise<boolean> {
