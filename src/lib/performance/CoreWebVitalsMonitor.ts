...
export class CoreWebVitalsMonitor {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (!this.checkBrowserSupport()) {
        logger.warn('Performance Observer not supported - degraded monitoring', {}, 'PERFORMANCE');
      ...
      logger.info('Core Web Vitals Monitor initialized successfully', {
      ...
      logger.error('Core Web Vitals Monitor initialization failed', { error }, 'PERFORMANCE');
      return false;
    }
  }
      ...
      const observer = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry: any) => {
          const fidMetric: PerformanceMetric = {
            ...
            percentile: this.calculatePercentile('fid', entry.processingStart - entry.startTime),
            delta: this.calculateDelta('fid', entry.processingStart - entry.startTime),
            trend: this.calculateTrend('fid', entry.processingStart - entry.startTime),
            entries: [entry]
          };
  ...
  private setupTTFBObserver(): void {
    ...
    try {
      const observer = new PerformanceObserver((list) => {
        const entries = list.getEntries();
        const navEntry = entries.find((entry: any) => entry.entryType === 'navigation') as any;
  ...
  private setupNavigationObserver(): void {
    ...
    if ('PerformanceObserver' in window) {
      try {
        ...
        logger.error('Failed to setup navigation observer', { error }, 'PERFORMANCE');
      }
    }
  }
  ...
  private setupResourceObserver(): void {
    ...
    if ('PerformanceObserver' in window) {
      try {
        const observer = new PerformanceObserver((list) => {
          const entries = list.getEntries();
          entries.forEach(entry => this.processResourceEntry(entry as PerformanceResourceTiming));
        });
        
        observer.observe({ type: 'resource', buffered: true });
        this.observers.set('resource', observer);
      } catch (error) {
        logger.error('Failed to setup resource observer', { error }, 'PERFORMANCE');
      }
    }
  }
  ...
  private setupLongTaskObserver(): void {
    ...
    if ('PerformanceObserver' in window) {
      try {
        ...
        logger.error('Failed to setup long task observer', { error }, 'PERFORMANCE');
      }
    }
  }
   ...
   */
  getPerformanceTrends(period: '1h' | '24h' | '7d' | '30d' = '24h'): PerformanceTrend[] {
  ...
  private calculateDelta(metric: string, value: number): number {
    const lastMetric = this.metrics[metric as keyof CoreWebVitalsMetrics] as PerformanceMetric;
    return lastMetric ? value - lastMetric.value : 0;
  }

  private calculateTrend(metric: string, value: number): 'improving' | 'stable' | 'degrading' {
  ...
  private updatePerformanceScores(): void {
    ...
    if (this.metrics.lcp) scores.push(this.getMetricScore('lcp', this.metrics.lcp.value));
    if (this.metrics.fid) scores.push(this.getMetricScore('fid', this.metrics.fid.value));
    if (this.metrics.cls) scores.push(this.getMetricScore('cls', this.metrics.cls.value));
    if (this.metrics.fcp) scores.push(this.getMetricScore('fcp', this.metrics.fcp.value));
    if (this.metrics.ttfb) scores.push(this.getMetricScore('ttfb', this.metrics.ttfb.value));
  ...
  private triggerPerformanceAlert(metric: string, value: number, budget: PerformanceBudget): void {
    const alert: PerformanceAlert = {
      id: `${metric}_${Date.now()}`,
      metric,
      severity: budget.status === 'exceeded' ? 'critical' : 'warning',
      value,
      threshold: budget.status === 'exceeded' ? budget.critical : budget.warning,
    ...
    this.alertSubscribers.forEach(callback => {
      try {
        ...
        logger.error('Performance alert callback failed', { error }, 'PERFORMANCE');
      }
    });
  ...
  private startBudgetMonitoring(): void {
    ...
    setInterval(() => {
      this.performanceBudgets.forEach(budget => {
        const currentMetric = this.metrics[budget.metric as keyof CoreWebVitalsMetrics] as PerformanceMetric;
        if (currentMetric) {
  ...
  private setupAdaptiveSampling(): void {
    ...
    logger.info('Adaptive sampling configured', {
      samplingRate: this.samplingRate,
  ...
  private calculateMetricTrend(metric: string, period: string): PerformanceTrend | null {
  ...
  private processResourceEntry(entry: PerformanceResourceTiming): void {
    ...
    if (entry.transferSize > 100000) { // Resources larger than 100KB
      logger.debug('Large resource detected', {
        name: entry.name,
  ...
  destroy(): void {
    this.observers.forEach((observer, type) => {
      try {
        ...
        logger.error(`Failed to disconnect ${type} observer`, { error }, 'PERFORMANCE');
      }
    });
