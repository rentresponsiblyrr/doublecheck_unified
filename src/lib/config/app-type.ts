...
export const APP_TYPE_CONFIGS: Record<AppType, AppTypeConfig> = {
  [AppType.INSPECTOR]: {
    ...
    excludedRoutes: ["/auditor", "/admin/*", "/debug-inspection/*"],
...
export function isRoleAllowed(role: string): boolean {
  ...
  if (
    config.requiredRoles.includes("admin") &&
...
export function isInspectorDomain(): boolean {
  ...
  return (
    domain.includes("app.doublecheckverified.com") ||
    (domain.includes("localhost") &&
      getAppTypeFromEnvironment() === AppType.INSPECTOR)
  );
}
...
export function isAdminDomain(): boolean {
  const domain = getCurrentDomain();
  return (
    domain.includes("admin.doublecheckverified.com") ||
    (domain.includes("localhost") &&
      getAppTypeFromEnvironment() === AppType.ADMIN)
  );
}
...
export function getAppRoutes(): string[] {
  ...
  if (config.type === AppType.ADMIN) {
    return ["/", "/auditor", "/admin/*", "/debug-inspection/:id", "/health"];
  }
...
export function getAppTypeDisplayName(): string {
  const config = getCurrentAppConfig();
  return config.type === AppType.INSPECTOR
    ? "Inspector App"
    : "Admin/Audit App";
}
