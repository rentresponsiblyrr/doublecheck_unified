...
export class PIIProtectionService {
    ...
    if (typeof value === "object") {
      ...
      for (const [key, val] of Object.entries(value)) {
        ...
        if (this.isFieldNameSensitive(key)) {
          ...
          scrubbed[key] = this.getReplacementForFieldType(
            fieldType,
            sensitivityLevel,
          );
          continue;
  ...
  private static getReplacementForType(
    ...
    const baseReplacement =
      PII_CONFIG.REPLACEMENT_CHARS[
        piiType as keyof typeof PII_CONFIG.REPLACEMENT_CHARS
      ] || "[REDACTED]";
  ...
  private static getFieldTypeFromName(fieldName: string): PIIType {
    ...
    if (fieldLower.includes("birth") || fieldLower.includes("dob"))
      return "DATE_BIRTH";
   ...
   */
  private static getReplacementForFieldType(
    fieldType: PIIType,
    sensitivityLevel: SensitivityLevel,
  ): string {
  ...
  static secureLog(
    ...
    const scrubContext = context
      ? this.scrubPII(context, scrubOptions)
      : context;
   ...
   */
  static validateNoPII(data: ScrubableData): {
    isClean: boolean;
    violations: string[];
  } {
...
export const scrubPII =
  PIIProtectionService.scrubPII.bind(PIIProtectionService);
export const validateNoPII =
  PIIProtectionService.validateNoPII.bind(PIIProtectionService);
export const generateTestData =
  PIIProtectionService.generateTestData.bind(PIIProtectionService);
