...
export const PropertyUrlSchema = z.string()
  ...
  .max(SECURITY_CONFIG.MAX_URL_LENGTH, `URL must be less than ${SECURITY_CONFIG.MAX_URL_LENGTH} characters`)
  ...
  .transform((url) => {
    ...
    logger.info('Property URL validated successfully', {
      url: sanitized,
...
export const PropertyAddressSchema = z.string()
  ...
  .max(SECURITY_CONFIG.MAX_ADDRESS_LENGTH, `Address must be less than ${SECURITY_CONFIG.MAX_ADDRESS_LENGTH} characters`)
  .refine((address) => {
...
export async function validateFileSignature(file: File): Promise<boolean> {
  try {
    ...
    const headerSize = Math.max(...Object.values(FILE_SIGNATURES).map(sig => sig.length));
    ...
    logger.info('File signature validated successfully', {
      fileName: file.name,
      fileType: file.type,
      fileSize: file.size,
      component: 'SecurityValidation',
    ...
    logger.error('File signature validation failed', error as Error, {
      fileName: file.name,
      fileType: file.type,
      fileSize: file.size,
      component: 'SecurityValidation',
 ...
 */
export const InspectionIdSchema = z.string()
