...
export class SecureAuthManager {
  ...
  async validateSession(userId?: string): Promise<AuthValidationResult> {
    ...
    try {
      ...
      const requiresRefresh =
        ...
        AUTH_CONFIG.TOKEN_REFRESH_THRESHOLD_MS;
      ...
      logger.error(
        ...
        error as Error,
        {
          userId,
          errorCode:
            error instanceof AuthenticationError ? error.code : "UNKNOWN",
          validationTimeMs: Date.now() - startTime,
          component: "SecureAuthManager",
          action: "validateSession",
        },
  ...
  private validateTokenIntegrity(token: string): boolean {
    try {
      ...
      logger.warn(
        "Token integrity validation error",
        error as Error,
        {
          component: "SecureAuthManager",
  ...
  private recordFailedAttempt(userId?: string): void {
    ...
    const recentAttempts = attempts.filter(
      (attempt) =>
        now - attempt.timestamp < AUTH_CONFIG.LOCKOUT_DURATION_MS * 4,
    );
  ...
  getSecurityStatus(): {
    ...
    return {
      ...
      rateLimitedUsers: Array.from(this.rateLimitTracker.values()).filter(
        (requests) => requests.length >= AUTH_CONFIG.MAX_REQUESTS_PER_MINUTE,
      ).length,
    };
  }
}
