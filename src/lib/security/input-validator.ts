...
export class InputValidator {
  ...
  sanitizeText(input: string, maxLength?: number): string {
    ...
    if (sanitized.length > limit) {
      throw new ValidationError(
        `Text too long (max ${limit} characters)`,
        "text",
        "TOO_LONG",
      );
    }
  ...
  async validateImage(file: File): Promise<File> {
    ...
    if (this.containsScript(sample)) {
      throw new ValidationError(
        "Image contains embedded script",
        "image",
        "EMBEDDED_SCRIPT",
      );
    }
  ...
  validateArray<T>(
    ...
    if (array.length > limit) {
      throw new ValidationError(
        `Array too long (max ${limit} items)`,
        "array",
        "TOO_LONG",
      );
    }
  ...
  validateJson(input: string): unknown {
    ...
    for (const pattern of SECURITY_CONFIG.dangerousPatterns) {
      if (pattern.test(input)) {
        throw new ValidationError(
          "JSON contains dangerous content",
          "json",
          "DANGEROUS_CONTENT",
        );
      }
    ...
    try {
      ...
      if (this.hasPrototypePollution(parsed)) {
        throw new ValidationError(
          "JSON contains prototype pollution attempt",
          "json",
          "PROTOTYPE_POLLUTION",
        );
      }
...
export const validateUuid = (id: string) => inputValidator.validateUuid(id);
...
export const validateImage = (file: File) => inputValidator.validateImage(file);
