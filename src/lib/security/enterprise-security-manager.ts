...
class EnterpriseSecurityManager {
  ...
  static getInstance(): EnterpriseSecurityManager {
    if (!EnterpriseSecurityManager.instance) {
      throw new Error(
        "EnterpriseSecurityManager not initialized. Call initialize() first.",
      );
    }
    return EnterpriseSecurityManager.instance;
  }
      ...
      for (const pattern of sqlPatterns) {
        if (pattern.test(input)) {
          ...
          this.createThreat(
            "sql_injection",
            "high",
            (context.ipAddress as string) || "unknown",
            {
    ...
    switch (operation) {
      case "transfer_funds":
        if (
          ...
          !userContext.permissions.includes("high_value_transfer")
        ) {
      ...
      case "delete_property":
        if (
          !userContext.permissions.includes("property:delete") &&
          !userContext.permissions.includes("admin:*")
        ) {
  ...
  validateSecurityConfiguration(): { isSecure: boolean; issues: string[] } {
    ...
    if (this.config.content.maxFileUploadSize > 50 * 1024 * 1024) {
      // 50MB
   ...
   */
  scanForVulnerableComponents(): Promise<{
        ...
        try {
          ...
          if (vulnerabilities.length > 0) {
            this.logSecurityEvent("vulnerable_components_detected", "high", {
              ...
              highCount: vulnerabilities.filter((v) => v.severity === "high")
                .length,
            });
          }
        } catch (error) {
  ...
  validateAuthentication(
    ...
    if (credentials.password) {
      const passwordStrength = this.validatePasswordStrength(
  ...
  getSecurityMetrics(): SecurityMetrics {
    ...
    const recentThreats = Array.from(this.threats.values()).filter(
      (threat) => threat.timestamp > last24Hours,
    );

    const threatsByType = recentThreats.reduce(
      (acc, threat) => {
    ...
    const topThreatTypes = Object.entries(threatsByType)
      .map(([type, count]) => ({
        type,
        count,
        trend: this.calculateThreatTrend(type) as
          | "increasing"
          | "decreasing"
          | "stable",
      }))
    ...
    const attackSources = recentThreats.reduce(
      (acc, threat) => {
    ...
    return {
      ...
      threatsBlocked: recentThreats.filter((t) => t.status === "mitigated")
        .length,
      falsePositives: recentThreats.filter((t) => t.status === "false_positive")
        .length,
  ...
  private detectPrivilegeEscalation(
    ...
    const isHighPrivilegeResource = highPrivilegeResources.some((r) =>
      resource.includes(r),
    );

    if (
      isHighPrivilegeResource &&
      !userContext.permissions.includes("admin:*")
    ) {
      return true;
    }
  ...
  private detectSuspiciousAuthPattern(
    username: string,
    context: { ipAddress: string; userAgent: string },
  ): boolean {
