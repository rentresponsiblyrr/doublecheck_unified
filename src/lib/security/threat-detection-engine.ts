...
class ThreatDetectionEngine {
  ...
  static getInstance(): ThreatDetectionEngine {
    if (!ThreatDetectionEngine.instance) {
      throw new Error('ThreatDetectionEngine not initialized. Call initialize() first.');
    }
    return ThreatDetectionEngine.instance;
  }
    ...
    return enterpriseServiceTracer.traceServiceOperation(
      ...
      async () => {
        ...
        for (const vector of this.threatVectors) {
          ...
          if (score > 0.1) { // Only consider meaningful scores
        ...
        const isAnomaly = anomalyScore > this.config.behavioral.anomalyThreshold;
        
        // Determine severity
        const severityLevel = this.determineSeverityLevel(anomalyScore);
        
        // Generate explanation
        const explanation = this.generateAnomalyExplanation(threatScores, event);
        
        // Generate recommended actions
        const recommendedActions = this.generateRecommendedActions(anomalyScore, activeThreatVectors);
        
        // Calculate confidence based on profile maturity
        const confidence = this.calculateConfidence(profile, activeThreatVectors.length);
  ...
  private initializeThreatVectors(): void {
    this.threatVectors = [
          ...
          if (!isTypicalHour) {
            const hourDistance = Math.min(
              ...profile.typicalLoginHours.map(h => 
                Math.min(Math.abs(h - currentHour), 24 - Math.abs(h - currentHour))
              )
            );
      ...
      // Volume-based anomaly detection
      {
        ...
        detector: (event, profile) => {
          ...
          const deviationRatio = currentVolume / profile.typicalRequestVolume.avg;
          
          if (deviationRatio > 3) { // 3x normal volume
      ...
      // Behavioral pattern anomaly
      {
        ...
        detector: (event, profile) => {
          ...
          if (event.responseTime && operationData.avgDuration > 0) {
            const timeDeviationRatio = event.responseTime / operationData.avgDuration;
      ...
      // Technical anomaly detection
      {
        ...
        detector: (event, profile) => {
          ...
          if (event.requestSize && event.requestSize > 10 * 1024 * 1024) { // 10MB
            score += 0.4;
          }
          
          // Check for unusual user agent
          if (event.userAgent && !profile.typicalUserAgents.has(event.userAgent)) {
      ...
      // Session anomaly detection
      {
        ...
        detector: (event, profile) => {
          ...
          if (uniqueIPs.size > 3) { // More than 3 concurrent locations
            return 0.8;
          }
    ...
    log.info('Threat detection vectors initialized', {
   ...
   */
  private computeWeightedAnomalyScore(threatScores: Map<string, number>): number {
    let weightedSum = 0;
    let totalWeight = 0;
  ...
  private async updateUserProfile(
    ...
    if (event.operation) {
      const existing = profile.commonOperations.get(event.operation) || { frequency: 0, avgDuration: 0 };
      existing.frequency++;
  ...
  private startRealTimeProcessing(): void {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        this.processingQueue = this.eventQueue.splice(0, this.config.realtime.batchSize);
        ...
        log.error('Real-time threat processing failed', error as Error, {
          component: 'threat-detection-engine',
  ...
  private async processBatch(events: SecurityEvent[]): Promise<void> {
    ...
    this.apm.recordTiming('threat_detection.batch_processing_time', processingTime, {
  ...
  getThreatIntelligence(): {
    ...
    const highRiskProfiles = Array.from(this.userProfiles.values())
      .filter(p => p.currentRisk > this.config.scoring.escalationThresholds.high);
    
    const newUsers = Array.from(this.userProfiles.values())
      .filter(p => Date.now() - p.createdAt < 86400000); // 24 hours
  ...
  // Helper methods
  private determineSeverityLevel(anomalyScore: number): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private generateAnomalyExplanation(threatScores: Map<string, number>, event: SecurityEvent): string {
    ...
    return `Anomalies detected: ${significantVectors.map(([name, score]) => 
      `${name} (${Math.round(score * 100)}%)`
    ).join(', ')}`;
  }

  private generateRecommendedActions(anomalyScore: number, threatVectors: string[]): string[] {
  ...
  private calculateConfidence(profile: UserBehaviorProfile, vectorCount: number): number {
    // Base confidence on profile maturity and number of threat vectors
    const profileMaturity = Math.min(1, (Date.now() - profile.createdAt) / (7 * 24 * 60 * 60 * 1000)); // 7 days
  ...
  private recordThreatMetrics(event: SecurityEvent, result: AnomalyDetectionResult): void {
    ...
    if (result.isAnomaly) {
      this.apm.recordGauge('threat_detection.anomaly_score', result.anomalyScore, {
  ...
  private logThreatDetection(event: SecurityEvent, result: AnomalyDetectionResult): void {
    log.warn('Threat anomaly detected', {
      component: 'threat-detection-engine',
      eventId: event.id,
      userId: event.userId,
  ...
  private async loadHistoricalProfile(profile: UserBehaviorProfile): Promise<void> {
  ...
  private getActiveSessions(userId: string): Array<{ sessionId: string; ipAddress: string }> {
