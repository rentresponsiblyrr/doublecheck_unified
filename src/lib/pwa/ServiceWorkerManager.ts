...
export class ServiceWorkerManager {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Service Worker initialization failed', { error }, 'SERVICE_WORKER');
      return false;
    }
  }
  ...
  private setupUpdateHandling(): void {
    ...
    this.registration.addEventListener('updatefound', () => {
      ...
      if (newWorker) {
        logger.info('New Service Worker found, installing...', {}, 'SERVICE_WORKER');
        ...
        newWorker.addEventListener('statechange', () => {
          if (newWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              ...
              logger.info('Service Worker update available', {}, 'SERVICE_WORKER');
            } else {
              // First install
              logger.info('Service Worker installed for first time', {}, 'SERVICE_WORKER');
            }
          }
        });
      }
  ...
  private handleServiceWorkerMessage(data: any): void {
    switch (data.type) {
      ...
      case 'CACHE_UPDATED':
        ...
        logger.info('Cache updated in background', { url: data.url }, 'SERVICE_WORKER');
        break;
      
      default:
        logger.warn('Unknown SW message type', { type: data.type }, 'SERVICE_WORKER');
    }
  }
  ...
  private async setupBackgroundSync(): Promise<void> {
    ...
    try {
      ...
      logger.error('Background sync registration failed', { error }, 'SERVICE_WORKER');
    }
  }
  ...
  private async precacheCriticalResources(): Promise<void> {
    ...
    logger.info('Critical resources precaching initiated', {
  ...
  async queueBackgroundSync(task: Omit<BackgroundSyncTask, 'id' | 'timestamp' | 'retryCount'>): Promise<string> {
    ...
    logger.info('Background sync task queued', {
      taskId: syncTask.id,
      type: syncTask.type,
  ...
  private handleBackgroundSyncSuccess(taskId: string): void {
    ...
    logger.info('Background sync completed successfully', { taskId }, 'SERVICE_WORKER');
  ...
  private handleBackgroundSyncFailure(taskId: string, error: any): void {
    ...
    if (task) {
      ...
      if (task.retryCount >= task.maxRetries) {
        ...
        logger.error('Background sync task failed permanently', {
          taskId,
        ...
        logger.warn('Background sync task failed, will retry', {
          taskId,
          retryCount: task.retryCount,
   ...
   */
  private updatePerformanceMetrics(type: 'hit' | 'miss', responseTime: number): void {
    if (type === 'hit') {
  ...
  private startPerformanceMonitoring(): void {
    ...
    setInterval(() => {
      const total = this.performanceMetrics.hitRate + this.performanceMetrics.missRate;
      if (total > 0) {
        const hitRate = (this.performanceMetrics.hitRate / total) * 100;
        
        logger.info('Service Worker performance metrics', {
          hitRate: Math.round(hitRate),
          avgResponseTime: Math.round(this.performanceMetrics.averageResponseTime),
          cacheSize: Math.round(this.performanceMetrics.cacheSize / 1024 / 1024), // MB
  ...
  private async checkForUpdates(): Promise<void> {
    ...
    try {
      ...
      logger.error('Service Worker update check failed', { error }, 'SERVICE_WORKER');
    }
  }
  ...
  async clearAllCaches(): Promise<void> {
    ...
    logger.info('All caches cleared', { count: cacheNames.length }, 'SERVICE_WORKER');
  }
