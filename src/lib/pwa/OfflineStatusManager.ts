...
export interface OfflineEvent {
  type: 'went_offline' | 'came_online' | 'connection_degraded' | 'connection_improved' | 'sync_started' | 'sync_completed' | 'sync_failed';
  timestamp: Date;
...
export class OfflineStatusManager {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      logger.info('Offline Status Manager initialized successfully', {
      ...
      logger.error('Offline Status Manager initialization failed', { error }, 'OFFLINE_MANAGER');
      return false;
    }
  }
   ...
   */
  async addToRetryQueue(item: Omit<RetryQueueItem, 'id' | 'timestamp' | 'retryCount' | 'nextRetryTime'>): Promise<string> {
    ...
    if (this.networkStatus.isOnline && (item.priority === 'critical' || item.priority === 'high')) {
  ...
  getRetryQueueStatus(): RetryQueueStatus {
    const itemsByPriority = this.retryQueue.reduce((acc, item) => {
      acc[item.priority] = (acc[item.priority] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const itemsByType = this.retryQueue.reduce((acc, item) => {
      acc[item.type] = (acc[item.type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const oldestItem = this.retryQueue.length > 0 ? 
      Math.min(...this.retryQueue.map(item => item.timestamp)) : null;

    const totalDataUsage = this.retryQueue.reduce((sum, item) => sum + item.estimatedDataUsage, 0);
    ...
    return {
      ...
      readyToProcess: this.retryQueue.filter(item => item.nextRetryTime <= Date.now()).length
    };
  }
  ...
  private async handleNetworkStatusChange(isOnline: boolean): Promise<void> {
    ...
    if (isOnline && !wasOnline) {
      ...
      if (this.networkStatus.lastOfflineTime) {
        const downtime = now.getTime() - this.networkStatus.lastOfflineTime.getTime();
      ...
      logger.info('Network connection restored', {
        ...
        downtime: this.networkStatus.lastOfflineTime ? 
          now.getTime() - this.networkStatus.lastOfflineTime.getTime() : 0
      }, 'OFFLINE_MANAGER');
      ...
      if (this.networkStatus.lastOnlineTime) {
        const uptime = now.getTime() - this.networkStatus.lastOnlineTime.getTime();
      ...
      logger.warn('Network connection lost', {
        ...
        uptime: this.networkStatus.lastOnlineTime ? 
          now.getTime() - this.networkStatus.lastOnlineTime.getTime() : 0
      }, 'OFFLINE_MANAGER');
  ...
  private async calculateNetworkQuality(): Promise<void> {
    ...
    const recommendations = this.generateQualityRecommendations(factors, category);
    ...
    logger.debug('Network quality assessed', {
      score: finalScore,
      category,
   ...
   */
  private generateQualityRecommendations(factors: QualityFactor[], category: NetworkQuality['category']): string[] {
    ...
    factors.forEach(factor => {
      if (factor.impact === 'critical' || factor.impact === 'high') {
        switch (factor.factor) {
          case 'latency':
            recommendations.push('High latency detected - reduce real-time operations');
            break;
          case 'bandwidth':
            recommendations.push('Low bandwidth - compress uploads and batch operations');
            break;
          case 'stability':
            recommendations.push('Unstable connection - enable aggressive retry mechanisms');
            break;
          case 'packet_loss':
            recommendations.push('Packet loss detected - use smaller data chunks');
            break;
        }
      }
    });
    ...
    switch (category) {
      case 'poor':
      case 'unusable':
        recommendations.push('Poor network quality - work offline when possible');
        recommendations.push('Enable data saving mode');
        break;
      case 'fair':
        recommendations.push('Moderate network quality - batch non-critical operations');
        break;
      case 'good':
        recommendations.push('Good network quality - normal operations recommended');
        break;
      case 'excellent':
        recommendations.push('Excellent network quality - all operations optimal');
        break;
    }
  ...
  private selectOptimalSyncStrategy(): SyncStrategy {
    return this.syncStrategies.find(strategy => 
  ...
  private async processBatch(items: RetryQueueItem[], strategy: SyncStrategy): Promise<BatchResult> {
    let succeeded = 0;
    let failed = 0;
  ...
  private async processQueueItem(item: RetryQueueItem, strategy: SyncStrategy): Promise<boolean> {
    try {
  ...
  private handleItemFailure(item: RetryQueueItem, strategy: SyncStrategy): void {
    item.retryCount++;
    item.nextRetryTime = Date.now() + (strategy.retryDelay * Math.pow(2, item.retryCount - 1));
    
    if (item.retryCount >= item.maxRetries) {
      logger.error('Queue item exceeded max retries', {
        itemId: item.id,
  ...
  private shouldPauseProcessing(): boolean {
    return !this.networkStatus.isOnline || this.networkStatus.quality.score < 0.2;
  }
  ...
  private notifyListeners(event: any): void {
    this.listeners.forEach(callback => {
      try {
        ...
        logger.error('Listener notification failed', { error }, 'OFFLINE_MANAGER');
      }
    });
  }

  private updateConnectionStability(duration: number, type: 'uptime' | 'downtime'): void {
    const stability = this.networkStatus.connectionStability;
    ...
    stability.stabilityScore = totalTime > 0 ? stability.averageUptime / totalTime : 1.0;
  }
  ...
  private async loadPersistedQueue(): Promise<void> {
    ...
    if (stored) {
      try {
        ...
        logger.error('Failed to load persisted queue', { error }, 'OFFLINE_MANAGER');
      }
    }
  }
  ...
  private async persistQueue(): Promise<void> {
    ...
    localStorage.setItem('str_certified_retry_queue', JSON.stringify(this.retryQueue));
  }
