...
class CacheWarmingSystem {
  ...
  async predictiveWarming(userId: string): Promise<void> {
    ...
    const warmingContext: WarmingContext = {
      ...
      trigger: 'predictive',
      metadata: {
        patterns: relevantPatterns.map(p => ({ path: p.path, frequency: p.frequency })),
    ...
    const predictiveRules = Array.from(this.rules.values()).filter(
      rule => rule.trigger === 'predictive' || rule.warmingStrategy === 'predictive'
    );
  ...
  trackUserBehavior(
    ...
    const existingPattern = userPattern.accessPatterns.find(p => p.path === action.path);
   ...
   */
  private isRuleApplicable(rule: CacheWarmingRule, context: WarmingContext): boolean {
    // Check user roles
   ...
   */
  private calculatePredictionConfidence(patterns: UserBehaviorPattern['accessPatterns']): number {
    if (patterns.length === 0) return 0;
    
    const averageFrequency = patterns.reduce((sum, p) => sum + p.frequency, 0) / patterns.length;
    const recencyScore = patterns.reduce((sum, p) => {
      const daysSinceAccess = (Date.now() - p.lastAccess) / (1000 * 60 * 60 * 24);
  ...
  private updateAverageWarmingTime(duration: number): void {
    ...
    this.stats.averageWarmingTime = totalWarmings === 1 
      ? duration 
