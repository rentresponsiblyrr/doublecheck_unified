...
export const useCacheInvalidation = (
      ...
      try {
        await intelligentCacheInvalidation.invalidateRelated(
          ...
          operation,
          {
            ...options,
            reason:
              options.reason || `${operation} operation from ${componentName}`,
          },
        );
      } catch (error) {
  ...
  const cancelScheduledInvalidation = useCallback(
    (scheduleId: string): boolean => {
      return intelligentCacheInvalidation.cancelScheduledInvalidation(
        scheduleId,
      );
    },
    [],
...
export const useCacheInvalidationRules = () => {
  const registerRule = useCallback((rule: CacheInvalidationRule) => {
    try {
      ...
      logger.error("Failed to register cache invalidation rule", {
        rule,
        error,
      });
      throw error;
...
export const useCacheDependencyTracking = (
  ...
  const trackDependency = useCallback(
  ...
  const invalidateWithDependencies = useCallback(
    async (
      operation: "create" | "update" | "delete",
      options: {
        reason?: string;
