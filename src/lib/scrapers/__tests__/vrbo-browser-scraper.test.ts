...
import {
  VRBOBrowserScraper,
  createVRBOBrowserScraper,
  scrapeBrowserVRBOProperty,
} from "../vrbo-browser-scraper";
...
describe("VRBOBrowserScraper", () => {
  ...
  describe("Browser Session Management", () => {
    it("should create and close browser session successfully", async () => {
      ...
      expect(mockBrowserManager.closeSession).toHaveBeenCalledWith(
        "test-session-123",
      );
    }, 30000); // 30 second timeout for browser tests

    it("should handle browser session creation failure", async () => {
      mockBrowserManager.createSession.mockRejectedValue(
    ...
    it("should clean up session even on error", async () => {
      mockGalleryAutomation.loadAllGalleryImages.mockRejectedValue(
        new Error("Gallery loading failed"),
      );
  ...
  describe("Gallery Automation Integration", () => {
    it("should successfully load gallery images", async () => {
      ...
      expect(result.data?.galleryImages[0].url).toBe(
        "https://images.vrbo.com/gallery1.jpg",
      );
    ...
    it("should handle gallery automation errors gracefully", async () => {
      ...
      expect(result.data?.galleryLoadingResult.errors).toContain(
        "Gallery modal failed to open",
      );
    });
  ...
  describe("Static Fallback Integration", () => {
    it("should use static fallback when enabled", async () => {
      ...
      browserScraper.scrapeComprehensiveProperty = vi
        .fn()
        .mockResolvedValue(mockComprehensiveResult);

      const result = await browserScraper.scrapeWithBrowserAutomation(
        mockVRBOUrl,
        {
    ...
    it("should fallback to static when browser automation fails", async () => {
      ...
      expect(result.data?.staticImages).toHaveLength(1);
      expect(result.data?.galleryLoadingResult.errors).toContain(
        "Browser automation failed, used static fallback",
      );
    });
  });
...
describe("Convenience Functions", () => {
  ...
  describe("scrapeBrowserVRBOProperty", () => {
    it("should scrape property and cleanup automatically", async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockScraper.scrapeWithBrowserAutomation).toHaveBeenCalledWith(
        mockVRBOUrl,
        {},
      );
    ...
    it("should cleanup even on error", async () => {
      const mockScraper = {
        scrapeWithBrowserAutomation: vi
          .fn()
