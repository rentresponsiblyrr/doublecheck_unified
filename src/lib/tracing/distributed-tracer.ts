...
import { correlationManager, type CorrelationContext, type TraceSpan } from './correlation-manager';
...
class DistributedTracer {
  ...
  static getInstance(): DistributedTracer {
    if (!DistributedTracer.instance) {
      throw new Error('DistributedTracer not initialized. Call initialize() first.');
    }
    return DistributedTracer.instance;
  }
  ...
  async trace<T>(
    ...
    try {
      const result = await correlationManager.withCorrelation(context, async () => {
  ...
  async traceChild<T>(
    ...
    const childContext = correlationManager.createChildSpan(operationName, component);
    if (!childContext) {
    ...
    try {
      return await correlationManager.withCorrelation(childContext, async () => {
        return await operation(childContext);
      });
    } catch (error) {
  ...
  async exportTraces(): Promise<void> {
    ...
    try {
      ...
      log.error('Failed to export traces', error as Error, {
        component: 'DistributedTracer',
        action: 'exportTraces',
        traceCount: traces.length,
      }, 'TRACE_EXPORT_FAILED');
  ...
  private updateMetrics(context: CorrelationContext, success: boolean): void {
    ...
    if (!success) {
      ...
      metrics.errorRate = (metrics.errorRate * (metrics.count - 1)) / metrics.count;
    }
        ...
        for (const entry of entries) {
          if (entry.entryType === 'gc') {
            ...
            this.serviceMetrics.gcMetrics.avgTime = 
              this.serviceMetrics.gcMetrics.totalTime / this.serviceMetrics.gcMetrics.collections;
          }
        }
      });
  ...
  private generatePerformanceInsights(): string[] {
    ...
    if (slowOps.length > 0) {
      insights.push(`${slowOps.length} operations are averaging over 2 seconds response time`);
    }
    
    // Check for high error rates
    const errorProneOps = metrics.filter(m => m.errorRate > 0.05);
    if (errorProneOps.length > 0) {
      insights.push(`${errorProneOps.length} operations have error rates above 5%`);
    }
