...
export async function createInspectionAtomic(
  ...
  success: boolean;
  data?: { inspection_id: string; checklist_item_ids: string[] };
  error?: string;
}> {
  try {
    logger.info(
      "Creating inspection atomically",
      { propertyId: data.inspection.property_id },
      "ATOMIC_OPERATIONS",
    );
    ...
    const checklistItems: ChecklistItemInsert[] = data.checklist_items.map(
      (item) => ({
        ...item,
    ...
    if (itemsError) {
      ...
      logger.error(
        "Failed to create checklist items, rolled back inspection",
        itemsError,
        "ATOMIC_OPERATIONS",
      );
    ...
    logger.info(
      "Successfully created inspection atomically",
      {
...
export async function updateChecklistItemWithMediaAtomic(
  ...
  success: boolean;
  data?: { media_ids: string[] };
  error?: string;
}> {
  try {
    logger.info(
      "Updating checklist item with media atomically",
      {
    ...
    if (data.media && data.media.length > 0) {
      ...
      if (mediaError) {
        ...
        logger.error(
          "Failed to insert media files after checklist update",
          mediaError,
          "ATOMIC_OPERATIONS",
        );
        return {
          success: false,
          error: `Checklist item updated but media insertion failed: ${mediaError.message}`,
        };
      }
    ...
    logger.info(
      "Successfully updated checklist item with media atomically",
      {
        checklistItemId: data.checklist_item_id,
    ...
    const errorObj = error as AtomicOperationError;
    logger.error(
      "Atomic checklist item update failed",
      errorObj,
      "ATOMIC_OPERATIONS",
    );
    return { success: false, error: errorObj.message };
...
export async function deleteInspectionAtomic(
  ...
  try {
    logger.info(
      "Deleting inspection atomically",
      { inspectionId },
      "ATOMIC_OPERATIONS",
    );
    ...
    if (fetchError) {
      logger.error(
        "Failed to fetch checklist items for deletion",
        fetchError,
        "ATOMIC_OPERATIONS",
      );
    ...
    if (itemsDeleteError) {
      logger.error(
        "Failed to delete checklist items",
        itemsDeleteError,
        "ATOMIC_OPERATIONS",
      );
    ...
    logger.info(
      "Successfully deleted inspection atomically",
      {
        inspectionId,
    ...
    const errorObj = error as AtomicOperationError;
    logger.error(
      "Atomic inspection deletion failed",
      errorObj,
      "ATOMIC_OPERATIONS",
    );
    return { success: false, error: errorObj.message };
...
export async function batchUpdateChecklistItemsAtomic(
  ...
  try {
    logger.info(
      "Batch updating checklist items atomically",
    ...
    for (const update of updates) {
      ...
      if (error) {
        ...
        logger.error(
          "Failed to update checklist item in batch",
          error,
          "ATOMIC_OPERATIONS",
        );
      } else {
    ...
    if (errors.length > 0) {
      ...
      logger.error(
        "Batch update had errors",
        { successCount, totalCount: updates.length, errors },
        "ATOMIC_OPERATIONS",
      );
    ...
    logger.info(
      "Successfully batch updated checklist items atomically",
...
export async function createMediaUploadAtomic(
  ...
  try {
    logger.info(
      "Creating media upload atomically",
      { checklistItemId, mediaType: mediaData.type },
      "ATOMIC_OPERATIONS",
    );
    ...
    if (itemStatusUpdate) {
      ...
      if (updateError) {
        ...
        logger.error(
          "Failed to update checklist item after media creation, rolled back media",
          updateError,
          "ATOMIC_OPERATIONS",
        );
        return { success: false, error: updateError.message };
    ...
    logger.info(
      "Successfully created media upload atomically",
      {
        checklistItemId,
        mediaId: media.id,
      },
    ...
    const errorObj = error as AtomicOperationError;
    logger.error(
      "Atomic media upload creation failed",
      errorObj,
      "ATOMIC_OPERATIONS",
    );
    return { success: false, error: errorObj.message };
