...
export class EnterpriseInspectionCreationService {
  ...
  public static getInstance(): EnterpriseInspectionCreationService {
    if (!EnterpriseInspectionCreationService.instance) {
      EnterpriseInspectionCreationService.instance = new EnterpriseInspectionCreationService();
      
      logger.info('EnterpriseInspectionCreationService initialized', {
        performanceThreshold: EnterpriseInspectionCreationService.instance.performanceThreshold,
        retryAttempts: EnterpriseInspectionCreationService.instance.retryAttempts
   ...
   */
  async createInspection(request: InspectionCreationRequest): Promise<InspectionCreationResult> {
    const overallStartTime = performance.now();
    let validationTime = 0;
    let databaseTime = 0;

    try {
      logger.info('Starting enterprise inspection creation', {
      ...
      try {
        ...
        logger.debug('Property ID converted successfully', {
      ...
      const rpcResult = await this.executeRpcWithRetry(request.propertyId, databasePropertyId, inspectorId, request.status);
      ...
      if (totalProcessingTime > this.performanceThreshold) {
        logger.warn('Inspection creation exceeded performance threshold', {
      ...
      return {
        ...
        data: {
          ...
          propertyId: inspectionData.property_id as DatabasePropertyId,
      ...
      logger.error('Inspection creation failed with unexpected error', {
  ...
  private async executeRpcWithRetry(
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        logger.debug(`RPC execution attempt ${attempt}/${this.retryAttempts}`, {
          propertyUuid,
          propertyId,
          status
        }, 'INSPECTION_CREATION_SERVICE');
        ...
        if (rpcError) {
          ...
          if (isNonRetryable || attempt === this.retryAttempts) {
            return this.handleRPCError(rpcError, { propertyUuid, propertyId, inspectorId, status });
          }
        ...
        return {
          success: true,
          data: rpcResult[0] as RpcFunctionResult
        };
   ...
   */
  private async validateCreationRequest(request: InspectionCreationRequest): Promise<{
    isValid: boolean;
    ...
    // Status validation
    if (request.status && !['draft', 'in_progress', 'completed', 'auditing'].includes(request.status)) {
   ...
   */
  private handleRPCError(rpcError: any, context: Record<string, unknown>): InspectionCreationResult {
    logger.error('RPC function error with context', {
      error: rpcError,
      code: rpcError.code,
    ...
    if (rpcError.message?.includes('INSPECTOR_INVALID') || rpcError.message?.includes('INSPECTOR_REQUIRED')) {
      return this.createErrorResult(
    ...
    if (rpcError.message?.includes('function "create_inspection_compatibility" does not exist')) {
      return this.createErrorResult(
        InspectionErrorCode.RPC_FUNCTION_MISSING,
   ...
   */
  private async resolveInspectorId(providedInspectorId?: InspectorId): Promise<string> {
    if (providedInspectorId) {
      logger.debug('Using provided inspector ID', { 
        inspectorId: '***' 
      }, 'INSPECTION_CREATION_SERVICE');
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to get authenticated user', { error }, 'INSPECTION_CREATION_SERVICE');
      ...
      logger.debug('Using authenticated user as inspector', { 
        userId: user.id 
      }, 'INSPECTION_CREATION_SERVICE');
      ...
      logger.error('Unexpected error during user resolution', { error }, 'INSPECTION_CREATION_SERVICE');
      throw new InspectionCreationError(
  ...
  private isValidUUID(value: string): boolean {
    ...
    return typeof value === 'string' && value.length === 36 && uuidRegex.test(value);
  }
 ...
 */
export const inspectionCreationService = EnterpriseInspectionCreationService.getInstance();
...
export const isValidInspectionStatus = (value: unknown): value is 'draft' | 'in_progress' | 'completed' | 'auditing' => {
