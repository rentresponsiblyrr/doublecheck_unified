...
export class AccessibilityTester {
  ...
  async auditElement(element: HTMLElement): Promise<AccessibilityTestResult> {
    ...
    const score = Math.max(0, Math.round((compliantElements / totalElements) * 100));
    ...
    interactiveElements.forEach((el, index) => {
      ...
      if (tabIndex && parseInt(tabIndex) > 0) {
        this.addViolation({
          type: 'warning',
          rule: 'keyboard-trap',
          description: 'Positive tabindex values can create keyboard navigation issues',
          element: this.getElementSelector(htmlEl),
          impact: 'moderate',
          wcagLevel: 'A',
          wcagCriterion: '2.1.1',
      ...
      const hasCustomFocus = computedStyle.outline !== 'none' || computedStyle.boxShadow !== 'none';
  ...
  private async testFormLabels(element: HTMLElement): Promise<void> {
    ...
    formControls.forEach(control => {
      ...
      const hasLabel = element.querySelector(`label[for="${id}"]`) ||
  ...
  private async testLandmarks(element: HTMLElement): Promise<void> {
    const landmarks = element.querySelectorAll('main, nav, header, footer, aside, [role="main"], [role="navigation"], [role="banner"], [role="contentinfo"], [role="complementary"]');
    ...
    modals.forEach(modal => {
      ...
      if (!hasTabindex || parseInt(hasTabindex) !== -1) {
        this.addViolation({
          ...
          description: 'Modal dialog should have tabindex="-1" for focus management',
  ...
  private async testTouchTargets(element: HTMLElement): Promise<void> {
    const touchTargets = element.querySelectorAll('button, [role="button"], a, input, select');
   ...
   * Helper methods
   */
  private addViolation(violation: Omit<AccessibilityViolation, 'id' | 'timestamp'>) {
 ...
 */
export const quickAccessibilityTest = async (elementId: string = 'root'): Promise<AccessibilityTestResult> => {
...
export const useAccessibilityTest = () => {
  const runTest = async (element?: HTMLElement): Promise<AccessibilityTestResult> => {
