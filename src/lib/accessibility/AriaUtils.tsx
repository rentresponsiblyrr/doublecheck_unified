...
export const convertAriaProps = (props: AriaProps): Record<string, any> => {
  ...
  Object.entries(props).forEach(([key, value]) => {
    ...
    const htmlKey = key === 'role' || key === 'tabIndex' ? key : `aria-${ariaKey.replace('aria-', '')}`;
...
export const useStatusAnnouncer = () => {
  ...
  const announceStatus = useCallback((message: string, priority: AriaLivePriority = 'polite') => {
    setStatus(message);
 ...
 */
interface AccessibleButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, AriaProps {
...
export const AccessibleButton: React.FC<AccessibleButtonProps> = ({
  ...
  const handleClick = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const buttonText = typeof children === 'string' ? children : ariaProps.ariaLabel || 'Button';
...
export const AccessibleField: React.FC<AccessibleFieldProps> = ({
  ...
  return (
    <div className={`accessible-field ${error ? 'accessible-field--error' : ''}`}>
      <label
        htmlFor={id}
...
export const AccessibleTabs: React.FC<AccessibleTabsProps> = ({
  ...
  return (
    <div className={`accessible-tabs accessible-tabs--${orientation} ${className}`}>
      <div
        role="tablist"
...
export const screenReaderUtils = {
  ...
  visuallyHidden: (text: string, focusable = false) => (
    <span className={`visually-hidden ${focusable ? 'visually-hidden-focusable' : ''}`}>
      {text}
    </span>
  ),
   ...
   */
  formatNumber: (num: number, type: 'currency' | 'percentage' | 'decimal' = 'decimal') => {
