...
export const KeyboardNavigation: React.FC<KeyboardNavigationProps> = ({
  ...
  const { announce, handleKeyboardNavigation, registerKeyboardShortcut, trapFocus } = useAccessibility();
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (arrowKeys && ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) {
  ...
  }, [arrowKeys, escapeKey, enterKey, spaceKey, shortcuts, handleKeyboardNavigation]);
  
  // Arrow key navigation logic
  const handleArrowNavigation = useCallback((event: React.KeyboardEvent, container: HTMLElement) => {
    event.preventDefault();
  ...
  // Escape key logic
  const handleEscapeKey = useCallback((event: React.KeyboardEvent, container: HTMLElement) => {
    // Close modals, dropdowns, etc.
    const closeButtons = container.querySelectorAll<HTMLElement>('[aria-label*="close"], [aria-label*="Close"]');
    ...
    if (restoreFocus) {
      event.preventDefault();
      const parentFocusable = container.closest<HTMLElement>('[tabindex], button, [href]');
      if (parentFocusable) {
  ...
  // Enter key logic  
  const handleEnterKey = useCallback((event: React.KeyboardEvent, container: HTMLElement) => {
  ...
  // Space key logic
  const handleSpaceKey = useCallback((event: React.KeyboardEvent, container: HTMLElement) => {
    const activeElement = document.activeElement as HTMLElement;
...
export const useFocusManagement = () => {
  ...
  const cycleFocus = useCallback((containerId: string, direction: 'forward' | 'backward' = 'forward') => {
    const container = document.getElementById(containerId);
    if (!container) return;
 ...
 */
export const useRovingTabindex = (containerId: string, orientation: 'horizontal' | 'vertical' | 'both' = 'both') => {
  useEffect(() => {
    const container = document.getElementById(containerId);
    if (!container) return;
