...
interface AccessibilityContextType {
  ...
  reportAccessibilityError: (
    error: Omit<
      AccessibilityState["accessibilityErrors"][0],
      "id" | "timestamp"
    >,
  ) => void;
  clearErrors: () => void;
...
export const AccessibilityProvider: React.FC<AccessibilityProviderProps> = ({
      ...
      setState((prev) => {
        ...
        if (typeof window !== "undefined") {
          const settingsToSave = Object.fromEntries(
            Object.entries(DEFAULT_SETTINGS).map(([key]) => [
              key,
              updated[key as keyof AccessibilitySettings],
            ]),
          );
  ...
  // Error reporting
  const reportAccessibilityError = useCallback(
    (
      error: Omit<
        AccessibilityState["accessibilityErrors"][0],
        "id" | "timestamp"
