...
export class EnhancedAIService extends STRCertifiedAIService {
  ...
  async analyzeInspectionPhotoWithContext(
    ...
    try {
      ...
      logger.info(
        "Retrieving CAG context for photo analysis",
        {
      ...
      const enhancedResult: EnhancedAnalysisResult = {
        ...
        context_used: {
          knowledge_sources: cagContext.selected_context.knowledge_entries.map(
            (k) => k.title,
          ),
          applied_patterns: cagContext.selected_context.applied_patterns.map(
            (p) => p.pattern_name,
          ),
      ...
      const basicResult = await this.analyzeInspectionPhoto(
        file,
  ...
  async generateContextualChecklist(
    ...
    try {
      ...
      const baseItems =
        await this.generateDynamicChecklist(enhancedPropertyData);
      ...
      return {
        ...
        context_applied: [
          ...
          ...cagContext.selected_context.applied_patterns.map(
            (p) => p.pattern_name,
          ),
        ],
  ...
  async validateInspectionCompletenessWithContext(
    ...
    try {
      ...
      const baseValidation = await this.validateInspectionCompleteness(
        checklistItems,
        photos,
      );
      ...
      const basic = await this.validateInspectionCompleteness(
        checklistItems,
        photos,
      );
      return {
  ...
  async processAuditorFeedback(
    ...
    try {
      ...
      logger.error(
        "Failed to process auditor feedback",
        error,
        "ENHANCED_AI_SERVICE",
      );
      throw error;
    ...
    return baseItems.map((item) => {
      ...
      cagContext.selected_context.applied_patterns.forEach((pattern) => {
        const priorityAdjustments =
          pattern.context_data.priority_adjustments || {};
      ...
      if (
        inspectionContext.temporal.season === "winter" &&
        item.category === "safety"
      ) {
        modified.description +=
          " Pay special attention to heating safety and ice prevention.";
      }
  ...
  private generateContextualInsights(
    ...
    if (inspectionContext.temporal.season === "winter") {
      insights.push(
        "Winter inspection protocols applied - heating and insulation given priority",
      );
    }
    ...
    if (cagContext.selected_context.applied_patterns.length > 0) {
      insights.push(
        `Applied ${cagContext.selected_context.applied_patterns.length} property-specific inspection patterns`,
      );
    }
    ...
    if (photosPerItem < 1) {
      insights.push(
        "Consider additional photo documentation for thorough inspection",
      );
    }
...
export const createEnhancedAIService = (
