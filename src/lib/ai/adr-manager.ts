...
export class ADRManager {
  ...
  async supersedeADR(
    ...
    const newADRId = await this.createADR({
      ...newADRTemplate,
      related_decisions: [
        ...newADRTemplate.related_decisions,
        `ADR-${oldADR.number.toString().padStart(4, "0")}`,
      ],
    });

    // Update old ADR status
    await this.updateADRStatus(
      oldADRId,
      "superseded",
      `Superseded by new ADR: ${newADRId}`,
    );
  ...
  queryADRs(query: ADRQuery): ADRRecord[] {
    ...
    if (query.ai_agent) {
      filtered = filtered.filter(
        (adr) => adr.ai_agent_info.created_by === query.ai_agent,
      );
    }
...
${adrs
  ...
  .map(
    (adr) =>
      `- [ADR-${adr.number.toString().padStart(4, "0")}](${path.basename(adr.file_path)}) - ${adr.title} (${adr.ai_agent_info.date})`,
  )
  .join("\n")}
...
`;
  ...
  private generateADRFilePath(number: number, title: string): string {
    ...
    return path.join(
      this.adrDirectory,
      `${number.toString().padStart(4, "0")}-${sanitizedTitle}.md`,
    );
  }
...
*Last modified: ${adr.last_modified}*`;
  ...
  private async writeADRFile(filePath: string, content: string): Promise<void> {
    ...
    logger.info(
      `ADR file would be written to: ${filePath}`,
