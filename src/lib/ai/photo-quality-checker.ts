...
export class PhotoQualityChecker {
  ...
  private async validatePhotoBasics(
    photo: File | Blob,
    ...
    if (
      dimensions.width < this.config.minResolution.width ||
    ...
    return {
      ...
      details:
        mockScore > 80
          ...
          : "Some areas appear slightly blurry",
    };
  }
  ...
  private async analyzeLighting(
    ...
    return {
      ...
      details: this.getLightingDetails(
        avgBrightness,
        underexposedRatio,
        overexposedRatio,
      ),
  ...
  private async analyzeComposition(
    ...
    if (!this.ctx || !this.canvas) {
      return {
        score: 0,
        rating: "unacceptable",
        details: "Canvas not available for analysis",
      };
    }
    ...
    const details = this.generateCompositionDetails(
      score,
      ruleOfThirdsScore,
      horizonScore,
      centerBiasScore,
  ...
  private detectQualityIssues(
    ...
    if (lighting.score < this.config.minLighting) {
      const avgBrightness = parseInt(
        lighting.details?.match(/\d+/) || ["0"],
      )[0];
      if (avgBrightness < 80) {
        issues.push({
          type: "underexposed",
    ...
    if (
      image.width < this.config.minResolution.width ||
    ...
    if (motionBlur?.hasMotionBlur) {
      issues.push({
        type: "blur",
        severity:
          motionBlur.severity === "high"
            ? "critical"
            : motionBlur.severity === "medium"
              ? "major"
              : "minor",
  ...
  private generateImprovementSuggestions(
    ...
    if (composition.score < 75) {
      ...
      if (
        composition.details?.includes("horizon") &&
        composition.details?.includes("tilted")
      ) {
        suggestions.push({
  ...
  private generateGuidanceMessages(
  ...
  private isPhotoAcceptable(metrics: PhotoQualityMetrics): boolean {
    return (
      metrics.overall_score >= this.config.minSharpness &&
  ...
  private calculateOverallScore(
    ...
    return Math.round(
      sharpness.score * 0.4 + lighting.score * 0.35 + composition.score * 0.25,
    );
  }
  ...
  private analyzeHorizonLevel(imageData: ImageData): number {
    ...
    for (let y = height * 0.2; y < height * 0.8; y++) {
      ...
      if (avgStrength > 15) {
        // Threshold for significant horizontal line
    ...
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        ...
        const bottom =
          ...
          3;
        const edgeStrength =
          Math.abs(right - current) + Math.abs(bottom - current);
   ...
   */
  private async detectMotionBlur(
    image: HTMLImageElement,
  ): Promise<{ hasMotionBlur: boolean; severity: "low" | "medium" | "high" }> {
    if (!this.ctx || !this.canvas) {
