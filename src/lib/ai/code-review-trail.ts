...
export class AICodeReviewTrail {
  ...
  async startReview(
    ...
    logger.info(
      `Code review started: ${review_type}`,
      {
        review_id: reviewId,
  ...
  async addFinding(finding: Omit<ReviewFinding, "id">): Promise<string> {
    ...
    const fileReview = this.activeReview.files_reviewed.find(
      (f) => f.file_path === finding.file_path,
    );
    if (fileReview) {
    ...
    logger.info(
      `Finding added to review: ${finding.title}`,
      {
  ...
  async addRecommendation(
    recommendation: Omit<ReviewRecommendation, "id">,
  ): Promise<string> {
    if (!this.activeReview) {
      throw new Error("No active review. Start a review first.");
    }
    ...
    logger.info(
      `Recommendation added to review: ${recommendation.title}`,
      {
  ...
  async completeReview(
    ...
    this.activeReview.follow_up_required = this.activeReview.findings.some(
      (f) => f.blocking,
    );
    ...
    logger.info(
      `Code review completed: ${this.activeReview.review_type}`,
      {
        review_id: this.activeReview.id,
  ...
  queryReviews(query: ReviewQuery): CodeReviewEntry[] {
    ...
    if (query.status) {
      filtered = filtered.filter(
        (r) => r.overall_assessment.status === query.status,
      );
    }

    if (query.risk_level) {
      filtered = filtered.filter(
        (r) => r.scope.risk_level === query.risk_level,
      );
    }
    ...
    if (query.approved_only) {
      filtered = filtered.filter(
        (r) => r.overall_assessment.status === "approved",
      );
    }
  ...
  getReviewSummary(): ReviewSummary {
    ...
    const reviewsByType = this.reviews.reduce(
      (acc, r) => {
    ...
    const averageQualityScore =
      totalReviews > 0
        ? this.reviews.reduce(
            (sum, r) => sum + r.overall_assessment.quality_score,
            0,
          ) / totalReviews
        : 0;
    ...
    const averageSecurityScore =
      totalReviews > 0
        ? this.reviews.reduce(
            (sum, r) => sum + r.overall_assessment.security_score,
            0,
          ) / totalReviews
        : 0;
    ...
    const averagePerformanceScore =
      totalReviews > 0
        ? this.reviews.reduce(
            (sum, r) => sum + r.overall_assessment.performance_score,
            0,
          ) / totalReviews
        : 0;
    ...
    const issueCounts = allFindings.reduce(
      (acc, f) => {
    ...
    const totalDuration = this.reviews.reduce(
      (sum, r) => sum + r.review_duration_minutes,
      0,
    );
    const totalFiles = this.reviews.reduce(
      (sum, r) => sum + r.files_reviewed.length,
      0,
    );
    const totalLines = this.reviews.reduce(
      (sum, r) => sum + r.scope.lines_of_code,
      0,
    );
  ...
  private updateOverallAssessment(): void {
    ...
    const criticalFindings = this.activeReview.findings.filter(
      (f) => f.severity === "critical",
    ).length;
    const highFindings = this.activeReview.findings.filter(
      (f) => f.severity === "high",
    ).length;
    const blockingFindings = this.activeReview.findings.filter(
      (f) => f.blocking,
    ).length;
    ...
    // Update risk score
    this.activeReview.overall_assessment.risk_score = Math.min(
      100,
      criticalFindings * 30 +
        highFindings * 15 +
        this.activeReview.findings.length * 5,
    );
  }
  ...
  private calculateFinalMetrics(): void {
    ...
    const avgConfidence =
      this.activeReview.files_reviewed.length > 0
        ? this.activeReview.files_reviewed.reduce(
            (sum, f) => sum + f.ai_confidence,
            0,
          ) / this.activeReview.files_reviewed.length
        : 0;

    this.activeReview.overall_assessment.overall_confidence = avgConfidence;

    // Update quality metrics based on findings
    const qualityIssues = this.activeReview.findings.filter(
      (f) => f.category === "code_quality",
    ).length;
    this.activeReview.overall_assessment.quality_score = Math.max(
      0,
      100 - qualityIssues * 10,
    );

    const securityIssues = this.activeReview.findings.filter(
      (f) => f.category === "security_vulnerability",
    ).length;
    this.activeReview.overall_assessment.security_score = Math.max(
      0,
      100 - securityIssues * 20,
    );

    const performanceIssues = this.activeReview.findings.filter(
      (f) => f.category === "performance_issue",
    ).length;
    this.activeReview.overall_assessment.performance_score = Math.max(
      0,
      100 - performanceIssues * 15,
    );
  }
  ...
  private async persistReview(review: CodeReviewEntry): Promise<void> {
    try {
      const { error } = await supabase.from("ai_code_reviews").insert([
        {
          id: review.id,
      ...
      logger.error(
        "Failed to persist code review to database",
        error,
        "AI_CODE_REVIEW_PERSIST",
      );
      throw error;
    }
  }
...
export const startReview =
  aiCodeReviewTrail.startReview.bind(aiCodeReviewTrail);
export const addFinding = aiCodeReviewTrail.addFinding.bind(aiCodeReviewTrail);
export const addRecommendation =
  aiCodeReviewTrail.addRecommendation.bind(aiCodeReviewTrail);
export const approveCode =
  aiCodeReviewTrail.approveCode.bind(aiCodeReviewTrail);
export const completeReview =
  aiCodeReviewTrail.completeReview.bind(aiCodeReviewTrail);
export const quickReview =
  aiCodeReviewTrail.quickReview.bind(aiCodeReviewTrail);
export const queryReviews =
  aiCodeReviewTrail.queryReviews.bind(aiCodeReviewTrail);
export const getReviewById =
  aiCodeReviewTrail.getReviewById.bind(aiCodeReviewTrail);
export const getReviewSummary =
  aiCodeReviewTrail.getReviewSummary.bind(aiCodeReviewTrail);
export const exportReviews =
  aiCodeReviewTrail.exportReviews.bind(aiCodeReviewTrail);
