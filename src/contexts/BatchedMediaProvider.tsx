...
interface BatchedMediaActions {
  // Core operations
  loadInspectionMedia: (inspectionId: string, force?: boolean) => Promise<BatchLoadResult | null>;
  getMediaForItem: (inspectionId: string, itemId: string) => ChecklistItemWithMedia | null;
...
const BatchedMediaActionsContext = createContext<BatchedMediaActions | null>(null);
...
> {
  ...
  static getDerivedStateFromError(error: Error): Partial<BatchedMediaErrorBoundaryState> {
    const now = Date.now();
  ...
  private scheduleErrorReset = () => {
    ...
    const delay = delays[Math.min(this.state.errorCount - 1, delays.length - 1)];
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div id="batched-media-error-fallback" className="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-800">
                Media loading is temporarily using fallback mode. Performance may be reduced.
              </p>
            </div>
          </div>
...
export const BatchedMediaProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const estimateMemoryUsage = useCallback((mediaMap: Map<string, Map<string, ChecklistItemWithMedia>>): number => {
    let totalSize = 0;
  ...
  const loadInspectionMedia = useCallback(async (
    inspectionId: string, 
    force: boolean = false
    ...
    try {
        ...
        data.forEach(row => {
          ...
          if (!itemGroups.has(itemId)) {
            itemGroups.set(itemId, {
              ...
              created_at: row.checklist_item_created_at || new Date().toISOString(),
              media: []
            });
          }
      ...
      setState(prev => {
        ...
        if (newInspectionMedia.size > prev.maxCacheSize) {
          const sortedByAccess = Array.from(newInspectionMedia.keys())
            .sort(); // In production, would sort by last access time
          
          // Remove oldest entries
          const toRemove = sortedByAccess.slice(0, newInspectionMedia.size - prev.maxCacheSize);
        ...
        const avgLoadTime = performanceRef.current.queryTimes.length > 0 ?
          performanceRef.current.queryTimes.reduce((a, b) => a + b, 0) / performanceRef.current.queryTimes.length : 0;
        
        const cacheHitRate = (performanceRef.current.cacheHits + performanceRef.current.cacheMisses) > 0 ?
          performanceRef.current.cacheHits / (performanceRef.current.cacheHits + performanceRef.current.cacheMisses) : 0;
      ...
      logger.info('Elite batched media load completed successfully', {
        ...
        mediaCount: Array.from(itemsMap.values()).reduce((sum, item) => sum + item.media.length, 0),
      ...
      setState(prev => {
        ...
        newErrors.set(inspectionId, error instanceof Error ? error : new Error(errorMessage));
        ...
        const errorRate = (performanceRef.current.errorCount / 
          (performanceRef.current.cacheHits + performanceRef.current.cacheMisses + performanceRef.current.errorCount)) || 0;
      ...
      if (!state.fallbackMode.get(inspectionId)) {
        logger.info('Attempting fallback load', { operationId, inspectionId });
        return await loadInspectionMediaFallback(inspectionId);
      }

      return null;
    }
  }, [state.inspectionMedia, state.fallbackMode, state.retryCount, state.fallbackThreshold, estimateMemoryUsage]);

  // ===== FALLBACK IMPLEMENTATION =====
  
  const loadInspectionMediaFallback = useCallback(async (inspectionId: string): Promise<BatchLoadResult | null> => {
    const startTime = performance.now();
  ...
  const getMediaForItem = useCallback((inspectionId: string, itemId: string): ChecklistItemWithMedia | null => {
  ...
  const actions = useMemo<BatchedMediaActions>(() => ({
    ...
    retryFailedLoad: async (inspectionId: string) => {
      setState(prev => {
        ...
        const newRetryCount = new Map(prev.retryCount);
        newRetryCount.delete(inspectionId);
        return { ...prev, errors: newErrors, fallbackMode: newFallbackMode, retryCount: newRetryCount };
      });
    ...
    getHealthReport: (): HealthReport => {
      ...
      if (state.metrics.averageLoadTime > 1000) {
        ...
        recommendations.push('Consider database optimization or caching improvements');
      }
      
      return {
        status: healthy ? 'healthy' : (issues.length > 2 ? 'critical' : 'degraded'),
        issues,
        recommendations,
  ...
  }, [state.maxCacheSize, state.fallbackThreshold, state.memoryLimit, getPerformanceMetrics, state.inspectionMedia.size, isHealthy]);
...
export const useBatchedMediaState = (): BatchedMediaState => {
  ...
  if (!context) {
    throw new Error('useBatchedMediaState must be used within BatchedMediaProvider');
  }
  return context;
};
...
export const useBatchedMediaActions = (): BatchedMediaActions => {
  ...
  if (!context) {
    throw new Error('useBatchedMediaActions must be used within BatchedMediaProvider');
  }
  return context;
};
...
export const useOptimizedChecklistItemMedia = (
  ...
  useEffect(() => {
    if (!state.inspectionMedia.has(inspectionId) && 
  ...
  }, [inspectionId, state.inspectionMedia, state.loadingInspections, state.retryCount, actions]);
...
const IndividualMediaFallbackProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  logger.warn('Using individual media fallback provider due to error boundary activation');
