...
export const VALID_INSPECTION_STATUSES: InspectionStatus[] =
...
export const needsInspectorAction = (status: string): boolean => {
  return STATUS_GROUPS.NEEDS_INSPECTOR_ACTION.includes(
    status as InspectionStatus,
  );
};

export const needsAuditorAction = (status: string): boolean => {
  return STATUS_GROUPS.NEEDS_AUDITOR_ACTION.includes(
    status as InspectionStatus,
  );
};
...
export const getStatusDisplay = (status: string) => {
  return (
    STATUS_DISPLAY[status as InspectionStatus] || {
      label: "Unknown",
      color: "gray",
      badgeVariant: "outline" as const,
