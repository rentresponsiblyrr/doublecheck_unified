      ...
      if (this.onmessage) {
        const response = {
          data: {
            id: data.id,
            success: true,
            result: data.type === 'COMPRESS_VIDEO' ? {
              compressedFile: new File(['compressed'], 'test.mp4', { type: 'video/mp4' }),
              ...
              compressedFile: new File(['compressed'], 'test.jpg', { type: 'image/jpeg' }),
              ...
              width: 1920,
              height: 1080
            } : data.type === 'SHOULD_COMPRESS' ? true : null
          }
        };
...
describe('MediaCompressionWorker Performance Tests', () => {
  ...
  describe('Video Compression Performance', () => {
    it('should compress large video files in <5 seconds', async () => {
      ...
      const largeVideoBlob = new Blob(['x'.repeat(15 * 1024 * 1024)], { type: 'video/webm' });
      const videoFile = new File([largeVideoBlob], 'large-video.webm', { type: 'video/webm' });
    ...
    it('should maintain UI responsiveness during compression', async () => {
      const videoFile = new File([new Blob(['test'])], 'test.webm', { type: 'video/webm' });
    ...
    it('should handle multiple concurrent compressions', async () => {
      const videoFiles = Array.from({ length: 3 }, (_, i) => 
        new File([new Blob(['test'])], `video-${i}.webm`, { type: 'video/webm' })
      );
  ...
  describe('Photo Compression Performance', () => {
    it('should compress photos in <1 second', async () => {
      const largePhotoBlob = new Blob(['x'.repeat(8 * 1024 * 1024)], { type: 'image/jpeg' });
      const photoFile = new File([largePhotoBlob], 'large-photo.jpg', { type: 'image/jpeg' });
    ...
    it('should batch process multiple photos efficiently', async () => {
      const photoFiles = Array.from({ length: 5 }, (_, i) => 
        new File([new Blob(['test'])], `photo-${i}.jpg`, { type: 'image/jpeg' })
      );
