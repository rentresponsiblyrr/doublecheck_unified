...
import { SecurityEvents, securityLogger } from '../../lib/security/security-audit-logger';
...
describe('Security Test Suite', () => {
  ...
  describe('File Upload Security', () => {
    it('should reject files with malicious extensions', async () => {
      ...
      await expect(SecureFileHandler.validateFile(maliciousFile))
    ...
    it('should detect file signature mismatches', async () => {
      ...
      await expect(SecureFileHandler.validateFile(fakeJpeg))
        .rejects.toThrow('File signature mismatch');
    });
    ...
    it('should detect embedded malicious scripts in images', async () => {
      ...
      const maliciousBytes = new Uint8Array([
        ...
        ...Array.from('<script>alert("XSS")</script>').map(c => c.charCodeAt(0))
      ]);
      const maliciousImage = new File([maliciousBytes], 'image.jpg', { type: 'image/jpeg' });
    ...
    it('should enforce file size limits', async () => {
      ...
      await expect(SecureFileHandler.validateFile(largeFile))
    ...
    it('should sanitize EXIF data from images', async () => {
      ...
      const imageWithExif = new File(['fake jpeg'], 'photo.jpg', { type: 'image/jpeg' });
  ...
  describe('Authentication Security', () => {
    ...
    it('should detect and block brute force attacks', async () => {
      ...
      await expect(authManager.validateSession({ id: userId }))
    ...
    it('should validate session token integrity', async () => {
      ...
      await expect(authManager.validateSession(invalidUser))
  ...
  describe('Worker Security', () => {
    ...
    it('should validate message integrity', async () => {
      ...
      const messagePromise = manager.sendMessage('COMPRESS_MEDIA', { test: 'data' });
    ...
    it('should enforce message timeouts', async () => {
      ...
      const messagePromise = manager.sendMessage('COMPRESS_MEDIA', { test: 'data' }, { timeout: 100 });
    ...
    it('should limit concurrent messages', async () => {
      ...
      expect(errors.some(e => e.message.includes('Too many concurrent'))).toBe(true);
  ...
  describe('Security Event Logging', () => {
    ...
    it('should trigger alerts for critical events', () => {
      ...
      SecurityEvents.malwareDetected('FileUpload', 'virus.exe', ['MALICIOUS_PATTERN']);
        ...
        promises.push(
          Promise.resolve().then(() => {
            ...
            try {
              ...
              SecurityEvents.xssAttemptBlocked('PropertyValidator', 'javascript:void(0)');
            }
