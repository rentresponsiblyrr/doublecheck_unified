...
describe("Inspection Workflow E2E Tests - Professional Standards", () => {
  ...
  describe("Complete Inspection Workflow Journey", () => {
    it("should complete full inspection from start to finish", async () => {
      ...
      expect(
        screen.getByText(/select property for inspection/i),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText("123 Test Street, Test City, TS 12345"),
      ).toBeInTheDocument();

      // Start inspection
      const startButton = screen.getByRole("button", {
        name: /start inspection/i,
      });
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/3 checklist items generated/i),
          ).toBeInTheDocument();
        },
        { timeout: 5000 },
      );
      ...
      for (let i = 0; i < checklistItems.length; i++) {
        ...
        const captureButton = within(itemCard).getByRole("button", {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/video recorded successfully/i),
        ).toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/uploading inspection data/i),
        ).toBeInTheDocument();
      });
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/inspection completed successfully/i),
          ).toBeInTheDocument();
        },
        { timeout: 10000 },
      );
    ...
    it("should handle workflow interruption and resume", async () => {
      ...
      const captureButton = within(firstItem).getByRole("button", {
        name: /capture photo/i,
      });
      await user.click(captureButton);
      ...
      const secondCaptureButton = within(secondItem).getByRole("button", {
        name: /capture photo/i,
      });
  ...
  describe("Mobile Device Compatibility", () => {
    ...
    it("should handle camera permission denial gracefully", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/camera permission required/i),
        ).toBeInTheDocument();
        expect(
          screen.getByText(/please enable camera access/i),
        ).toBeInTheDocument();
      });
    ...
    it("should work in portrait and landscape orientations", async () => {
      ...
      // Test portrait mode
      expect(screen.getByTestId("workflow-container")).toHaveClass(
        "portrait-layout",
      );
  ...
  describe("Offline Mode Support", () => {
    ...
    it("should sync data when coming back online", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByTestId("offline-queue-count"),
  ...
  describe("Error Recovery Scenarios", () => {
    ...
    it("should handle AI service failures gracefully", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByRole("button", { name: /proceed without ai/i }),
        ).toBeInTheDocument();
      });

      // Should allow manual approval
      const proceedButton = screen.getByRole("button", {
        name: /proceed without ai/i,
      });
      await user.click(proceedButton);

      // Should continue workflow
      expect(
        screen.getByText(/photo saved for manual review/i),
      ).toBeInTheDocument();
    });
    ...
    it("should handle storage quota exceeded", async () => {
      ...
      mockSupabase.storage
        .from()
        .upload.mockRejectedValueOnce(new Error("Storage quota exceeded"));
  ...
  describe("Data Integrity Validation", () => {
    it("should maintain data consistency throughout workflow", async () => {
      ...
      const inspectionStore = useInspectionStore.getState();
      expect(inspectionStore.selectedProperty?.property_name).toBe(
        mockProperty.property_name,
      );
      expect(inspectionStore.selectedProperty?.street_address).toBe(
      ...
      expect(storeAfterChecklist.checklist?.[0].title).toBe(
        "Check Kitchen Faucet",
      );
      ...
      const updatedItem = storeAfterPhoto.checklist?.find(
        (item) => item.id === "item-1",
      );
