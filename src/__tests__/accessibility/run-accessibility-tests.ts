...
export const runComponentAccessibilityTest = async (
  ...
  const score = totalChecks > 0 ? Math.round((results.passes.length / totalChecks) * 100) : 0;
...
export const validateTouchTargets = (container: HTMLElement): boolean => {
  ...
  for (const element of interactiveElements) {
    ...
    if (rect.width < 44 || rect.height < 44) {
      console.error(`Touch target too small: ${element.tagName} (${rect.width}x${rect.height}px)`);
      return false;
    }
  }
 ...
 */
export const validateKeyboardNavigation = async (container: HTMLElement): Promise<boolean> => {
  ...
  for (const element of focusableElements) {
    ...
    if (!hasOutline && !hasBoxShadow && !hasBorder) {
      console.error(`Element lacks visible focus indicator: ${element.tagName}`);
      return false;
    }
  }
 ...
 */
export const validateColorContrast = async (container: HTMLElement): Promise<boolean> => {
...
- Focus on failed components: ${results.filter(r => r.status === 'FAIL').map(r => r.component).join(', ')}
...
export const runAccessibilityTestSuite = async (): Promise<void> => {
  ...
  results.forEach(result => {
    ...
    console.log(`${status} ${result.component}: ${result.score}% (${result.violations} violations)`);
  });
  ...
  if (hasFailures) {
    throw new Error('Accessibility tests failed. See report above for details.');
  }
