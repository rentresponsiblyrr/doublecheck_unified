...
describe("LearningSystem", () => {
  ...
  describe("Learning Data Recording", () => {
    it("should record classification learning data correctly", async () => {
      ...
      expect(
        vi.mocked(require("@/lib/logging/enterprise-logger").log.info),
  ...
  describe("Accuracy Calculations", () => {
    it("should calculate classification accuracy correctly", () => {
      ...
      // Test partial match
      const partialMatch =
        learningSystemPrivate.calculateClassificationAccuracy(
          { classification: { category: "safety", severity: "high" } },
  ...
  describe("Learning Insights Generation", () => {
    ...
    it("should calculate model performance metrics correctly", async () => {
      const performance =
        await learningSystem.getModelPerformance("v1.1.0-cag");
  ...
  describe("Error Handling and Resilience", () => {
    it("should handle database connection errors gracefully", async () => {
      // Mock database error
      vi.mocked(
        require("@/integrations/supabase/client").supabase.from,
      ).mockImplementation(() => ({
        insert: vi.fn(() => ({
      ...
      expect(
        vi.mocked(require("@/lib/logging/enterprise-logger").log.error),
      ).toHaveBeenCalled();
    });
